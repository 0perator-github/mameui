...
const options_entry emu_options::s_option_entries[] =
{
	...
	{ OPTION_WRITECONFIG ";wc",                          "1",         core_options::option_type::BOOLEAN,    "write configuration to (driver).ini on exit" },   // MAMEUI: Some more changes based on personal preference.
																																								// MAME has this set to "0"
	// search path options
	...
	{ OPTION_INIPATH,                                    "ini;ini/presets;.",     core_options::option_type::MULTIPATH,     "path to ini files" },  // MAMEUI: Switched the default ini path back to 'ini\'
	...
	{ OPTION_SNAPBILINEAR,                               "0",         core_options::option_type::BOOLEAN,    "specify if the snapshot/movie should have bilinear filtering applied" },  // MAMEUI: MAME has this one set to "1".
...
//      assert(iter != m_image_options_canonical.end());
		...
		// MAMEUI: Not sure if this is a problem anymore. It was noted that the assertion placed
		// here erroneously faults for BML3's that possess a 1802 slot.
