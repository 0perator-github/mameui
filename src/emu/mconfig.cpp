...
//              throw emu_fatalerror("Unknown slot option '%s' in slot '%s'", selval, owner.tag()+1);   // MAMEUI: emu_fatalerror was replaced with osd_printf_warning.
				osd_printf_warning("Unknown slot option '%s' in slot '%s'\n", selval, owner.tag()+1);   // Is this necessary? Exceptions are thrown for a reason...
		}
	}
