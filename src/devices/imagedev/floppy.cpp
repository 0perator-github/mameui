...
floppy_connector::floppy_connector(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	...
	formats(nullptr),
#if defined(MAMEUI_WINAPP) // MAMEUI: setting this to true is purely an aesthetic preference, probably patched by Robbbert.
	m_enable_sound(true),
#else
...
floppy_image_device::floppy_image_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, type, tag, owner, clock),
		...
		m_track_dirty(false),
		m_ready_counter(0),
#if defined(MAMEUI_WINAPP) // MAMEUI: m_enable_sound(true) necessitated that this also be true.
		m_make_sound(true),
#else
		m_make_sound(false),
#endif
