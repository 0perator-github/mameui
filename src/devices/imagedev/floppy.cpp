...
floppy_connector::floppy_connector(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	...
	formats(nullptr),
	m_enable_sound(true)    // MAMEUI: This seems to be a purely aesthetic preference.
{                           // MAME has 'm_enable_sound' set to false.
}
...
floppy_image_device::floppy_image_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, type, tag, owner, clock),
		...
		m_track_dirty(false),
		m_ready_counter(0),
		m_make_sound(true),     // MAMEUI: same reason as noted on line: 215.
		m_sound_out(nullptr),   // MAME has 'm_make_sound' set to false.
