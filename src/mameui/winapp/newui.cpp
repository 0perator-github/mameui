...
{
	if (!di)
		std::cout << "Error: dialog_add_trigger: Called with a null dialogbox pointer" << std::endl;
	assert(di);
	if (!trigger_flags)
		std::cout << "Error: dialog_add_trigger: Called with a zero trigger_flags" << std::endl;
