...
static bool win_append_menu(HMENU menu, UINT flags, UINT_PTR id, const wchar_t *item)
{
	if (!menu)
	{
		std::cerr << "Error: win_append_menu_utf8 called with a null menu handle." << std::endl;
		return false;
	}

	if (!item || !*item)
	{
		std::cerr << "Error: win_append_menu_utf8 called with an empty menu item string." << std::endl;
		return false;
	}
...
static int dialog_write(dialog_box *di, const void *ptr, size_t sz, int align)
{
	if (!di)
	{
		std::cerr << "Error: dialog_write: Called with a null dialogbox pointer" << std::endl;
		return 1;
	}
	if (!ptr || sz == 0)
	{
		std::cerr << "Error: dialog_write: Called with a empty buffer to write" << std::endl;
		return 1;
	}
...
static int dialog_write_string(dialog_box *di, const std::wstring_view str)
{
	if (!di)
	{
		std::cerr << "Error: dialog_write_string: Called with a null dialogbox pointer" << std::endl;
		return 1;
	}
...
static void win_dialog_exit(dialog_box *dialog)
{
	...
	if (!dialog)
	{
		std::cerr << "Error: win_dialog_exit: Called with a null dialogbox pointer" << std::endl;
		return;
	}
	assert(dialog);
...
static dialog_box *win_dialog_init(const std::wstring_view title, const struct dialog_layout *layout)
{
	...
	if (title.empty())
	{
		std::cerr << "Error: win_dialog_init: Called with an empty title string" << std::endl;
		return nullptr;
	}
...
static void dialog_trigger(HWND dlgwnd, WORD trigger_flags)
{
	...
	if (!di)
	{
		std::cerr << "Error: dialog_trigger: Called with a null dialogbox pointer" << std::endl;
		return;
	}
	assert(di);
	...
	for (dialog_info_trigger *trigger = di->trigger_first; trigger; trigger = trigger->next)
	{
		if (trigger->trigger_flags & trigger_flags)
		{
			...
			if (!dialog_item)
			{
				std::cerr << "Error: dialog_trigger: Could not find dialog item with ID " << trigger->dialog_item << std::endl;
				continue;
			}
			assert(dialog_item);
...
static int dialog_write_item(dialog_box *di, DWORD style, short x, short y, short width, short height, const std::wstring_view str, const wchar_t *class_name, WORD *id)
{
	if (!di)
	{
		std::cerr << "Error: dialog_write_item: Called with a null dialogbox pointer" << std::endl;
		return 1;
	}

	if (!class_name || !*class_name)
	{
		std::cerr << "Error: dialog_write_item: Called with an empty class name" << std::endl;
		return 1;
	}
	...
	WPARAM wparam, LPARAM lparam, void (*storeval)(void *param, int val), void *storeval_param)
{
	if (!di)
	{
		std::cerr << "Error: dialog_add_trigger: Called with a null dialogbox pointer" << std::endl;
		return 1;
	}

	if (!trigger_flags)
	{
		std::cerr << "Error: dialog_add_trigger: Called with a zero trigger_flags" << std::endl;
		return 1;
	}
...
static void dialog_prime(dialog_box *di)
{
	if (!di)
	{
		std::cerr << "Error: dialog_prime: Called with a null dialogbox pointer" << std::endl;
		return;
	}

	if (di->buffer.empty())
	{
		std::cerr << "Error: dialog_prime: Called with an empty dialogbox buffer" << std::endl;
		return;
	}
...
static void seqselect_settext(HWND editwnd)
{
	...
	if (stuff == nullptr)
	{
		std::cerr << "Error: seqselect_settext: Called with a null seqselect_info pointer" << std::endl;
	...
	std::cout << "seqselect_settext: " << new_seq << std::endl;
	// reset the selection
...
static int add_portselect_entry(dialog_box *di, short &x, short &y, std::wstring_view port_name, std::wstring_view port_suffix, ioport_field *field, bool is_analog, int seq_type)
{
	if (!di)
	{
		std::cerr << "Error: add_portselect_entry: Called with a null dialogbox pointer" << std::endl;
		return 1;
	}

	if (port_name.empty())
	{
		std::cerr << "Error: add_portselect_entry: Called with an empty port_name" << std::endl;
		return 1;
	}

	if (field == nullptr)
	{
		std::cerr << "Error: add_portselect_entry: Called with a null field pointer" << std::endl;
		return 1;
	}
...
static int win_dialog_add_portselect(dialog_box *dialog, ioport_field *field)
{
	...
	if (port_name.empty())
	{
		std::cout << "Error: win_dialog_add_portselect: Blank port_name encountered" << std::endl;
		return 1; // error
	}
...
static void win_dialog_runmodal(running_machine &machine, HWND wnd, dialog_box *dialog)
{
	if (!dialog)
	{
		std::cerr << "Error: win_dialog_runmodal: Called with a null dialog box pointer" << std::endl;
		return;
	}
...
static void customise_input(running_machine &machine, HWND wnd, const wchar_t *title, int player, int inputclass)
{
	...
	if (!dlg)
	{
		std::cerr << "Error: customise_input: Failed to create dialog box" << std::endl;
		return;
	}
...
static void load_item(HWND wnd, device_image_interface *img, bool is_save)
{
	...
	std::wstring as = mui_utf16_from_utf8string(slmap.find(opt_name)->second);
...
static void set_command_state(HMENU menu_bar, UINT command, UINT state)
{
	if (!menu_bar)
	{
		std::cerr << "error: set_command_state: Menu bar is null" << std::endl;
		return;
	}
	...
	if (!result)
	{
		...
		std::cerr << "error: " << error_code <<": set_command_state: SetMenuItemInfo failed for command " << command << std::endl;
	}
}
...
static void help_display(HWND wnd, const wchar_t *chapter)
{
	...
	if (!htmlhelp)
	{
		...
		if (!hhctrl_ocx)
		{
			std::cerr << "error: help_display: unable to load hhctrl.ocx" << std::endl;
			return;
		}
		...
		if (htmlhelp)
		{
			...
			std::cerr << "error: help_display: HtmlHelpW() not found" << std::endl;
...
//              win_use_mouse = !win_use_mouse;
	...
	//			std::cout << "Loading software part: " << instance << ", index: " << std::hex << mapindex << std::dec << std::endl;
...
static int win_setup_menus(running_machine &machine, HMENU menu_bar)
{
	...
	state_directoryname = short_name;

	if (!state_generate_filename(full_name))
	{
		std::cout << "Unable to create state filename for " << machine.system().name << std::endl;
		return 1;
	}
