...
constexpr UINT SEQWM_SETFOCUS = WM_APP + 549;
constexpr UINT SEQWM_KILLFOCUS = WM_APP + 550;

constexpr short DIM_VERTICAL_SPACING = 3;
constexpr short DIM_HORIZONTAL_SPACING = 5;
constexpr short DIM_NORMAL_ROW_HEIGHT = 10;
constexpr short DIM_COMBO_ROW_HEIGHT = 12;
constexpr short DIM_SLIDER_ROW_HEIGHT = 18;
constexpr short DIM_BUTTON_ROW_HEIGHT = 12;
constexpr short DIM_EDIT_WIDTH = 120;
constexpr short DIM_BUTTON_WIDTH = 50;
constexpr short DIM_ADJUSTER_SCR_WIDTH = 12;
constexpr short DIM_ADJUSTER_HEIGHT = 12;
constexpr short DIM_SCROLLBAR_WIDTH = 10;
constexpr short DIM_BOX_VERTSKEW = -3;

constexpr wchar_t DLGTEXT_OK[] = L"OK";
constexpr wchar_t DLGTEXT_APPLY[] = L"Apply";
constexpr wchar_t DLGTEXT_CANCEL[] = L"Cancel";

constexpr WORD FONT_SIZE = 8;
constexpr wchar_t FONT_FACE[] = L"Arial";

constexpr unsigned int SEQSELECT_POLL_TIMER_ID = 0xFACE;

constexpr unsigned short ID_FRAMESKIP_0 = 10000;
constexpr unsigned short ID_DEVICE_0 = 11000;
constexpr unsigned short ID_JOYSTICK_0 = 12000;
constexpr unsigned short ID_VIDEO_VIEW_0 = 14000;
constexpr unsigned short ID_SWPART = 15000;

constexpr unsigned short MAX_JOYSTICKS = 8;

constexpr unsigned int SCROLL_DELTA_LINE = 10;
constexpr unsigned int SCROLL_DELTA_PAGE = 100;

constexpr long DIALOG_STYLE = WS_POPUP | WS_BORDER | WS_SYSMENU | DS_MODALFRAME | WS_CAPTION | DS_SETFONT;
constexpr unsigned int MAX_DIALOG_HEIGHT = 200;
...
constexpr WORD dlgitem_button[] = { 0xFFFF, 0x0080 };
constexpr WORD dlgitem_edit[] = { 0xFFFF, 0x0081 };
constexpr WORD dlgitem_static[] = { 0xFFFF, 0x0082 };
//constexpr WORD dlgitem_listbox[] = { 0xFFFF, 0x0083 }; // Listbox controls are not currently used in the codebase, but this is kept for potential future use.
constexpr WORD dlgitem_scrollbar[] = { 0xFFFF, 0x0084 };
constexpr WORD dlgitem_combobox[] = { 0xFFFF, 0x0085 };

static const wchar_t *DLGITEM_BUTTON = reinterpret_cast<const wchar_t*>(dlgitem_button);
static const wchar_t *DLGITEM_EDIT = reinterpret_cast<const wchar_t*>(dlgitem_edit);
static const wchar_t *DLGITEM_STATIC = reinterpret_cast<const wchar_t*>(dlgitem_static);
//static const wchar_t *DLGITEM_LISTBOX = reinterpret_cast<const wchar_t*>(dlgitem_listbox);
static const wchar_t *DLGITEM_SCROLLBAR = reinterpret_cast<const wchar_t*>(dlgitem_scrollbar);
static const wchar_t *DLGITEM_COMBOBOX = reinterpret_cast<const wchar_t*>(dlgitem_combobox);
...
//	-------------------------------------------------------
//  truncate_long_text_utf8 - This function truncates a long UTF-8 string, replacing the
//	end with ... for displaying long text in the UI, such as in tooltips or labels.
//	Parameters:
// 		incoming - the input UTF-8 string to be truncated
// 		character_count - the maximum number of characters to keep before truncation
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  truncate_long_text - This function truncates a long wide string, replacing the
//	end with ... for displaying long text in the UI, such as in tooltips or labels.
//	Parameters:
// 		incoming - the input wide string to be truncated
// 		character_count - the maximum number of characters to keep before truncation
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  convert_pipe_to_null_delimited_string - This function converts a
//	pipe-delimited string into a null-delimited string.
//	Parameters:
// 		input - the input pipe-delimited string to be converted
//	-------------------------------------------------------
...
#if 0 // This function is not used in the current codebase, but is kept for potential future use.
//	-------------------------------------------------------
//  convert_null_to_pipe_delimited_string - This function converts a
//	null-delimited string into a pipe-delimited string.
//	Parameters:
//		input - the input null-delimited string to be converted
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  win_get_file_name_dialog - This function invokes the Windows file
//	open/save dialog
//	Parameters:
//		ofn - pointer to a win_open_file_name structure containing dialog
//		parameters
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  win_scroll_window - This function handles scrolling of a window
//	Parameters:
//		window - handle to the window to be scrolled
//		wparam - WPARAM from the WM_VSCROLL or WM_HSCROLL message
//		scroll_bar - SB_VERT or SB_HORZ
//		scroll_delta_line - the amount to scroll for line up/down
//	-------------------------------------------------------
...
#if 0 // This function is not used in the current codebase, but is kept for potential future use.
//	-------------------------------------------------------
//  win_append_menu - This function appends a menu item to a menu
//	Parameters:
//		menu - handle to the menu to append to
//		flags - menu item flags (MF_STRING, MF_SEPARATOR, etc.)
//		id - menu item ID
//		item - menu item string
//	-------------------------------------------------------
...
#endif // This function is not used in the current codebase, but is kept for potential future use.
//	-------------------------------------------------------
//  win_append_menu_utf8 - This function appends a menu item to a menu using
//	UTF-8 encoding
//	Parameters:
//		menu - handle to the menu to append to
//		flags - menu item flags (MF_STRING, MF_SEPARATOR, etc.)
//		id - menu item ID
//		item - menu item string in UTF-8 encoding
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  call_windowproc - This function calls the appropriate window procedure
//	Parameters:
//		wndproc - the window procedure to call
...
//	-------------------------------------------------------
//  dialog_write - This function writes data to the dialog box buffer
//	Parameters:
//		di - pointer to the dialog box structure
//		ptr - pointer to the data to write
//		sz - size of the data to write
...
//	-------------------------------------------------------
//  dialog_write_string - This function writes a string to the dialog box buffer
//	Parameters:
//		di - pointer to the dialog box structure
...
//	-------------------------------------------------------
//  win_dialog_exit - This function cleans up and exits a dialog box
//	Parameters:
//		dialog - pointer to the dialog box structure to clean up
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  win_dialog_init - This function initializes a dialog box structure
//	Parameters:
//		title - the title of the dialog box
//		layout - pointer to the dialog layout structure (optional)
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  calc_dlgunits_multiple - This function calculates the conversion factors
//	from pixels to dialog units for both X and Y dimensions.
//	Parameters:
//		None
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_trigger - This function triggers the appropriate actions for a dialog box
//	Parameters:
//		dlgwnd - handle to the dialog window
//		trigger_flags - flags indicating which triggers to activate
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_proc - This function is the window procedure for the dialog box
//	Parameters:
//		dlgwnd - handle to the dialog window
...
//	-------------------------------------------------------
//  dialog_write_item - This function writes a dialog item to the dialog box
//	buffer
//	Parameters:
//		di - pointer to the dialog box structure
//		style - style of the dialog item
//		x - x position of the dialog item
//		y - y position of the dialog item
//		width - width of the dialog item
//		height - height of the dialog item
//		str - string associated with the dialog item
//		class_name - class name of the dialog item
//		id - pointer to store the ID of the dialog item
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_add_trigger - This function adds a trigger to the dialog box
//	Parameters:
//		di - pointer to the dialog box structure
//		dialog_item - ID of the dialog item to trigger
//		trigger_flags - flags indicating which triggers to activate
//		message - message to send to the dialog item
//		trigger_proc - function to call for the trigger
//		wparam - WPARAM for the message
//		lparam - LPARAM for the message
//		storeval - function to store the result of the trigger
//		storeval_param - parameter to pass to the storeval function
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_scrollbar_init - This function initializes the scrollbar for the dialog box
//	Parameters:
//		dialog - pointer to the dialog box structure
//		dlgwnd - handle to the dialog window
//		message - message to send to the dialog item
//		wparam - WPARAM for the message
//		lparam - LPARAM for the message
...
//	-------------------------------------------------------
//  dialog_add_scrollbar - This function adds a scrollbar to the dialog box
//	Parameters:
//		dialog - pointer to the dialog box structure
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_prime - This function finalizes the dialog box template before creation
//	Parameters:
//		di - pointer to the dialog box structure
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_get_combo_value - This function retrieves the selected value from
//	a combo box in the dialog
//	Parameters:
//		dialog - pointer to the dialog box structure
//		dialog_item - handle to the combo box dialog item
//		message - message to send to the dialog item
//		wparam - WPARAM for the message
//		lparam - LPARAM for the message
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_get_adjuster_value - This function retrieves the value from
//	an adjuster control in the dialog
//	Parameters:
//		dialog - pointer to the dialog box structure
//		dialog_item - handle to the adjuster dialog item
//		message - message to send to the dialog item
//		wparam - WPARAM for the message
//		lparam - LPARAM for the message
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_new_control - This function calculates the position for
//	a new control in the dialog
//	Parameters:
//		di - pointer to the dialog box structure
//		x - pointer to store the x position of the new control
//		y - pointer to store the y position of the new control
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_finish_control - This function updates the dialog box size after
//	adding a new control
//	Parameters:
//		di - pointer to the dialog box structure
//		x - x position of the new control
//		y - y position of the new control
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  dialog_combo_changed - This function is called when a combo box
//	selection changes
//	Parameters:
//		dialog - pointer to the dialog box structure
//		dlgitem - handle to the combo box dialog item
//		message - message to send to the dialog item
//		wparam - WPARAM for the message (contains the changed callback)
//		lparam - LPARAM for the message (contains the changed callback parameter)
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  win_dialog_add_active_combobox - This function adds an active combo box to the dialog
//	Parameters:
//		dialog - pointer to the dialog box structure
//		item_label - label for the combo box
//		default_value - default selected value for the combo box
//		storeval - function to store the selected value
//		storeval_param - parameter to pass to the storeval function
//		changed - function to call when the selection changes (optional)
//		changed_param - parameter to pass to the changed function (optional)
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  win_dialog_add_combobox - This function adds a combo box to the dialog
//	Parameters:
//		dialog - pointer to the dialog box structure
//		item_label - label for the combo box
//		default_value - default selected value for the combo box
...
//	-------------------------------------------------------
//  win_dialog_add_combobox_item - This function adds an item to a combo box
//	in the dialog
//	Parameters:
//		dialog - pointer to the dialog box structure
//		item_label - label for the combo box item
//		item_data - data associated with the combo box item
//	-------------------------------------------------------
...
//	-------------------------------------------------------
//  adjuster_sb_wndproc - This function is the window procedure for the adjuster scrollbar
//	Parameters:
//		sbwnd - handle to the scrollbar window
//		msg - the message to process
//		wparam - WPARAM for the message
//		lparam - LPARAM for the message
//	-------------------------------------------------------
...
static void seqselect_start_read_from_main_thread(void *param)
{
	...
	windows::set_timer(editwnd, SEQSELECT_POLL_TIMER_ID, 10, nullptr);
}
...
static INT_PTR CALLBACK seqselect_wndproc(HWND editwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_TIMER:
		if (wparam == SEQSELECT_POLL_TIMER_ID && stuff->poll_state == SEQSELECT_STATE_POLLING)
		{
			...
			{
				windows::kill_timer(editwnd, SEQSELECT_POLL_TIMER_ID);
...
static void prepare_menus(HWND wnd)
{
	...
	for (device_slot_interface &slot : slot_interface_enumerator(window->machine().root_device()))
	{
		...
		std::string menu_text(slot.slot_name());

		// name this option
		std::string_view opt_name = window->machine().options().slot_option(slot.slot_name()).value();

		const device_slot_interface::slot_option *option = slot.option(opt_name);

		sub_menu = menus::create_menu();
		// add the slot
		menus::append_menu_utf8(slot_menu, MF_POPUP, (UINT_PTR)sub_menu, menu_text.c_str());
		// build a list of user-selectable options
		std::vector<device_slot_interface::slot_option*> option_list;
		for (auto &option_entry : slot.option_list())
			if (option_entry.second->selectable())
				option_list.push_back(option_entry.second.get());
		...
		slot_map[cnt] = slot_data{ menu_text, "" };
		menu_flags = MF_STRING | (!option ? MF_CHECKED : 0);
		...
		{
			menu_text = std::string(opt->name()) + " (" + std::string(opt->devtype().fullname()) + ")";
			slot_map[cnt] = slot_data{ std::string(slot.slot_name()), std::string(opt->name())};
			menu_flags = MF_STRING | (opt->name() == opt_name ? MF_CHECKED : 0);
			menus::append_menu_utf8(sub_menu, menu_flags, cnt++, menu_text.c_str());
		}
	}
}
...
static void help_display(HWND wnd, const wchar_t *chapter)
{
	...
	if (!htmlhelp)
	{
		...
		if (htmlhelp)
		{
			htmlhelp(nullptr, nullptr, 28, (DWORD_PTR)&htmlhelp_cookie);
