...
typedef struct dialog_box_t dialog_box;
typedef struct dialog_info_trigger_t dialog_info_trigger;
typedef struct dialog_layout_t dialog_layout;
typedef struct dialog_object_pool_t dialog_object_pool;
typedef struct seqselect_info_t seqselect_info;
typedef struct win_open_file_name_t win_open_file_name;

typedef enum seqselect_state_t
{
...
typedef enum dialog_trigger_flag_t
{
...
typedef enum win_file_dialog_type_t
{
	WIN_FILE_DIALOG_OPEN = 1,
	WIN_FILE_DIALOG_SAVE
}win_file_dialog_type;

struct win_open_file_name_t
{
...
static bool win_get_file_name_dialog(win_open_file_name *ofn)
{
	...
	if (ofn->filter)
	{
		...
		std::unique_ptr<wchar_t[]> utf8_to_wcs(mui_wcstring_from_utf8(ofn->filter));
		if (!utf8_to_wcs)
			return result;
	...
	if (*(ofn->filename) != '\0')
	{
		...
		std::unique_ptr<wchar_t[]> utf8_to_wcs(mui_wcstring_from_utf8(ofn->filename));
		if (!utf8_to_wcs)
			return result;
	...
	{
		wcs_initial_directory = std::unique_ptr<wchar_t[]>(mui_wcstring_from_utf8(ofn->initial_directory));
	...
	switch(ofn->type)
	{
		...
		case WIN_FILE_DIALOG_SAVE:
			dialog_result = GetSaveFileNameW(&os_ofn);
			break;
	...
	if (wcs_file)
	{
		std::unique_ptr<char[]> wcs_to_utf8(mui_utf8_from_wcstring(wcs_file.get()));
		if (!wcs_to_utf8)
			return result;
...
static bool win_append_menu_utf8(HMENU menu, UINT flags, UINT_PTR id, const char *item)
{
	...
	{
		std::unique_ptr<const wchar_t[]> wcs_item(mui_wcstring_from_utf8(item));
		result = AppendMenuW(menu, flags, id, wcs_item.get());
	}
	else
		result = AppendMenuA(menu, flags, id, item);
...
static int dialog_write_string(dialog_box *di, const wchar_t *str)
{
	if (!str)
		str = L"";
	return dialog_write(di, str, (mui_wcslen(str) + 1) * sizeof(WCHAR), 2);
}
...
static void win_dialog_exit(dialog_box *dialog)
{
	...
	if (objpool)
	{
		for (i = 0; i < std::size(objpool->objects); i++)
			(void)DeleteObject(objpool->objects[i]);
	}

	if (dialog->handle)
		(void)GlobalFree(dialog->handle);
...
dialog_box *win_dialog_init(const char *title, const dialog_layout *layout)
{
	...
	wcs_title = std::unique_ptr<const wchar_t[]>(mui_wcstring_from_utf8(title));
...
static INT_PTR CALLBACK dialog_proc(HWND dlgwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	{
		case WM_INITDIALOG:
			(void)windows::set_window_long_ptr(dlgwnd, GWLP_USERDATA, (LONG_PTR) lparam);
		...
		case WM_COMMAND:
			...
			if (!mui_strcmp(buf.c_str(), DLGTEXT_OK))
				command = IDOK;
			else if (!mui_strcmp(buf.c_str(), DLGTEXT_CANCEL))
				command = IDCANCEL;
			else
				command = 0;
		...
		case WM_VSCROLL:
			if (lparam)
				...
				// pass it on
				(void) windows::send_message((HWND) lparam, msg, wparam, lparam);
			else
				// scroll the dialog
...
static int dialog_write_item(dialog_box *di, DWORD style, short x, short y, short width, short height, const char *str, const wchar_t *class_name, WORD *id)
{
	...
	std::unique_ptr<const wchar_t[]> wcs_str(mui_wcstring_from_utf8(str));
...
static LRESULT dialog_get_adjuster_value(dialog_box *dialog, HWND dialog_item, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	GetWindowTextW(dialog_item, buf, buf_size);
	...
	int min_value, int max_value, bool is_percentage, dialog_itemstoreval storeval, void *storeval_param)
{
	...
	if (!wcs_percent_string)
		return 1;
	if (dialog_add_trigger(dialog, dialog->item_count, TRIGGER_INITDIALOG, WM_SETTEXT, NULL, 0, (LPARAM)wcs_percent_string.get(), NULL, NULL))
		goto error;
...
static void seqselect_settext(HWND editwnd)
{
	...
	{
		DWORD start = 0, end = 0;
		(void) windows::send_message(editwnd, EM_GETSEL, (WPARAM) (LPDWORD) &start, (LPARAM) (LPDWORD) &end);
...
static void seqselect_start_read_from_main_thread(void *param)
{
	...
	std::shared_ptr<win_window_info> old_window_list;

	seq_poll = std::make_unique<switch_sequence_poller>(Machine->input());
	...
	old_window_list = std::static_pointer_cast<win_window_info>(osd_common_t::s_window_list.front());
...
static INT_PTR CALLBACK seqselect_wndproc(HWND editwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	switch(msg)
	{
		...
		case WM_SETFOCUS:
			PostMessageW(editwnd, SEQWM_SETFOCUS, 0, 0);
			break;

		case WM_KILLFOCUS:
			PostMessageW(editwnd, SEQWM_KILLFOCUS, 0, 0);
			break;
...
static int dialog_add_single_seqselect(dialog_box *di, short x, short y, short cx, short cy, ioport_field *field, int is_analog, int seqtype)
{
	...
	result = dialog_write_item(di, WS_CHILD | WS_VISIBLE | SS_ENDELLIPSIS | ES_CENTER | SS_SUNKEN, x, y, cx, cy, NULL, DLGITEM_EDIT, NULL);
	if (result)
		goto done;

	// allocate a seqselect_info
	stuff = new seqselect_info();
	...
	result = dialog_add_trigger(di, di->item_count, TRIGGER_INITDIALOG, 0, seqselect_setup, di->item_count, (LPARAM)stuff, NULL, NULL);
	if (result)
		goto done;

	result = dialog_add_trigger(di, di->item_count, TRIGGER_APPLY, 0, seqselect_apply, 0, 0, NULL, NULL);
...
static int win_dialog_add_portselect(dialog_box *dialog, ioport_field *field)
{
	...
	int seq_types[3]{SEQ_TYPE_STANDARD, SEQ_TYPE_DECREMENT, SEQ_TYPE_INCREMENT},
		is_analog[3]{true, false, false};
	std::string port_name;
	std::string port_suffix[3]{" Analog", " Dec", " Inc"};
	short x;
	short y;

	port_name = field->name();
	result = !port_name.empty();
	if (!result)
	...
	if (field->type() < IPT_ANALOG_FIRST && field->type() < IPT_ANALOG_LAST)
	{
		for (seq = 0; seq < seq_count; seq++)
		{
			...
			result = dialog_write_item(dialog, WS_CHILD | WS_VISIBLE | SS_LEFT | SS_NOPREFIX, x, y,
				dialog->layout->label_width, DIM_NORMAL_ROW_HEIGHT, std::string(port_name+port_suffix[seq]).c_str(), DLGITEM_STATIC, NULL);
			if (result)
				break;
	...
	else
	{
		...
		result = dialog_write_item(dialog, WS_CHILD | WS_VISIBLE | SS_LEFT | SS_NOPREFIX, x, y,
				dialog->layout->label_width, DIM_NORMAL_ROW_HEIGHT, port_name.c_str(), DLGITEM_STATIC, NULL);
		if (result)
			return result;
...
static char *win_dirname(const char *filename)
{
	...
	dirname = new char[mui_strlen(filename) + 1];
	if (!dirname)
		return NULL;

	// copy in the name
	(void)mui_strcpy(dirname, filename);

	// search backward for a slash or a colon
	for (c = dirname + mui_strlen(dirname) - 1; c >= dirname; c--)
		if (*c == '\\' || *c == '/' || *c == ':')
		{
...
static void state_dialog(HWND wnd, win_file_dialog_type dlgtype, DWORD fileproc_flags, bool is_load, running_machine &machine)
{
	...
	if (!state_filename.empty())
		dir = win_dirname(state_filename.c_str());
	else
	{
...
static bool get_softlist_info(HWND wnd, device_image_interface *img)
{
	...
	std::size_t rompath_size = mui_strlen(window->machine().options().emu_options::media_path()) + 2;

	/* Get the media_path */
	std::unique_ptr<char[]> rompath(new char [rompath_size]);
	std::fill_n(rompath.get(), rompath_size-1, '\0');
	(void)mui_strcpy(rompath.get(), window->machine().options().emu_options::media_path());
	...
	if (has_software)
	{
		...
		char* sl_root = strtok(rompath.get(), ";");
		while (sl_root && !passes_tests)
		{
			std::string test_path = sl_root + sl_dir;
			std::unique_ptr<wchar_t[]> wcs_path(mui_wcstring_from_utf8(test_path.c_str()));

			DWORD dwAttrib = GetFileAttributesW(wcs_path.get());
...
static bool get_menu_item_string(HMENU menu, UINT item, bool by_position, HMENU *sub_menu, LPWSTR buffer, size_t buffer_len)
{
	...
	if (!GetMenuItemInfoW(menu, item, by_position, &mii))
		return false;
...
static HMENU find_sub_menu(HMENU menu, const char *menutext, bool create_sub_menu)
{
	...
	while(*menutext)
	{
		std::unique_ptr<wchar_t[]> wcs_menutext(mui_wcstring_from_utf8(menutext));
		...
		if (!sub_menu && create_sub_menu)
		{
			...
			if (!SetMenuItemInfoW(menu, i, true, &mii))
			{
				i = GetLastError();
				return NULL;
			}
...
static void set_command_state(HMENU menu_bar, UINT command, UINT state)
{
	...
	mii.fState = state;
	(void)SetMenuItemInfoW(menu_bar, command, false, &mii);
}
...
static void setup_joystick_menu(running_machine &machine, HMENU menu_bar)
{
	...
	if (joystick_count > 0)
	{
		for (int i = 0; i < joystick_count; i++)
		{
			...
			win_append_menu_utf8(joystick_menu, MF_STRING, ID_JOYSTICK_0 + i, joystick_id.c_str());
			child_count++;
		}
	}
...
static void prepare_menus(HWND wnd)
{
	...
	do
	{
		get_menu_item_string(video_menu, 0, true, NULL, wcs_buf, std::size(wcs_buf));
	...
	{
		std::unique_ptr<const wchar_t[]> wcs_view_name(mui_wcstring_from_utf8(view_name));
	...
	for (device_slot_interface &slot : slot_interface_enumerator(window->machine().root_device()))
	{
		...
		win_append_menu_utf8(sub_menu, menu_flags, cnt++, "[Empty]");
		...
		for (device_slot_interface::slot_option *opt : option_list)
		{
			...
			if (opt->name()==opt_name)
				menu_flags |= MF_CHECKED;
			win_append_menu_utf8(sub_menu, menu_flags, cnt++, temp.c_str());
		}
	}
}
...
static void win_toggle_menubar(void)
{
	...
	for (const auto &window : osd_common_t::window_list())
	{
		...
		gdi::redraw_window(hwnd, NULL, NULL, 0);
	}
}
...
//#endif
		htmlhelp = (htmlhelpproc) GetProcAddress(LoadLibraryW(L"hhctrl.ocx"), "HtmlHelpW");
		if (!htmlhelp)
			return;
	...
	std::unique_ptr<const wchar_t[]> wcs_chapter(mui_wcstring_from_utf8(chapter));
...
//  shell::shell_execute(wnd, L"open", L"http://www.microsoft.com/directx", L"", NULL, SW_SHOWNORMAL);
	shell::shell_execute(wnd, L"open", wcs_chapter.get(), L"", NULL, SW_SHOWNORMAL);
}
...
{
	// convert to WCHAR
	std::unique_ptr<wchar_t[]> t_text(const_cast<wchar_t*>(mui_wcstring_from_utf8(text)));

	// invoke SetMenuItemInfo()
	MENUITEMINFOW mii = { sizeof(mii) };
	mii.fMask = MIIM_TYPE;
	mii.dwTypeData = t_text.get();
	SetMenuItemInfoW(menu_bar, command, false, &mii);
}
...
static int win_setup_menus(running_machine &machine, HMODULE module, HMENU menu_bar)
{
	...
	for(i = 0; i < frameskip_level_count(machine); i++)
	{
		...
		win_append_menu_utf8(frameskip_menu, MF_STRING, ID_FRAMESKIP_0 + i, buf.c_str());
	}
	...
	// initialize state_filename for each driver, so we don't carry names in-between them
	{
		...
		do
		{
			if (isalnum(*src) || mui_strchr("(),. \0", std::string_view(src, 1)))
				*(dst++) = *src;
		}
		while(*(src++));
	}
...
int winwindow_create_menu(running_machine &machine, HMENU *menus)
{
	...
	HMENU menu_bar = LoadMenu(module, menus::make_int_resource(IDR_RUNTIME_MENU));
