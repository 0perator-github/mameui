...
LPEXFOLDERDATA NewExtraFolderData(std::string title, std::size_t folderId, std::size_t parentId, DWORD iconId, DWORD subIconId = ICON_NONE, DWORD dwFlags = 0UL);
...
static LPTREEFOLDER NewFolder(std::string lpTitle, std::size_t nFolderId, std::size_t nParent, DWORD nIconId, DWORD dwFlags);
...
LPEXFOLDERDATA NewExtraFolderData(std::string title, std::size_t folderId, std::size_t parentId, DWORD iconId, DWORD subIconId, DWORD dwFlags)
{
...
static bool FolderHasIni(LPTREEFOLDER lpFolder)
{
	...
	if (lpFolder->m_nParent != FOLDER_NONE && FOLDER_SOURCE == treeFolders[lpFolder->m_nParent]->m_nFolderId)
		return true;
...
static bool AddFolder(LPTREEFOLDER lpFolder)
{
	...
	if (numFolders + 1 >= folderArrayLength)
	{
		...
		for (std::size_t index = 0;index < folderArrayLength;index++)
		{
...
bool InitFolders(void)
{
	...
	for (size_t i = 0; !g_lpFolderData[i].m_lpTitle.empty(); i++)
	{
		if (RequiredDriverCache() || (!RequiredDriverCache() && !g_lpFolderData[i].m_process))
		{
			...
			// create the folder
			(void)AddFolder(NewFolder(fData->m_lpTitle.c_str(), fData->m_nFolderId, FOLDER_NONE, fData->m_nIconId, dwFolderFlags));
		}
	}
...
// std::cout << "Trying to load " << numExtraIcons << " extra custom-folder icons" << "\n";
	...
	{
		ErrorMessageBox("Error with icon list--too few images.  %i < %i", ImageList_GetImageCount(hTreeSmall), (INT)ICON_MAX);
		return false;
	}
...
LPTREEFOLDER GetFolderByName(int nParentId, const char *pszFolderName)
{
	...
	if (!pszFolderName || nParentId == FOLDER_NONE || *pszFolderName == '\0')
		return nullptr;
...
static int InitExtraFolders(void)
{
	...
	for (const auto& entry : std::filesystem::directory_iterator(folder_path))
	{
		...
		ExtraFolderData[count] = NewExtraFolderData(std::move(stem), next_folder_id, FOLDER_NONE, icon[0] ? -icon[0] : IDI_FOLDER, icon[1] ? -icon[1] : IDI_FOLDER, F_CUSTOM);
		...
		if (!read_ini_buffer.empty() && read_ini_buffer.front() == '[')
		{
			...
			else
			{
				...
				ExtraFolderData[current_id] = NewExtraFolderData(
					...
					ExtraFolderData[extrafolder_id]->m_nFolderId, ICON_NONE, ICON_NONE,
...
DWORD GetTreeViewIconIndex(int icon_id)
{
	...
	for (DWORD i = 0; i < std::size(treeIconNames); i++)
