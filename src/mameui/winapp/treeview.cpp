...
void CreateSourceFolders(int parent_index)
{
	...
	for (size_t driver_index = 0; driver_index < driver_total; driver_index++)
	{
		...
		if (!folder_found)
		{
			...
			LPEXFOLDERDATA new_extraFolderData = NewExtraFolderData(driver_filename, next_folder_id, treeFolders[parent_index]->m_nFolderId, IDI_SOURCE);
...
void CreateScreenFolders(int parent_index)
{
	...
	for (size_t driver_index = 0; driver_index < driver_total; driver_index++)
	{
		...
		if (!folder_found)
		{
			...
			LPEXFOLDERDATA new_extraFolderData = NewExtraFolderData(screen_number, next_folder_id, treeFolders[parent_index]->m_nFolderId, IDI_SCREEN);
			if (!new_extraFolderData)
				continue;
		...
		while(mfr_iter && mfr_iter[0] != '\0')
		{
			...
			if(!parsed_ref.empty())
			{
				...
				if (!folder_found)
				{
					...
					LPTREEFOLDER new_folder = NewFolder(trimmed_ref, next_folder_id, parent_index, IDI_MANUFACTURER, GetFolderFlags(numFolders));
					if (!new_folder)
						continue;

					LPEXFOLDERDATA new_extraFolderData = NewExtraFolderData(parsed_ref, next_folder_id, treeFolders[parent_index]->m_nFolderId, IDI_MANUFACTURER);
					if (!new_extraFolderData)
						continue;
...
void CreateBIOSFolders(int parent_index)
{
	...
	for (size_t driver_index = 0; driver_index < driver_total; driver_index++)
	{
		...
		// Skip if no BIOS driver is found or if there is no fullname.
		const char *driver_fullname = driver_list::driver(nParentIndex).type.fullname();
		if (!driver_fullname || !*driver_fullname)
			continue;

		bool folder_found = false;
		for (int last_folder = numFolders - 1; last_folder >= start_folder; last_folder--)
		{
			if (!mui_strcmp(treeFolders[last_folder]->m_lpTitle, driver_fullname))
			{
				AddGame(treeFolders[last_folder], driver_index);
				folder_found = true;
				break;
	...
	for (size_t driver_index = 0; driver_index < driver_total; driver_index++)
	{
		...
		for (device_execute_interface &device : execute_interface_enumerator(config.root_device()))
		{
			...
			if (dev_name && *dev_name) // skip empty names
			{
				...
				{
					if (!mui_strcmp(device_folders[j]->m_lpTitle, dev_name))
					{
...
void CreateDeficiencyFolders(int parent_index)
{
	...
	for (int driver_index = 0; driver_index < driver_total; ++driver_index)
	{
		...
		{
			if (extract_bit(cache, spec.cacheBit))
...
void CreateYearFolders(int parent_index)
{
	...
	for (int driver_index = 0; driver_index < driver_total; driver_index++)
	{
		...
		year_end = mui_strlen(driver_list::driver(driver_index).year);
		if (year_end == MAX_PATH) // skip invalid entry
			continue;

		year_size = year_end + 1;
		std::unique_ptr<char[]> s(new char [year_size]);
		(void)mui_strcpy(s.get(), driver_list::driver(driver_index).year);
		...
		else
		{
			...
			else
			{
				for (screen_device &screen : screen_device_enumerator(config.root_device()))
				{
					...
					std::string_view orientation = (driver_list::driver(driver_index).flags & ORIENTATION_SWAP_XY) ? "V" : "H";
	...
	for (size_t i=0; i<numFolders; i++)
	{
		...
		if (lpFolder->m_nParent != index_parent)
		{
			...
			while (true)
			{
				...
				if (((LPTREEFOLDER)tvi.lParam) == treeFolders[lpFolder->m_nParent])
					break;
...
static int InitExtraFolders(void)
{
	...
	for (const auto& entry : std::filesystem::directory_iterator(folder_path))
	{
		...
		ExtraFolderData[count] = NewExtraFolderData(stem, next_folder_id, -1, icon[0] ? -icon[0] : IDI_FOLDER, icon[1] ? -icon[1] : IDI_FOLDER, F_CUSTOM);
...
bool TrySaveExtraFolder(LPTREEFOLDER lpFolder)
{
	...
	if (ofsSaveExtraFolder.is_open())
	{
		...
		ofsSaveExtraFolder << std::endl << "[ROOT_FOLDER]" << std::endl;
		...
		for (size_t i = 0; i < numFolders; i++)
		{
			...
			{
				ofsSaveExtraFolder << std::endl << "[" << folder_data->m_lpTitle << "]" << std::endl;

				for (size_t ii = 0; ii < driver_total; ii++)
				{
					if (TestBit(folder_data->m_lpGameBits, ii))
						ofsSaveExtraFolder << std::endl << driver_list::driver(ii).name;
				}
			}
		}
