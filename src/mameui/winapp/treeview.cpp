...
LPEXFOLDERDATA NewExtraFolderData(const char* title, int folderId, int parentId, int iconId, int subIconId = -1, DWORD dwFlags = 0UL);
...
void CreateSourceFolders(int parent_index)
{
	...
	for (size_t jj = 0; jj < nGames; jj++)
	{
		...
		if (i == start_folder-1)
		{
			...
			(void)NewExtraFolderData(s, next_folder_id, treeFolders[parent_index]->m_nFolderId, IDI_SOURCE);
			next_folder_id++;
		...
		while(mfr_iter && mfr_iter[0] != '\0')
		{
			...
			if(parsed_ref && parsed_ref[0] != '\0')
			{
				...
				if (i == start_folder - 1)
				{
					...
					LPTREEFOLDER lpTemp = NewFolder(trimmed_ref.c_str(), next_folder_id, parent_index, IDI_MANUFACTURER, GetFolderFlags(numFolders));
...
static const char *ParseManufacturer(const char *s, int *pParsedChars )
{
	...
	*ptmp = '\0';
	...
	if(contains_any_of)
	{
		ptmp = const_cast<char*>(mui_strchr(tmp, '('));
		if ( ptmp == nullptr )
		{
			ptmp = const_cast<char*>(mui_strchr(tmp, ','));
			if( ptmp != nullptr)
...
LPEXFOLDERDATA NewExtraFolderData(const char* title, int folderId, int parentId, int iconId, int subIconId,DWORD dwFlags)
{
...
{
	int count = 0;
	std::string ini_path = dir_get_value(DIRPATH_CATEGORYINI_PATH);
		...
		while (std::getline(file_stream, line))
		{
			if (line == "[FOLDER_SETTINGS]")
			{
				while (std::getline(file_stream, line))
				{
					...
					mui_stringtokenizer tokenizer(line, " =\t\r\n");

					if (tokenizer.has_next())
					{
						auto key = tokenizer.next_token();

						if (key == "RootFolderIcon" && tokenizer.has_next())
							SetExtraIcons(tokenizer.next_token(), &icon[0]);
						else if (key == "SubFolderIcon" && tokenizer.has_next())
							SetExtraIcons(tokenizer.next_token(), &icon[1]);
					}
				}
				break;
			}
...
bool TryAddExtraFolderAndChildren(int parent_index)
{
	...
	// "folder\title.ini"
	auto ini_path = std::filesystem::path(dir_get_value(DIRPATH_CATEGORYINI_PATH)) / (std::string(ExtraFolderData[id]->m_szTitle) + ".ini");
	...
	while (std::getline(file_stream, read_ini_buffer))
	{
		...
		// do we have [...] ?
		if (read_ini_buffer.front() == '[')
		{
			std::size_t start_pos = 1, end_pos = read_ini_buffer.find(']', start_pos);
			...
			else
			{
				...
				else
				{
					...
					lpTemp = NewFolder(name.c_str(), current_id, parent_index, ExtraFolderData[id]->m_nSubIconId, GetFolderFlags(numFolders) | F_CUSTOM);
					ExtraFolderData[current_id] = NewExtraFolderData(name.c_str(), current_id - MAX_EXTRA_FOLDERS, ExtraFolderData[id]->m_nFolderId, -1, -1, ExtraFolderData[id]->m_dwFlags);
					(void)AddFolder(lpTemp);
				}
			}
		}
		...
		{
			mui_stringtokenizer tokenizer(read_ini_buffer, " \t\r\n");
			if (tokenizer.has_next())
			{
				...
				if (lpTemp == nullptr)
				{
					ErrorMsg("Error parsing %s: missing [folder name] or [ROOT_FOLDER]", ini_path.string().c_str());
					current_id = lpFolder->m_nFolderId;
					lpTemp = lpFolder;
				}
...
static bool TryRenameCustomFolderIni(LPTREEFOLDER lpFolder, std::string_view old_name, std::string_view new_name)
{
	try
	{
		std::filesystem::path ini_dirpath = get_ini_dir_utf8();
...
{
	bool result = false;
	const std::string ini_dirpath = dir_get_value(DIRPATH_CATEGORYINI_PATH);
...
bool TrySaveExtraFolder(LPTREEFOLDER lpFolder)
{
	...
	const std::string ini_path = dir_get_value(DIRPATH_CATEGORYINI_PATH);
...
static void SaveExternalFolders(int parent_index, const char* fname)
{
	const std::string option_value = dir_get_value(DIRPATH_CATEGORYINI_PATH);
	...
	std::filesystem::path new_directory, categoryini_path;
	mui_stringtokenizer tokenizer(option_value, ";");
	while (tokenizer.has_next())
	{
		categoryini_path = tokenizer.next_token();
		if (std::filesystem::exists(categoryini_path) && std::filesystem::is_directory(categoryini_path))
		{
			path_exists = true;
			break;
		}
	...
	if (!path_exists)
	{
		// get the first entry if one exists
		tokenizer.reset_position();
		if (!tokenizer.has_next())
			categoryini_path = tokenizer.next_token();

		if (categoryini_path.empty() || !std::filesystem::create_directory(categoryini_path))
		{
			std::cout << "SaveExternalFolders: Unable to create the directory \"" << categoryini_path << "\"" << std::endl;
			return;
		}
	}

	// create/truncate file
	std::filesystem::path filename_path = categoryini_path / (std::string(fname) + ".ini");
	std::ofstream save_extern_ofs = std::ofstream(filename_path);
