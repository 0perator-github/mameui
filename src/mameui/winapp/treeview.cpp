...
static bool ci_contains(std::string string, std::string sub_string);
...
void ResetWhichGamesInFolders(void)
{
	...
	for (size_t i = 0; i < numFolders; i++)
	{
		...
		for (size_t k = 0; g_lpFolderData[k].m_lpTitle; k++)
		{
...
bool GameFiltered(int nGame, DWORD dwMask)
{
	...
	driver_fullname = driver_list::driver(nGame).type.fullname();
	driver_name = driver_list::driver(nGame).name;
	driver_manufacturer = driver_list::driver(nGame).manufacturer;
	driver_source = driver_list::driver(nGame).type.source();
	...
	if (mui_strlen(GetSearchText()) && mui_stricmp(GetSearchText(), &SEARCH_PROMPT[0]))
	{
...
void CreateSourceFolders(int parent_index)
{
	...
	for (size_t jj = 0; jj < nGames; jj++)
	{
		...
		if (i == start_folder-1)
		{
			...
			ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
			...
			title = ExtraFolderData[next_folder_id]->m_szTitle;
			(void)mui_strcpy(title, s);
...
void CreateManufacturerFolders(int parent_index)
{
	...
	for (size_t jj = 0; jj < nGames; jj++)
	{
		...
		{
			const char *parsed_ref = ParseManufacturer(mfr_iter, &iChars);
			mfr_iter += iChars;
			//shift to next start char
			if(parsed_ref && parsed_ref[0] != '\0')
			{
				...
				if (i == start_folder - 1)
				{
					...
					LPTREEFOLDER lpTemp = NewFolder(&trimmed_ref[0], next_folder_id, parent_index, IDI_MANUFACTURER, GetFolderFlags(numFolders));
					ExtraFolderData[next_folder_id] = new EXFOLDERDATA{};
					...
					ExtraFolderData[next_folder_id]->m_nSubIconId = -1;
					title = ExtraFolderData[next_folder_id]->m_szTitle;
					(void)mui_strcpy(title, parsed_ref);
					ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
					(void)AddFolder(lpTemp);
					AddGame(lpTemp, jj);
				}
...
static const char *ParseManufacturer(const char *s, int *pParsedChars )
{
	bool contains_all_of, contains_any_of;
	static char tmp[256];
	char *ptmp;
	char *t;
	*pParsedChars= 0;

	contains_any_of = (*s == '?' || *s == '<' || s[3] == '?');
	if (!s || contains_any_of)
	{
		(*pParsedChars) = mui_strlen(s);
		return "<unknown>";
	}
	...
	while (*s)
	{
		...
		contains_any_of = (s[1] == '(' || s[1] == '/' || s[1] == '+');
		contains_all_of = (*s == ' ' && contains_any_of);
		contains_any_of = (*s == ']' || *s == '/' || *s == '?');
		if (contains_all_of || contains_any_of)
		{
			(*pParsedChars)++;
			if (s[1] == '/' || s[1] == '+')
				(*pParsedChars)++;
		...
		contains_all_of = (s[1] == ',' && s[2] == ' ');
		contains_any_of = (s[3] == 's' || s[3] == 'd');
		if (contains_all_of && contains_any_of)		{
			//*ptmp++ = *s;
			++s;
			break;
	...
	*ptmp = '\0';
	t = tmp;
	contains_any_of = ( tmp[0] == '(' || tmp[mui_strlen(tmp)-1] == ')' || tmp[0] == ',');
	if(contains_any_of)
	{
		ptmp = mui_strchr(tmp, "(");
		if ( ptmp == NULL )
		{
			...
			if( ptmp != NULL)
			{
				...
				if (!mui_strnicmp(ptmp, " supported by", 13))
				{
					ptmp += 13;
				}
				else if (!mui_strnicmp(ptmp, " distributed by", 15))
				{
					ptmp += 15;
				}
				else
		...
		if (!mui_strnicmp(ptmp, "licensed from ", 14))
		{
			ptmp += 14;
		}
		// for the licenced from case
		if (!mui_strnicmp(ptmp, "licenced from ", 14))
		{
			ptmp += 14;
		}
		...
		while (true)
		{
			...
			if (*ptmp == ' ' && !mui_strnicmp(ptmp, " license", 8))
			{
				break;
			}
			if (*ptmp == ' ' && !mui_strnicmp(ptmp, " licence", 8))
			{
				break;
			}
			*t++ = *ptmp++;
		...
		for (device_execute_interface &device : execute_interface_enumerator(config.root_device()))
		{
			...
			if (dev_name) // skip null
			{
				...
				if (folder == NULL)
				{
					...
					ExtraFolderData[next_folder_id]->m_nSubIconId = -1;
					title = ExtraFolderData[next_folder_id]->m_szTitle;
					(void)mui_strcpy(title, dev_name);
					ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
					(void)AddFolder(lpTemp);
...
void CreateDeficiencyFolders(int parent_index)
{
	...
	title = "Unemulated Protection";
	lpProt = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	ExtraFolderData[next_folder_id]->m_nParent = lpFolder->m_nFolderId;
	ExtraFolderData[next_folder_id]->m_nSubIconId = -1;
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpProt);

	title = "Wrong Colors";
	lpWrongCol = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpWrongCol);

	title = "Imperfect Colors";
	lpImpCol = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpImpCol);

	title = "Imperfect Graphics";
	lpImpGraph = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpImpGraph);

	title = "Missing Sound";
	lpMissSnd = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpMissSnd);

	title = "Imperfect Sound";
	lpImpSnd = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpImpSnd);

	title = "No Cocktail";
	lpFlip = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, title.c_str());
	ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
	(void)AddFolder(lpFlip);

	title = "Requires Artwork";
	lpArt = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
...
void CreateDumpingFolders(int parent_index)
{
	...
	title = "Bad Dump";
	lpBad = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER_DUMP, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
	...
	title = "No Dump";
	lpNo = NewFolder(&title[0], next_folder_id, parent_index, IDI_FOLDER_DUMP, GetFolderFlags(numFolders));
	ExtraFolderData[next_folder_id] = new EXFOLDERDATA();
...
void CreateYearFolders(int parent_index)
{
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		for (i=numFolders-1;i>=start_folder;i--)
		{
			if (strncmp(treeFolders[i]->m_lpTitle, s.get(), year_size) == 0)
			{
				AddGame(treeFolders[i], jj);
				break;
			}
		...
		if (i == start_folder-1)
		{
			...
			ExtraFolderData[next_folder_id]->m_nParent = lpFolder->m_nFolderId;
			ExtraFolderData[next_folder_id]->m_nSubIconId = -1;
			(void)mui_strcpy(ExtraFolderData[next_folder_id]->m_szTitle, s.get());
			ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
			(void)AddFolder(lpTemp);
			AddGame(lpTemp, jj);
		}
		...
		else
		{
			...
			else
			{
				for (screen_device &screen : screen_device_enumerator(config.root_device()))
				{
					...
					folder_title = util::string_format("%d x %d (%c)", vertical_size, horizontal_size, (driver_list::driver(jj).flags & ORIENTATION_SWAP_XY) ? 'V':'H');
					...
					if (i == start_folder-1)
					{
						...
						ExtraFolderData[next_folder_id]->m_nSubIconId = -1;
						title = ExtraFolderData[next_folder_id]->m_szTitle;
						(void)mui_strcpy(title, folder_title.c_str());
						ExtraFolderData[next_folder_id++]->m_dwFlags = 0;
						(void)AddFolder(lpTemp);
						AddGame(lpTemp,jj);
					}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		else
		{
			...
			else
			{
				for (screen_device &screen : screen_device_enumerator(config.root_device()))
				{
					folder_title = util::string_format("%f Hz", ATTOSECONDS_TO_HZ(screen.refresh_attoseconds()));
...
void ResetTreeViewFolders(void)
{
	...
	for (size_t i=0; i<numFolders; i++)
	{
		...
		if (lpFolder->m_nParent == -1)
		{
			...
			tvi.pszText = lpFolder->m_lpwTitle;
		...
		tvi.iSelectedImage = 0;
		tvi.pszText = treeFolders[i]->m_lpwTitle;
...
static LPTREEFOLDER NewFolder(const char *lpTitle, UINT nFolderId, int nParent, UINT nIconId, DWORD dwFlags)
{
	size_t nTitleSize = mui_strlen(lpTitle) + 1;
	LPTREEFOLDER lpFolder = new TREEFOLDER();
	lpFolder->m_lpTitle = (LPSTR)new char[nTitleSize];
	(void)(void)mui_strcpy(lpFolder->m_lpTitle,lpTitle);
	lpFolder->m_lpwTitle = const_cast<wchar_t*>(mui_wcstring_from_utf8(lpFolder->m_lpTitle));
...
static void DeleteFolder(LPTREEFOLDER lpFolder)
{
	if (lpFolder)
	{
		...
		{
			delete[] lpFolder->m_lpTitle;
			lpFolder->m_lpTitle = 0;
		}

		if (lpFolder->m_lpwTitle)
		{
			delete[] lpFolder->m_lpwTitle;
			lpFolder->m_lpwTitle = 0;
		}
...
BOOL InitFolders(void)
{
	...
	if (folderArrayLength == 0)
	{
		...
		treeFolders = new TREEFOLDER *[folderArrayLength];
		if (!treeFolders)
		{
			folderArrayLength = 0;
			return 0;
	...
	for (size_t i = 0; g_lpFolderData[i].m_lpTitle; i++)
	{
		if (RequiredDriverCache() || (!RequiredDriverCache() && !g_lpFolderData[i].m_process))
		{
			...
			/* create the folder */
			(void)AddFolder(NewFolder(fData->m_lpTitle, fData->m_nFolderId, -1, fData->m_nIconId, dwFolderFlags));
		}
	}
	...
	for (size_t i = 0; i < numExtraFolders; i++)
	{
		...
		if (RequiredDriverCache())
			for (size_t ii = 0; g_lpFolderData[ii].m_lpTitle; ii++)
			{
				if (!mui_strcmp(fExData->m_szTitle, g_lpFolderData[ii].m_lpTitle))
				{
					folder_exists = true;
					break;
				}
		...
		if (!folder_exists)
		{
			(void)AddFolder(NewFolder(fExData->m_szTitle, fExData->m_nFolderId, fExData->m_nParent, fExData->m_nIconId, dwFolderFlags));
...
static void TreeCtrlOnPaint(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	(void)CallWindowProcW(g_lpTreeWndProc, hWnd, uMsg, (WPARAM)memDC, 0);
	...
	if (gdi::get_device_caps(hDC, RASTERCAPS) & RC_PALETTE && hPAL != NULL)
	{
...
static LRESULT CALLBACK TreeWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	/* message not handled */
	return CallWindowProcW(g_lpTreeWndProc, hWnd, uMsg, wParam, lParam);
}
...
static int InitExtraFolders(void)
{
	...
	std::fill(std::begin(ExtraFolderData), std::end(ExtraFolderData), (LPEXFOLDERDATA)0);

	// Save the current working directory
	_getcwd(working_directory, MAX_PATH);

	// Change to the ini path
	ini_path = dir_get_value(DIRMAP_CATEGORYINI_PATH);

	// bail with count 0 if directory doesn't exist
	if(_chdir(ini_path.c_str()) != 0) 
		return count;

	// Reset global count of extra icons
	numExtraIcons = 0;
	...
	do
	{
		...
		{
			std::ifstream file_stream(files.name);
			...
			{
				int icon[2] = { 0, 0 };
				std::string ext, stem, read_ini_buffer;

				while (std::getline(file_stream, read_ini_buffer))
				{
					if (read_ini_buffer[0] == '[')
					{
						std::string name;

						start_pos = 1;
						end_pos = read_ini_buffer.rfind(']');
						if (end_pos == std::string::npos)
							continue;
						else
							end_pos--;

						name = std::string(read_ini_buffer, start_pos, end_pos);
						if (name == "FOLDER_SETTINGS")
						{
							while (std::getline(file_stream, read_ini_buffer))
							{
								name = mui_strtok(read_ini_buffer, " =\r\n");
								if (name.empty())
									break;

								if (name == "RootFolderIcon")
								{
									name = mui_strtok("", " =\r\n");
									if (!name.empty())
										SetExtraIcons(&name[0], &icon[0]);
								}
								else if (name == "SubFolderIcon")
								{
									name = mui_strtok("", " =\r\n");
									if (!name.empty())
										SetExtraIcons(&name[0], &icon[1]);
								}
							}
							break;
						}
				...
				ini_filename = std::string(files.name);
				start_pos = 0;
				end_pos = ini_filename.rfind('.');
				if (end_pos == std::string::npos)
					continue;
				stem = std::string(ini_filename, start_pos, end_pos);
				ext = std::string(ini_filename, end_pos + 1, ini_filename.size());
				if (ext == "ini")
				{
					ExtraFolderData[count] = new EXFOLDERDATA{};
					if (ExtraFolderData[count])
					{
						char* title = ExtraFolderData[count]->m_szTitle;

						(void)mui_strcpy(title, stem.c_str());
...
BOOL TryAddExtraFolderAndChildren(int parent_index)
{
	...
	// "folder\title.ini"
	ini_path = util::string_format("%s\\%s.ini", dir_get_value(DIRMAP_CATEGORYINI_PATH), ExtraFolderData[id]->m_szTitle);
	std::ifstream file_stream(&ini_path[0]);
	if (!file_stream.good())
		return false;
...
//		char *name;
		...
		{
			start_pos = 1;
			end_pos = read_ini_buffer.find(']', start_pos);
			if (end_pos == std::string::npos)
				continue;
			else
				end_pos--;
			...
			else
			{
				...
				else
				{
					...
					lpTemp = NewFolder(&name[0], current_id, parent_index, ExtraFolderData[id]->m_nSubIconId, GetFolderFlags(numFolders) | F_CUSTOM);

					ExtraFolderData[current_id] = new EXFOLDERDATA{};
					...
					title = ExtraFolderData[current_id]->m_szTitle;
					(void)mui_strcpy(title, &name[0]);
		...
		else if (current_id != -1)
		{
			...
			if (lpTemp == 0)
			{
				ErrorMsg("Error parsing %s: missing [folder name] or [ROOT_FOLDER]", &ini_path[0]);
				current_id = lpFolder->m_nFolderId;
				lpTemp = lpFolder;
			}
			AddGame(lpTemp, GetGameNameIndex(&name[0]));
		}
	}
...
static BOOL TryRenameCustomFolderIni(LPTREEFOLDER lpFolder, const char *old_name, const char *new_name)
{
	...
	const std::string ini_dirpath = get_ini_dir_utf8(), ini_ext = ".ini"s;
	...
	if (lpFolder->m_nParent >= 0)
	{
		...
		if (lpParent)
		{
			...
			old_filepath = util::string_format("%s\\%s\\%s", ini_dirpath, lpParent->m_lpTitle, old_name);
			new_filepath = util::string_format("%s\\%s\\%s", ini_dirpath, lpParent->m_lpTitle, new_name);
			result = storage::move_file_utf8((old_filepath + ini_ext).c_str(), (new_filepath + ini_ext).c_str());
			//Rename the Directory, if it exists
			if (!result)
				result = storage::move_file_utf8(old_filepath.c_str(), new_filepath.c_str());
		}
	}
	else
	{
		//Rename the File, if it exists
		old_filepath = util::string_format("%s\\%s", ini_dirpath, old_name);
		new_filepath = util::string_format("%s\\%s", ini_dirpath, new_name);
		result = storage::move_file_utf8((old_filepath + ini_ext).c_str(), (new_filepath + ini_ext).c_str());
		//Rename the Directory, if it exists
		if (!result)
			result = storage::move_file_utf8(old_filepath.c_str(), new_filepath.c_str());
...
{
	BOOL result = 0;
	const size_t required_title_size = mui_strlen(new_name) + 1;
	std::string old_filepath, new_filepath;
	const std::string ini_dirpath = dir_get_value(DIRMAP_CATEGORYINI_PATH), ini_ext = ".ini";
	if (lpFolder->m_nParent >= 0)
	{
		...
		// set new title
		lpFolder->m_lpTitle = new char[required_title_size];
		(void)mui_strcpy(lpFolder->m_lpTitle, new_name);

		// try to save
		result = TrySaveExtraFolder(lpFolder);
		if (!result)
		{
		...
		{
			(void)TryRenameCustomFolderIni(lpFolder, old_title, new_name);
	...
	else
	{
		...
		old_filepath = util::string_format("%s\\%s" + ini_ext, ini_dirpath, lpFolder->m_lpTitle);
		new_filepath = util::string_format("%s\\%s" + ini_ext, ini_dirpath, new_name);

		result = storage::move_file_utf8(old_filepath.c_str(), new_filepath.c_str());
		if (!result)
		{
			std::string errmsg_rencustfile = util::string_format("Error while renaming custom file %s to %s", old_filepath, new_filepath);
			std::unique_ptr<char[]> utf8_mameuiname(mui_utf8_from_wcstring(&MAMEUINAME[0]));
			dialog_boxes::message_box_utf8(GetMainWindow(), &errmsg_rencustfile[0], utf8_mameuiname.get(), MB_OK | MB_ICONERROR);
		}
		else
		{
			(void)TryRenameCustomFolderIni(lpFolder, lpFolder->m_lpTitle, new_name);
			delete[] lpFolder->m_lpTitle;
			lpFolder->m_lpTitle = new char[required_title_size];
			(void)mui_strcpy(lpFolder->m_lpTitle, new_name);
		}
...
BOOL TrySaveExtraFolder(LPTREEFOLDER lpFolder)
{
	...
	const std::string ini_path = dir_get_value(DIRMAP_CATEGORYINI_PATH);
	...
	filename_path = util::string_format("%s\\%s.ini", ini_path, extra_folder->m_szTitle);
	...
	if (error)
	{
		std::string errmsg_savcustfile = util::string_format("Error while saving custom file %s", filename_path);
		std::unique_ptr<char[]> utf8_mameuiname(mui_utf8_from_wcstring(&MAMEUINAME[0]));
		dialog_boxes::message_box_utf8(GetMainWindow(), &errmsg_savcustfile[0], utf8_mameuiname.get(), MB_OK | MB_ICONERROR);
	}
...
static void SaveExternalFolders(int parent_index, const char *fname)
{
	std::ofstream save_extern_ofs;
	std::string filename_path,ini_path;
	const std::string option_value = dir_get_value(DIRMAP_CATEGORYINI_PATH);
	std::string::size_type end_pos;

	end_pos = option_value.find(';');
	if (end_pos != std::string::npos)
		ini_path = std::string(option_value, 0, end_pos - 1);
	else
	{
		if (!option_value.empty() && option_value.size() > 2)
			ini_path = option_value;
		else
		{
			std::cout << "SaveExternalFolders: Couldn't find the category ini folder. option_value = '" << option_value << "'" << std::endl;
			return;
		}
	}
	...
	{
		std::unique_ptr<const wchar_t[]> wcs_ini_path(mui_wcstring_from_utf8(ini_path.c_str()));
		if (!CreateDirectoryW(wcs_ini_path.get(), NULL) && GetLastError() == ERROR_PATH_NOT_FOUND)
		{
			std::cout << "SaveExternalFolders: Unable to create the directory \"" << ini_path << "\"" << std::endl;
			return;
		}
	}

	// create/truncate file
	filename_path = ini_path + "\\" + fname + ".ini";
	save_extern_ofs = std::ofstream(filename_path);
	if (!save_extern_ofs.is_open())
	{
		std::cout << "SaveExternalFolders: Unable to open file " << filename_path << " for writing." << std::endl;
		return;
	}

	// Populate the file
	save_extern_ofs << "[FOLDER_SETTINGS]" << std::endl;
	save_extern_ofs << "RootFolderIcon custom" << std::endl;
	save_extern_ofs << "SubFolderIcon custom" << std::endl;
	...
	{
		if (TestBit(folder_data->m_lpGameBits, i))
			save_extern_ofs << GetGameName(i) << std::endl;
	}
	...
	for (size_t i = 0; i < numFolders; i++)
	{
		...
		if (folder_data->m_nParent >= 0 && treeFolders[folder_data->m_nParent] == lpFolder)
		{
			...
			for (size_t ii = 0; ii < driver_list::total(); ii++)
			{
				if (TestBit(folder_data->m_lpGameBits, ii))
					save_extern_ofs << GetGameName(ii) << std::endl;
			}
		}
	}

	save_extern_ofs.close();
	std::cout << "SaveExternalFolders: Saved file " << filename_path << std::endl;
}

bool ci_contains(std::string string, std::string sub_string)
{
	if (string.empty() || string.size() < sub_string.size())
		return false;
	...
	for (char &character : sub_string)
		character = std::toupper(character);

	if (string.rfind(sub_string) != std::string::npos)
		return true;
