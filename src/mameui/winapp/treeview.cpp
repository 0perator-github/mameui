...
static void SaveExternalFolders(int parent_index, std::string fname);
...
static void SaveExternalFolders(int parent_index, std::string fname)
{
	...
	std::filesystem::path filename_path = categoryini_path / (fname + ".ini");
	std::ofstream save_extern_ofs = std::ofstream(filename_path);
	if (!save_extern_ofs.is_open())
	{
		std::cout << "SaveExternalFolders: Unable to open file " << filename_path.string() << " for writing." << std::endl;
		return;
	}
	...
	std::cout << "SaveExternalFolders: Saved file " << filename_path.string() << std::endl;
}
