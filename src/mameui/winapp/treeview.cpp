...
static void RebuildGameList(LPTREEFOLDER folder, const FOLDERDATA& folderData, int totalGames)
{
...
void CreateResolutionFolders(int parent_index)
{
	...
	for (int driver_index = 0; driver_index < driver_total; driver_index++)
	{
		if (DriverUsesVectorDisplay(driver_index))
			folder_title = "Vector";
		else
		{
			if (DriverNumScreens(driver_index) == 0)
