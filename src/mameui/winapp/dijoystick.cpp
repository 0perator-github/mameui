...
const struct osd_joystick_t  DIJoystick =
{
...
static struct directinput_joystick_t   This;
...
static int DIJoystick_is_joy_pressed(int joycode)
{
	...
	if (!(joy_num > 0 && joy_num < 20) || This.num_joysticks < joy_num)
		return 0;
	...
	{
		button--;
		if (!(button > 0 && button < 32) || button >= This.joysticks[joy_num].num_buttons
			|| dir != JOYCODE_DIR_BTN)
			return 0;
	...
	if (stick == JOYCODE_STICK_POV)
	{
		...
		num_pov = button / 4;
		if (!(num_pov > 0 && num_pov < 4) || num_pov >= This.joysticks[joy_num].num_pov)
			return 0;
	...
	if (!(axis > 0 && axis < 20) || This.joysticks[joy_num].num_axes < axis)
		return 0;
...
wchar_t *DIJoystick_GetPhysicalJoystickName(int num_joystick)
{
...
wchar_t *DIJoystick_GetPhysicalJoystickAxisName(int num_joystick, int num_axis)
{
...
{
	int name_size = _snwprintf(0, 0, L"%s (%s)", pdidi->tszProductName, pdidi->tszInstanceName) + 1;
	wchar_t *joystick_name = new wchar_t[name_size];

	This.joysticks[This.num_joysticks].guidDevice = pdidi->guidInstance;
	_snwprintf(joystick_name, name_size, L"%s (%s)", pdidi->tszProductName, pdidi->tszInstanceName);
	This.joysticks[This.num_joysticks].name = joystick_name;
	This.num_joysticks++;
...
static BOOL CALLBACK DIJoystick_EnumAxisObjectsProc(LPCDIDEVICEOBJECTINSTANCEW lpddoi, LPVOID pvRef)
{
	...
	joystick->axes[joystick->num_axes].name = new wchar_t[mui_wcslen(lpddoi->tszName) + 1];
	(void)mui_wcscpy(joystick->axes[joystick->num_axes].name, lpddoi->tszName);
	...
	{
		delete[] joystick->axes[joystick->num_axes].name;
		joystick->axes[joystick->num_axes].name = 0;
		return DIENUM_CONTINUE;
	}
...
typedef struct directx_return_code_t
{
	HRESULT     hr;
	const char *szError;
