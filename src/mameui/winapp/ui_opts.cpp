...
void winui_ui_options::create_index(std::ifstream &fp)
{
    ...
    while (std::getline(fp, file_line))
    {
        ...
        auto option_name = cursor.next_token();
        // Extract option_data using std::optional to allow for missing tokens
        auto option_data = cursor.has_next() ? cursor.next_token() : std::optional<std::string>{};

        // Insert into the map with an empty string if option_data is missing
        if (option_name)
            m_list[*option_name] = option_data.value_or("");  // This ensures an empty string is inserted if option_data is missing
    }
}
