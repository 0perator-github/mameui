...
void winui_ui_options::create_index(std::ifstream& fp)
{
	...
	while (std::getline(fp, file_line))
	{
		...
		std::string_view option_name = tokenizer.next_token();
		if (!tokenizer.has_next())
			continue;

		std::string_view option_data = tokenizer.next_token();
		if (!option_name.empty())
			m_list.emplace(std::string(option_name), std::string(option_data));
	}

	return;
}

bool winui_ui_options::ends_with_filters(std::string_view option_name)
{
	bool result = false;
	...
	result = (option_name.compare(option_name.size() - ending_part.size(), ending_part.size(), ending_part) == 0);
	return result;
}

std::string_view winui_ui_options::getter(const char* option_name)
{
	std::string_view option_value = ""sv;
	auto found_entry = m_list.find(option_name);

	if (found_entry != m_list.end())
		option_value = found_entry->second;

	return option_value;
}

void winui_ui_options::load_file(const char* filename)
{
	if (!m_filename)
		m_filename = filename;
...
void winui_ui_options::reset_and_save(const char* filename)
{
...
void winui_ui_options::save_file(const char* filename)
{
	...
	inistring = std::string("\nYOU CAN SAFELY DELETE THIS FILE TO RESET THE EMULATOR BACK TO DEFAULTS.\n\n$start\n");

	for (auto const& it : m_list)
		inistring += it.first + "\t"s + it.second + "\n"s;

	std::ofstream outfile(filename, std::ios::out | std::ios::trunc);
	const size_t inistring_size = inistring.size();
	outfile.write(&inistring[0], inistring_size);
	outfile.close();
	return;
}

void winui_ui_options::setter(const char* option_name, int value)
{
...
void winui_ui_options::setter(const char* option_name, std::string_view value)
{
	m_list[option_name] = value;
	save_file(m_filename);
}
