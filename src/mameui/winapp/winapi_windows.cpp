...
{
	HWND result = 0;
	std::unique_ptr<wchar_t[]>wcs_class_name, wcs_window_name;

	if (class_name)
		wcs_class_name = std::unique_ptr<wchar_t[]>(mui_wcstring_from_utf8(class_name));

	if (window_name)
		wcs_window_name = std::unique_ptr<wchar_t[]>(mui_wcstring_from_utf8(window_name));

	result = find_window(wcs_class_name.get(), wcs_window_name.get());
...
HWND mameui::winapi::windows::find_window_ex_utf8(HWND parent_window_handle, HWND child_window_handle, const char *class_name, const char *window_name)
{
	...
	result = find_window_ex(parent_window_handle, child_window_handle, wcs_class_name.get(), wcs_window_name.get());
...
ATOM mameui::winapi::windows::w(const LPWNDCLASSW window_class_struct)
{
