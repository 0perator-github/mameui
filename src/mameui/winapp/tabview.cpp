...
static int TabView_GetTabFromTabIndex(HWND hwndTabView, int tab_index)
{
	...
	std::cout << "invalid tab index " << tab_index << std::endl;
	return 0;
}
...
{
	std::cout << "TabView_Reset: A" << std::endl;
	struct TabViewInfo *pTabViewInfo;
	pTabViewInfo = GetTabViewInfo(hwndTabView);

	std::cout << "TabView_Reset: B" << std::endl;
	(void)TabCtrl_DeleteAllItems(hwndTabView);

	TC_ITEM tci{ TCIF_TEXT };
	tci.cchTextMax = 20;

	std::cout << "TabView_Reset: C" << std::endl;
	for (size_t i = 0; i < pTabViewInfo->nTabCount; i++)
	{
		if (!pTabViewInfo->pCallbacks->pfnGetShowTab || pTabViewInfo->pCallbacks->pfnGetShowTab(i))
		{
			std::unique_ptr<const wchar_t[]> wcs_text(mui_wcstring_from_utf8(pTabViewInfo->pCallbacks->pfnGetTabLongName(i)));
			if( !wcs_text)
				return;
			tci.pszText = const_cast<wchar_t*>(wcs_text.get());
			(void)TabCtrl_InsertItem(hwndTabView, i, &tci);
		}
	}
	std::cout << "TabView_Reset: E" << std::endl;
	TabView_UpdateSelection(hwndTabView);
	std::cout << "TabView_Reset: Finished" << std::endl;
}
...
{
	//assert(hwndTabView);
	std::cout << "SetupTabView: A" << std::endl;
	struct TabViewInfo *pTabViewInfo;
	...
	// And fill it out
	std::cout << "SetupTabView: B" << std::endl;
	...
	std::cout << "SetupTabView: C" << std::endl;
