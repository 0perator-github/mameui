...
/***************************************************************************
    Internal structures
 ***************************************************************************/
using DriversInfo = struct drivers_info
{
	int screenCount;
	bool isClone;
	bool isBroken;
...
{
	std::unique_ptr<wchar_t[]> wcs_name(mui_utf16_from_utf8cstring(cName));

	if(!wcs_name)
		return;

	HINSTANCE hErr = shell::shell_execute(hWnd, nullptr, wcs_name.get(), nullptr, nullptr, SW_SHOWNORMAL);
...
void DisplayTextFile(HWND hWnd, const char *cName)
{
	...
	dialog_boxes::message_box(nullptr, msg, wcs_name.get(), MB_OK);
}
...
static DriversInfo *GetDriversInfo(uint32_t driver_index)
{
	if (bFirst)
	{
		bFirst = false;
