...
static void InitDriversInfo(void)
{
	...
	for (uint32_t ndriver = 0; ndriver < total; ndriver++)
	{
		...
		gameinfo->isClone = ( !have_parent || (0 > parent_idx) || extract_bit(GetDriverCacheLower(parent_idx),9)) ? false : true;
		gameinfo->isBroken = (cache & 0x4040) ? true : false;  // (MACHINE_NOT_WORKING | MACHINE_MECHANICAL)
		gameinfo->supportsSaveState = extract_bit(cache, 7) ^ 1;  //MACHINE_SUPPORTS_SAVE
...
static int InitDriversCache(void)
{
	...
	for (uint32_t ndriver = 0; ndriver < total; ndriver++)
	{
		...
		gameinfo->supportsSaveState =  extract_bit(cache_lower, 7) ? false : true;  //MACHINE_SUPPORTS_SAVE
		gameinfo->isVertical        =  extract_bit(cache_lower, 2) ? true : false;  //ORIENTATION_XY
