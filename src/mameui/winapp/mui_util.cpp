...
static void InitDriversInfo(void)
{
	...
	for (uint32_t ndriver = 0; ndriver < total; ndriver++)
	{
		...
		if (has_parent)
		{
			...
			if (0 < parent_index)
				has_parent = extract_bit(GetDriverCacheLower(parent_index), 6); // IS_BIOS_ROOT
		}
		gameinfo->isClone = has_parent;
		gameinfo->isBroken = (cache & lower_cache_flags::NOT_WORKING);
		gameinfo->supportsSaveState = extract_bit(cache, 33); // SAVE_SUPPORTED
		gameinfo->isHarddisk = false;
		gameinfo->isVertical = extract_bit(cache, 2); // SWAP_XY
...
static int InitDriversCache(void)
{
	...
	for (uint32_t ndriver = 0; ndriver < total; ndriver++)
	{
		...
		gameinfo->isBroken          =  (cache_lower & lower_cache_flags::NOT_WORKING);
		gameinfo->supportsSaveState = extract_bit(cache_lower, 33); // SAVE_SUPPORTED
		gameinfo->isVertical        = extract_bit(cache_lower, 2); // SWAP_XY
...
{
	assert(driver_index < driver_list::total());
	return extract_bit(GetDriverCacheLower(driver_index), 6); // IS_BIOS_ROOT
}

bool DriverIsMechanical(int driver_index)
{
	assert(driver_index < driver_list::total());
	return extract_bit(GetDriverCacheLower(driver_index), 10); // MECHANICAL
}

bool DriverIsArcade(int driver_index)
{
	assert(driver_index < driver_list::total());
	uint64_t system_type = GetDriverCacheLower(driver_index) & lower_cache_flags::MASK_SYSTEMTYPE;
	return system_type == lower_cache_flags::SYSTEMTYPE_ARCADE;
}
