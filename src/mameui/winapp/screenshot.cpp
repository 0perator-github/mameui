...
int GetScreenShotWidth()
{
	...
	long absolute_value = std::labs(static_cast<LPBITMAPINFO>(m_hDIB)->bmiHeader.biWidth);
	if (absolute_value <= std::numeric_limits<int>::max())
		return static_cast<int>(absolute_value);
	else
		return 0;
}
...
int GetScreenShotHeight(void)
{
	...
	long absolute_value = std::labs(static_cast<LPBITMAPINFO>(m_hDIB)->bmiHeader.biHeight);
	if (absolute_value <= std::numeric_limits<int>::max())
		return static_cast<int>(absolute_value);
	else
		return 0;
...
bool LoadScreenShot(int driver_index, const std::string& lpSoftwareName, int nType)
{
	...
	//std::cout << "LoadScreenShot: H" << std::endl;
	if (loaded)
	{
