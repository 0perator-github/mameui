...
static BOOL AllocatePNG(util::png_info *p, HGLOBAL *phDIB, HPALETTE *pPal)
{
	...
	{
		HDC hDC = gdi::create_compatible_dc(0); /* Desktop DC */
		*pPal = gdi::create_half_tone_palette(hDC);
		(void)gdi::delete_dc(hDC);
	}
	else
	{
...
static BOOL LoadDIB(const char *filename, HGLOBAL *phDIB, HPALETTE *pPal, int pic_type)
{
	...
	{
		case TAB_ARTWORK:
			t = dir_get_value(DIRMAP_ARTPREV_PATH);
			zip_name = "artpreview";
			break;
		case TAB_BOSSES:
			t = dir_get_value(DIRMAP_BOSSES_PATH);
			zip_name = "bosses";
			break;
		case TAB_CABINET:
			t = dir_get_value(DIRMAP_CABINETS_PATH);
			zip_name = "cabinets";
			break;
		case TAB_CONTROL_PANEL:
			t = dir_get_value(DIRMAP_CPANELS_PATH);
			zip_name = "cpanel";
			break;
		case TAB_COVER:
			t = dir_get_value(DIRMAP_COVER_PATH);
			zip_name = "covers";
			break;
		case TAB_ENDS:
			t = dir_get_value(DIRMAP_ENDS_PATH);
			zip_name = "ends";
			break;
		case TAB_FLYER:
			t = dir_get_value(DIRMAP_FLYERS_PATH);
			zip_name = "flyers";
			break;
		case TAB_GAMEOVER:
			t = dir_get_value(DIRMAP_GAMEOVER_PATH);
			zip_name = "gameover";
			break;
		case TAB_HOWTO:
			t = dir_get_value(DIRMAP_HOWTO_PATH);
			zip_name = "howto";
			break;
		case TAB_LOGO:
			t = dir_get_value(DIRMAP_LOGOS_PATH);
			zip_name = "logo";
			break;
		case TAB_MARQUEE:
			t = dir_get_value(DIRMAP_MARQUEES_PATH);
			zip_name = "marquees";
			break;
		case TAB_PCB:
			t = dir_get_value(DIRMAP_PCBS_PATH);
			zip_name = "pcb";
			break;
		case TAB_SCORES:
			t = dir_get_value(DIRMAP_SCORES_PATH);
			zip_name = "scores";
			break;
		case TAB_SCREENSHOT:
			t = dir_get_value(DIRMAP_SNAPSHOT_DIRECTORY);
			zip_name = "snap";
			break;
		case TAB_SELECT:
			t = dir_get_value(DIRMAP_SELECT_PATH);
			zip_name = "select";
			break;
		case TAB_TITLE:
			t = dir_get_value(DIRMAP_TITLES_PATH);
			zip_name = "titles";
			break;
		case TAB_VERSUS:
			t = dir_get_value(DIRMAP_VERSUS_PATH);
			zip_name = "versus";
			break;
		default :
			// shouldn't get here
	...
	for (u8 extnum = 0; extnum < 3; extnum++)
	{
		...
		(void)mui_strcpy(fullpath, t.c_str());
		char tempfile[MAX_PATH];
		(void)mui_strcpy(tempfile, filename);
		...
		while (partpath && filerr)
		{
			...
			if (file_name)
			{
				...
				fname = std::string(system_name) + PATH_SEPARATOR + std::string(file_name) + ext;
				filerr = OpenRawDIBFile(partpath, fname.c_str(), file);

				// Try dir/system.zip/game.png
				if (filerr)
				{
					fname = std::string(file_name) + ext;
				...
				if (filerr)
				{
					fname = std::string(system_name) + "/" + std::string(file_name) + ext;
					filerr = OpenZipDIBFile(partpath, system_name, fname.c_str(), file, &buffer);
				}
			...
			if (filerr)
			{
				fname = std::string(system_name) + ext;
				filerr = OpenRawDIBFile(partpath, fname.c_str(), file);
			}
			...
			if (filerr)
			{
				fname = std::string(system_name) + PATH_SEPARATOR + std::string(system_name) + ext;
				filerr = OpenRawDIBFile(partpath, fname.c_str(), file);
			}
...
BOOL LoadScreenShot(int nGame, LPCSTR lpSoftwareName, int nType)
{
	...
	if (loaded)
	{
		...
		(void)ReleaseDC(GetMainWindow(),hdc);
	}
...
BOOL LoadDIBBG(HGLOBAL *phDIB, HPALETTE *pPal)
{
	...
	util::core_file::ptr background_file = NULL;
	if (pPal)
		(void)gdi::delete_palette(*pPal);

	// look for the raw file
	file_path = GetBgDir();
	if (!util::core_file::open(&file_path[0], OPEN_FLAG_READ, background_file))
	{
