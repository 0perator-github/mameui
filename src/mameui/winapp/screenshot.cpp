...
namespace
{
	...
	***************************************************************************/
	...
	bool png_read_bitmap_gui(util::core_file& mfile, HGLOBAL* phDIB, HPALETTE* pPAL)
	{
		...
		{
			std::cout << "PNG Unsupported color type " << p.color_type << " (has to be 2 or 3)" << std::endl;
		}

		if (p.interlace_method != 0)
		{
			std::cout << "PNG Interlace unsupported" << std::endl;
			return false;
		}
		...
		{
			std::cout << "PNG Unable to allocate memory to display screenshot" << std::endl;
			return false;
		}
	...
	bool LoadDIB(std::string_view full_name, HGLOBAL* phDIB, HPALETTE* pPal, int pic_type)
	{
		...
		{
		case TAB_ARTWORK:
			directory_paths = dir_get_value(DIRPATH_ARTPREV_PATH);
			directory_name = "artpreview";
			break;
		case TAB_BOSSES:
			directory_paths = dir_get_value(DIRPATH_BOSSES_PATH);
			directory_name = "bosses";
			break;
		case TAB_CABINET:
			directory_paths = dir_get_value(DIRPATH_CABINETS_PATH);
			directory_name = "cabinets";
			break;
		case TAB_CONTROL_PANEL:
			directory_paths = dir_get_value(DIRPATH_CPANELS_PATH);
			directory_name = "cpanel";
			break;
		case TAB_COVER:
			directory_paths = dir_get_value(DIRPATH_COVER_PATH);
			directory_name = "covers";
			break;
		case TAB_ENDS:
			directory_paths = dir_get_value(DIRPATH_ENDS_PATH);
			directory_name = "ends";
			break;
		case TAB_FLYER:
			directory_paths = dir_get_value(DIRPATH_FLYERS_PATH);
			directory_name = "flyers";
			break;
		case TAB_GAMEOVER:
			directory_paths = dir_get_value(DIRPATH_GAMEOVER_PATH);
			directory_name = "gameover";
			break;
		case TAB_HOWTO:
			directory_paths = dir_get_value(DIRPATH_HOWTO_PATH);
			directory_name = "howto";
			break;
		case TAB_LOGO:
			directory_paths = dir_get_value(DIRPATH_LOGOS_PATH);
			directory_name = "logo";
			break;
		case TAB_MARQUEE:
			directory_paths = dir_get_value(DIRPATH_MARQUEES_PATH);
			directory_name = "marquees";
			break;
		case TAB_PCB:
			directory_paths = dir_get_value(DIRPATH_PCBS_PATH);
			directory_name = "pcb";
			break;
		case TAB_SCORES:
			directory_paths = dir_get_value(DIRPATH_SCORES_PATH);
			directory_name = "scores";
			break;
		case TAB_SCREENSHOT:
			directory_paths = dir_get_value(DIRPATH_SNAPSHOT_DIRECTORY);
			directory_name = "snap";
			break;
		case TAB_SELECT:
			directory_paths = dir_get_value(DIRPATH_SELECT_PATH);
			directory_name = "select";
			break;
		case TAB_TITLE:
			directory_paths = dir_get_value(DIRPATH_TITLES_PATH);
			directory_name = "titles";
			break;
		case TAB_VERSUS:
			directory_paths = dir_get_value(DIRPATH_VERSUS_PATH);
...
bool LoadScreenShot(int nGame, const std::string& lpSoftwareName, int nType)
{
	bool isclone, loaded = false;
	int nParentIndex = -1;

	//std::cout << "LoadScreenShot: A" << std::endl;
	// Delete the last ones
	FreeScreenShot();

	//std::cout << "LoadScreenShot: B" << std::endl;
	isclone = DriverIsClone(nGame);
	if (isclone)
	{
		nParentIndex = GetParentIndex(&driver_list::driver(nGame));
	}

	//std::cout << "LoadScreenShot: C" << std::endl;
	...
	//std::cout << "LoadScreenShot: D" << std::endl;
	// If game, see if picture exists. Or, if no picture for the software, use game's picture.
	if (!loaded)
	{
		//std::cout << "LoadScreenShot: E" << std::endl;
		loaded = LoadDIB(driver_list::driver(nGame).name, &m_hDIB, &m_hPal, nType);
		//std::cout << "LoadScreenShot: F" << std::endl;
		// None? Try parent
		if (!loaded && isclone)
		{
			//std::cout << "LoadScreenShot: G" << std::endl;
			loaded = LoadDIB(driver_list::driver(nParentIndex).name, &m_hDIB, &m_hPal, nType);
		}
	}

	//std::cout << "LoadScreenShot: K" << std::endl;
	if (loaded)
	{
		HDC hdc = gdi::get_dc(GetMainWindow());
		m_hDDB = DIBToDDB(hdc, m_hDIB, nullptr);
		gdi::release_dc(GetMainWindow(), hdc);
	}

	std::cout << "LoadScreenShot: Finished" << std::endl;
	return loaded;
}
...
bool LoadDIBBG(HGLOBAL *phDIB, HPALETTE *pPal)
{
	bool success = false;
	std::error_condition filerr;
	std::string_view file_path = GetBgDir();
	...
	// look for the raw file
	filerr = util::core_file::open(&file_path[0], OPEN_FLAG_READ, background_file);
	if (!filerr)
	{
		success = png_read_bitmap_gui(*background_file, phDIB, pPal);
		background_file.reset();
	}
