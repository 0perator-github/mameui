...
constexpr auto ListView_GetItemRect(T1 hwnd, T2  i, T3  prc, T4  code) {
	return (BOOL)windows::send_message((hwnd), LVM_GETITEMRECT, (WPARAM)(int)(i), ((prc) ? (((RECT*)(prc))->left = (code), (LPARAM)(RECT*)(prc)) : (LPARAM)(RECT*)NULL));
}
#endif
...
typedef struct CompareProcParams_t
{
	HWND hwndPicker;
...
{
	LONG_PTR lpPickerInfo = windows::get_window_long_ptr(hWnd, GWLP_USERDATA);
	return (PickerInfo *) lpPickerInfo;
}
...
static LRESULT CALLBACK ListViewWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		...
		case WM_DESTROY:
			...
			(void)windows::set_window_long_ptr(hWnd, GWLP_USERDATA, (LONG_PTR) NULL);
			break;
	}
...
void Picker_SetViewID(HWND hwndPicker, int nViewID)
{
	...
	(void)gdi::redraw_window(hwndPicker, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME);
}
...
int Picker_GetSelectedItem(HWND hWnd)
{
	...
	nItem = list_view::get_next_item(hWnd, -1, LVIS_SELECTED | LVIS_FOCUSED);
	if (nItem < 0)
		return nItem;
...
void Picker_SetSelectedPick(HWND hWnd, uint32_t nIndex)
{
	...
	int nCount = list_view::get_item_count(hWnd);
	// No games to show
	if (nCount < 1)
		return;
	nCount--;
	if (nCount < nIndex)
		nIndex = nCount;
	// Highlight a game
	list_view::set_item_state(hWnd, nIndex, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);
	// Bring the game into view
	(void)list_view::ensure_visible(hWnd, nIndex, false);
}
...
void Picker_SetSelectedItem(HWND hWnd, uint32_t nItem)
{
	...
	lvfi.lParam = nItem;
	int i = list_view::find_item(hWnd, -1, &lvfi);
	if (i == -1)
	{
		POINT p = {0,0};
		lvfi.flags = LVFI_NEARESTXY;
		lvfi.pt = p;
		i = list_view::find_item(hWnd, -1, &lvfi);
	}
...
static int CALLBACK Picker_CompareProc(LPARAM index1, LPARAM index2, LPARAM nParamSort)
{
	...
	wchar_t szBuffer1[MAX_PATH], szBuffer2[MAX_PATH];
	const wchar_t *s1, *s2;
...
void Picker_Sort(HWND hwndPicker)
{
	...
	int nItem = list_view::find_item(hwndPicker, -1, &lvfi);

	list_view::ensure_visible(hwndPicker, nItem, false);
}
...
static LPCWSTR MakeShortString(HDC hDC, LPCWSTR lpszLong, int nColumnLen, int nOffset)
{
	...
	(void)GetTextExtentPoint32W(hDC, lpszLong, lpszLong_len, &size);
	if (lpszLong_len == 0 || size.cx + nOffset <= nColumnLen)
		return lpszLong;

	(void)mui_wcscpy(szShort, lpszLong);
	(void)GetTextExtentPoint32W(hDC, szThreeDots, std::size(szThreeDots), &size);
	int nAddLen = size.cx;

	for (size_t i = lpszLong_len; i > 0; i--)
	{
		szShort[i] = 0;
		(void)GetTextExtentPoint32W(hDC, szShort, i, &size);
...
void Picker_HandleDrawItem(HWND hWnd, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	lvi.pszText    = szBuff;
	lvi.cchTextMax = std::size(szBuff);
	...
	if (hBackground)
	{
		...
		if (nItem == list_view::get_item_count(hWnd) - 1)
	...
	for (nColumn = 1; nColumn < nColumnMax; nColumn++)
	{
		...
		lvItem.cchTextMax = std::size(szBuff);
...
{
	HWND hwndHeader;
	hwndHeader = list_view::get_header(hwndPicker);
