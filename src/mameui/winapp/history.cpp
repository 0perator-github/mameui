...
static bool create_index(std::string_view datsdir, std::ifstream& fp, int filenum)
{
	...
	while (std::getline(fp, file_line))
	{
		if ((std::count(file_line.begin(), file_line.end(), '=') == 1) && (file_line[0] == '$')) // line must start with $ and contain one =
		{
			...
			auto first_token = cursor.next_token();
			if (first_token && !first_token->empty())
			{
				const std::string& first = *first_token;

				while (auto second_token = cursor.next_token())
				{
					if (!second_token->empty())
					{
						std::string key = first + "=" + *second_token;
...
bool FindFirstExistingPath(const std::string& paths, fs::path& out_path)
{
    stringtokenizer tokenizer(paths, ";");
    auto cursor = tokenizer.begin();

    while (auto token = cursor.next_token())
    {
        if (token && fs::exists(*token))
        {
            out_path = *token;
            return true;
        }
