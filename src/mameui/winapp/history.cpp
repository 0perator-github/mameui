...
static bool create_index_history(std::string_view datsdir, std::ifstream& fp, int filenum)
{
	...
	fs::path idx_path = fs::path(datsdir) / "history.idx";
...
static bool create_index(std::string_view datsdir, std::ifstream& fp, int filenum)
{
	...
	std::streampos  position = fp.tellg();
	std::string file_line;
	while (std::getline(fp, file_line))
	{
		if ((std::count(file_line.begin(), file_line.end(), '=') == 1) && (file_line[0] == '$')) // line must start with $ and contain one =
		{
			std::string first, second;
			// now start by removing all spaces
			file_line.erase(remove_if(file_line.begin(), file_line.end(), ::isspace), file_line.end());

			mui_stringtokenizer tokenizer(file_line, "=,");
			first = tokenizer.next_token();  // get first part of key
			if (!first.empty())
			{
				while (tokenizer.has_next())
				{
					second = tokenizer.next_token(); // get second part
					if (!second.empty())
					{
						// store into index
...
std::string load_swinfo(const game_driver* drv, std::string_view datsdir, std::string_view software, int filenum)
{
	...
	// datafile name
	fs::path filename = fs::path(datsdir) / m_swInfo[filenum].filename;
...
std::string load_gameinfo(const game_driver *drv, std::string_view datsdir, int filenum)
{
	...
	// datafile name
	fs::path filename = fs::path(datsdir) / m_gameInfo[filenum].filename;
...
std::string load_sourceinfo(const game_driver *drv, std::string_view datsdir, int filenum)
{
	...
	// datafile name
	fs::path filename = fs::path(datsdir) / m_sourceInfo[filenum].filename;
	std::ifstream fp (filename);
	if (!fp) // File could not be opened
		return {};
...
std::string GetGameHistory(int driver_index, std::string_view software)
{
	...
	if (validate_datfiles())
	{
		...
		std::string history_paths = dir_get_value(DIRPATH_HISTORY_PATH);
		mui_stringtokenizer tokenizer(history_paths, ";");

		while (tokenizer.has_next() && !path_exists)
		{
			history_path = tokenizer.next_token();
			path_exists = fs::exists(history_path);
		}
		...
		if (path_exists)
		{
			...
			{
				// if no software, then we need to load the gameinfo and sourceinfow
				for (size_t filenum = 0; filenum < MAX_HFILES; filenum++)
				{
					fullbuf.append(load_gameinfo(&drv, history_path.string(), filenum));
					fullbuf.append(load_sourceinfo(&drv, history_path.string(), filenum));
				}
			}
			else
			{
				// if software, then we need to load the swinfo and gameinfo
				// and sourceinfo

				for (size_t filenum = 0; filenum < MAX_HFILES; filenum++)
				{
					fullbuf.append(load_swinfo(&drv, history_path.string(), software, filenum));
					fullbuf.append(load_gameinfo(&drv, history_path.string(), filenum));
					fullbuf.append(load_sourceinfo(&drv, history_path.string(), filenum));
				}
			}
