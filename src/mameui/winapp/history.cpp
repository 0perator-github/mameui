...
// ===========================================================================
// history.cpp - history database engine; Collect all information on the
// selected driver, and return it as a string. Called by winui.cpp
// ===========================================================================
...
constexpr std::size_t MAX_HFILES = 8;
...
// --------------------------------------------------------
// create_index_history - create an index for history.xml
// Parameters:
//     datsdir - the directory where the dat files are located
//     fp - the file stream for history.xml
//     filenum - the index of the dat file in m_gameInfo
// --------------------------------------------------------
...
static bool create_index_history(std::string_view datsdir, std::ifstream& fp, int filenum)
{
	...
	{
		std::size_t start = file_line.find(quote);
		if (start != std::string::npos)
		{
			start++;
			std::size_t end = file_line.find(quote, start);
	...
	if (idx_file)
	{
		...
		if (std::getline(idx_file, idx_ver) && idx_ver == xml_ver)
		{
			...
			{
				std::size_t eq_pos = file_line.find('=');
	...
	while (fp.good())
	{
		...
		{
			std::size_t start = file_line.find(quote), end = std::string::npos;
			if (start != std::string::npos)
			{
				start++;
	...
	if (mymap[filenum].empty())
	{
		std::cerr << "No valid entries found in history.xml.\n";
		return false;
	}
	...
	if (!out.good())
	{
		std::cerr << "Error writing to history.idx.\n";
		return false;
	}
...
// --------------------------------------------------------
// fnv1a_hash - compute the FNV-1a hash of a file stream
// Parameters:
//     fp - the file stream to hash
// --------------------------------------------------------
...
// --------------------------------------------------------
// create_index - create an index for a dat file
// Parameters:
//     datsdir - the directory where the dat files are located
//     fp - the file stream for the dat file
//     filenum - the index of the dat file in m_gameInfo
// --------------------------------------------------------
...
// --------------------------------------------------------
// convert_xml - convert XML entities to actual characters
// Parameters:
...
// --------------------------------------------------------
// load_datafile_text - load text from a dat file based on a keycode
// Parameters:
//     fp - the file stream for the dat file
//     keycode - the key to search for in the dat file
//     filenum - the index of the dat file in m_gameInfo
//     tag - the tag to look for in the dat file
// --------------------------------------------------------
...
// --------------------------------------------------------
// load_swinfo - load software information from a dat file
// Parameters:
//     drv - pointer to the game driver
//     datsdir - the directory where the dat files are located
//     software - the software name to search for
//     filenum - the index of the dat file in m_swInfo
// --------------------------------------------------------
...
// --------------------------------------------------------
// load_gameinfo - load game information from a dat file
// Parameters:
//     drv - pointer to the game driver
//     datsdir - the directory where the dat files are located
//     filenum - the index of the dat file in m_gameInfo
...
// --------------------------------------------------------
// load_sourceinfo - load source information from a dat file
// Parameters:
//     drv - pointer to the game driver
//     datsdir - the directory where the dat files are located
//     filenum - the index of the dat file in m_sourceInfo
// --------------------------------------------------------
...
// --------------------------------------------------------
// load_driver_geninfo - load general information about the driver
// Parameters:
//     drv - pointer to the game driver
//     drvindex - the index of the driver in the driver list
// --------------------------------------------------------
...
std::string load_driver_geninfo(const game_driver* drv, int drvindex)
{
	...
	for (const device_execute_interface &exec : cpuiter)
	{
		...
		for (const device_execute_interface &scan : cpuiter)
	...
	for (const device_sound_interface &sound : sounditer)
	{
		...
		for (const device_sound_interface &scan : sounditer)
	...
	{
		for (const screen_device &screen : screeniter)
		{
			if (screen.is_vector())
			...
			else
			{
				...
				if (drv->flags & ORIENTATION_SWAP_XY)
					general_machine_info << visarea.width() << " x " << visarea.height() << " (V) " << screen.frame_period().as_hz() << " Hz\n";
				else
					general_machine_info << visarea.width() << " x " << visarea.height() << " (H) " << screen.frame_period().as_hz() << " Hz\n";
			}
		}
	}
	...
	std::size_t driver_index = driver_list::clone(*drv);
	if (driver_index < 0)
	...
	else
	{
		...
		for (const device_t &device : device_enumerator(config.root_device()))
		{
			for (const rom_entry *region = rom_first_region(device); region; region = rom_next_region(region))
			{
				for (const rom_entry *rom = rom_first_file(region); rom; rom = rom_next_file(rom))
				{
					...
					general_machine_info << std::right << std::setfill('0') << std::setw(9) << rom_file_size(rom) << " \t";
	...
	while (driver_index < driver_list::total())
	{
		std::string current_source = driver_list::driver(driver_index).type.source();
		size_t j = current_source.find_last_of("/");
		current_source.erase(0, j+1);
		if (!mui_strcmp(source_file, current_source) && !(DriverIsBios(driver_index)))
			general_machine_info << driver_list::driver(driver_index).type.fullname() << "\n";
...
// --------------------------------------------------------
// validate_datfiles - validate that the dat files are properly configured
// Parameters:
//     None
// --------------------------------------------------------
...
// --------------------------------------------------------
// FindFirstExistingPath - find the first existing path from a semicolon-separated list
// Parameters:
//     paths - a semicolon-separated list of paths
//     out_path - the first existing path found (output parameter)
// --------------------------------------------------------

static bool FindFirstExistingPath(const std::string &paths, std::filesystem::path &out_path)
{
...
// --------------------------------------------------------
// AppendHistoryFiles - append history files to the full buffer
// Parameters:
//     drv - reference to the game driver
//     history_str - the path to the history files
//     software - the software name to search for
//     fullbuf - the full buffer to append to (output parameter)
// --------------------------------------------------------

static void AppendHistoryFiles(const game_driver& drv, const std::string& history_str, std::string_view software, std::string& fullbuf)
{
...
// --------------------------------------------------------
// GetGameHistory - get the game history for a specific driver index and software
// Parameters:
//     driver_index - the index of the driver in the driver list
//     software - the software name to search for
// --------------------------------------------------------
