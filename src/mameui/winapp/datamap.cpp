...
enum class _control_type:uint32_t
{
	UNKNOWN,
	BUTTON,
	STATIC,
...
using control_type = _control_type;
...
using datamap_entry = datamap_entry_t;
...
void datamap_add(datamap *map, int dlgitem, datamap_entry_type type, const char *option_name)
{
	...
	{
		std::cout << "Datamap.cpp Line __LINE__ too many entries" << std::endl;
		return;
	}
...
static datamap_entry *find_entry(datamap *map, int dlgitem)
{
	...
	std::cout << "Datamap.cpp line __LINE__ couldn't find an entry" << std::endl;
	return NULL;
}
...
static bool is_control_displayonly(HWND hwnd)
{
	...
	if (!windows::is_window_enabled(hwnd))
		displayonly = true;
	return displayonly;
}
				...
				case datamap_entry_type::DM_STRING:
					{
						...
						emu_set_value(o, option_name, std::move(svalue));
						break;
					}
...
static void populate_control(datamap *map, HWND hwnd, windows_options *o, datamap_entry *entry, const char *option_name)
{
	...
	std::string c = emu_get_value(o, option_name);
