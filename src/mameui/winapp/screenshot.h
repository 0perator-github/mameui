...
typedef struct my_bitmap_info_t
{
	int bmWidth;
	int bmHeight;
	int bmColors;
...
extern bool LoadScreenShot(int nGame, const std::string& lpSoftwareName, int nType);
...
typedef struct
{
	...
	const char*     zipname;
	int             tabenum;
	const std::string_view    (*pfnGetTheseDirs)(void);
}
SSINFO;
