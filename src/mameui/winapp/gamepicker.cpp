...
// ============================================================================
// picker.cpp - Game picker control
// ============================================================================
...
using PickerInfo = struct picker_info
{
	...
	int nLastItem;
	int nColumnCount;
	std::vector<int> pnColumnsShown;
	std::vector<int> pnColumnsOrder;
	UINT_PTR nTimer;
...
// helpers
inline static BOOL ListView_GetItemRect_Modified(HWND hwnd, int i, RECT *prc, int code);
...
// window procs
static LRESULT CALLBACK CallParentWndProc(WNDPROC pfnParentWndProc, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
...
static void Picker_Free(PickerInfo *pPickerInfo)
{
	...
	if (!pPickerInfo->pnColumnsOrder.empty())
		pPickerInfo->pnColumnsOrder.clear();
	if (!pPickerInfo->pnColumnsShown.empty())
		pPickerInfo->pnColumnsShown.clear();
...
static void Picker_InternalResetColumnDisplay(HWND hWnd, bool bFirstTime)
{
	...
	std::vector<int> widths(pPickerInfo->nColumnCount);
	std::vector<int> order(pPickerInfo->nColumnCount);
	std::vector<int> shown(pPickerInfo->nColumnCount);

	pPickerInfo->pCallbacks->pfnGetColumnWidths(widths.data());
	pPickerInfo->pCallbacks->pfnGetColumnOrder(order.data());
	pPickerInfo->pCallbacks->pfnGetColumnShown(shown.data());
	...
	if (!bFirstTime)
	{
		...
		pPickerInfo->pCallbacks->pfnSetColumnWidths(widths.data());
...
bool SetupPicker(HWND hwndPicker, const PickerOptions *pOptions)
{
	...
	if (pPickerInfo->nColumnCount)
	{
		...
		pPickerInfo->pnColumnsOrder = std::vector<int>(pPickerInfo->nColumnCount);
		pPickerInfo->pnColumnsShown = std::vector<int>(pPickerInfo->nColumnCount);
		if (pPickerInfo->pnColumnsOrder.empty() || pPickerInfo->pnColumnsShown.empty())
			goto error;
		...
		if (pPickerInfo->pCallbacks->pfnGetColumnOrder)
			pPickerInfo->pCallbacks->pfnGetColumnOrder(pPickerInfo->pnColumnsOrder.data());
		if (pPickerInfo->pCallbacks->pfnGetColumnShown)
			pPickerInfo->pCallbacks->pfnGetColumnShown(pPickerInfo->pnColumnsShown.data());
	}
...
bool Picker_HandleNotify(LPNMHDR lpNmHdr)
{
	...
	case LVN_ITEMCHANGED:
			...
			&& !(pnmv->uNewState & LVIS_SELECTED))
		{
			...
			if (pPickerInfo->pCallbacks->pfnOnLeavingItem)
				pPickerInfo->pCallbacks->pfnOnLeavingItem(hWnd, pnmv->lParam);
		}
		if (!(pnmv->uOldState & LVIS_SELECTED)
			&& (pnmv->uNewState & LVIS_SELECTED))
		{
			if (pPickerInfo->pCallbacks->pfnOnEnteringItem)
				pPickerInfo->pCallbacks->pfnOnEnteringItem(hWnd, pnmv->lParam);
		}
		bResult = true;
		break;
	...
	case LVN_BEGINDRAG:
		if (pPickerInfo->pCallbacks->pfnOnDragNDropBegin)
			pPickerInfo->pCallbacks->pfnOnDragNDropBegin(pnmv);
		break;
	}
	return bResult;
}
...
int Picker_GetNumColumns(HWND hWnd)
{
	...
	// Use vector to automatically manage memory and avoid memory leaks
	std::vector<int> shown(pPickerInfo->nColumnCount);
...
inline static BOOL ListView_GetItemRect_Modified(HWND hwnd, int i, RECT* prc, int code)
{
	if (!prc) return FALSE;
	prc->left = code;
	return static_cast<BOOL>(windows::send_message(hwnd, LVM_GETITEMRECT, static_cast<WPARAM>(i), reinterpret_cast<LPARAM>(prc)));
}
...
{
	PickerInfo* pPickerInfo = GetPickerInfo(hwndPicker);
	if (!pPickerInfo) return false;

	std::vector<int> widths(pPickerInfo->nColumnCount);
	std::vector<int> order(pPickerInfo->nColumnCount);
	std::vector<int> tmpOrder(pPickerInfo->nColumnCount);

	// retrieve the initial values
	pPickerInfo->pCallbacks->pfnGetColumnWidths(widths.data());
	pPickerInfo->pCallbacks->pfnGetColumnOrder(order.data());

	// switch the list view to LVS_REPORT style so column widths are reported correctly
	...
	// Get actual column count from the UI header
	int nColumnMax = Picker_GetNumColumns(hwndPicker);

	// Prevent out-of-bounds if UI has more columns than allocated memory structures
	int safeColumnMax = std::min(nColumnMax, static_cast<int>(pPickerInfo->nColumnCount));

	// Get the Column Order and save it safely
	(void)list_view::get_column_order_array(hwndPicker, safeColumnMax, tmpOrder.data());

	for (int i = 0; i < safeColumnMax; i++)
	{
		int realColumn = Picker_GetRealColumnFromViewColumn(hwndPicker, i);

		// Guard against arbitrary indices returned by Picker_GetRealColumnFromViewColumn
		if (realColumn >= 0 && realColumn < static_cast<int>(pPickerInfo->nColumnCount))
		{
			widths[realColumn] = list_view::get_column_width(hwndPicker, i);
		}
	...
	pPickerInfo->pCallbacks->pfnSetColumnWidths(widths.data());
	pPickerInfo->pCallbacks->pfnSetColumnOrder(order.data());
