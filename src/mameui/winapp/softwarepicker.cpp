...
//  messui.cpp at static const LPCTSTR mess_column_names[] =
...
std::string SoftwarePicker_LookupBasename(HWND hwndPicker, int nIndex)
{
...
std::string SoftwarePicker_LookupFilename(HWND hwndPicker, int nIndex)
{
	software_picker_info *pPickerInfo;
	pPickerInfo = GetSoftwarePickerInfo(hwndPicker);
	if ((nIndex < 0) || !(pPickerInfo->file_index_length > nIndex))
...
//    unsigned char *pBuffer;
	...
	if (rc)
	{
		...
		if (nIndex > 0)
			(void)ListView_RedrawItems(hwndPicker, nIndex, nIndex);
	}
...
static bool SoftwarePicker_InternalAddFile(HWND hwndPicker, std::wstring_view filename, bool bForce, bool check)
{
	...
	std::string_view utf8_filename = std::unique_ptr<char[]>(mui_utf8_from_wcstring(&filename[0])).get();
	std::string file_ext = std::string(utf8_filename, utf8_filename.find('.'), utf8_filename.size());

	if (file_ext == ".zip")
		ziperr = util::archive_file::open_zip(utf8_filename, pZip);
	else
	if (file_ext == ".7z")
		ziperr = util::archive_file::open_7z(utf8_filename, pZip);
	...
	if (!ziperr)
	{
		...
		while(rc && (result >= 0))
		{
			rc = SoftwarePicker_AddZipEntFile(hwndPicker, utf8_filename, bForce, pZip, check);
			result = pZip->next_file();
		}
	pZip.reset();
	}
	else
		rc = SoftwarePicker_AddFileEntry(hwndPicker, utf8_filename, 0, 0, bForce, check);
...
bool SoftwarePicker_AddFile(HWND hwndPicker, std::wstring_view file_name, bool check)
{
	...
	return SoftwarePicker_InternalAddFile(hwndPicker, file_name, true, check);
}
...
bool SoftwarePicker_AddDirectory(HWND hwndPicker, std::wstring_view directory_name)
{
	...
	pSearchInfo->directory_name = directory_name;
...
LPCTSTR SoftwarePicker_GetItemString(HWND hwndPicker, int nRow, int nColumn, wchar_t *pszBuffer, UINT nBufferLength)
{
	software_picker_info *pPickerInfo;
	const file_info *pFileInfo;
	LPCTSTR item_string = NULL;
	std::unique_ptr<wchar_t[]> wcs_buf;
	...
	switch(nColumn)
	{
		case SW_COLUMN_IMAGES:
			wcs_buf = std::unique_ptr<wchar_t[]>(mui_wcstring_from_utf8(pFileInfo->base_name.c_str()));
			break;
	...
	if (!wcs_buf)
		return item_string;
	mui_wcsncpy(pszBuffer, wcs_buf.get(), nBufferLength);
	item_string = pszBuffer;
