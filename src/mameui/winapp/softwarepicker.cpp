...
static bool SoftwarePicker_InternalAddFile(HWND hwndPicker, std::wstring_view filename, bool bForce, bool check)
{
	...
	std::string utf8_filename = mui_utf8_from_utf16string(filename);
...
bool SoftwarePicker_AddDirectory(HWND hwndPicker, std::wstring_view directory_name)
{
	...
	pSearchInfo->directory_name = normalized_path;
...
LPCWSTR SoftwarePicker_GetItemString(HWND hwndPicker, int nRow, int nColumn, wchar_t *pszBuffer, UINT nBufferLength)
{
	software_picker_info *pPickerInfo;
	const file_info *pFileInfo;
	LPCWSTR item_string = nullptr;
	...
	switch(nColumn)
	{
		case SW_COLUMN_IMAGES:
			wcs_buf = std::unique_ptr<wchar_t[]>(mui_utf16_from_utf8cstring(pFileInfo->base_name.c_str()));
			break;
