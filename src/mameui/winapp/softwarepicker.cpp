...
static bool SoftwarePicker_AddFileEntry(HWND hwndPicker, std::string_view file_name, UINT nZipEntryNameLength, UINT32 nCrc, bool bForce, bool check)
{
	...
	{
		for (device_image_interface &dev : image_interface_enumerator(pPickerInfo->config->machine_config_ptr()->root_device()))
		{
	...
	const size_t index_size = static_cast<size_t>(pPickerInfo->file_index_length + 1);
