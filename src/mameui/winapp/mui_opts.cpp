...
/***************************************************************************
    External functions
 ***************************************************************************/
std::string GetGameName(std::size_t driver_index)
{
...
static BitBuffer GetShowFolderFlags()
{
	BitBuffer bits(MAX_FOLDERS);

	bits.set_all(true);

	std::string show_folder_option = settings.getter(MUIOPTION_HIDE_FOLDERS);
	if (!show_folder_option.empty())
	{
		...
		{
			for (size_t flag_index = 0; flag_index < MAX_FOLDERS; ++flag_index)
			{
				const FOLDERDATA &folder_data = g_folderData[flag_index];
				if (short_name == folder_data.short_name)
				{
					bits.reset(folder_data.m_nFolderId);
					break;
				}
			}
		}
...
bool GetShowFolder(std::size_t folder)
{
	BitBuffer show_folder_flags = GetShowFolderFlags();
	bool result = show_folder_flags.test(folder);
	return result;
}

void SetShowFolder(std::size_t folder, bool show)
{
	BitBuffer show_folder_flags = GetShowFolderFlags();
	...
	if (show)
		show_folder_flags.set(folder);
	else
		show_folder_flags.reset(folder);

	// we save the ones that are NOT displayed, so we can add new ones
	// and upgraders will see them
	for (size_t flag_index = 0; flag_index < MAX_FOLDERS; flag_index++)
	{
		if (show_folder_flags.test(flag_index) == true)
			continue;

		for (size_t folder_index = 0; folder_index < MAX_FOLDERS; folder_index++)
		{
			const FOLDERDATA &folder_data = g_folderData[folder_index];
			if (folder_data.m_nFolderId == flag_index)
			{
				if (do_append)
					str.append(",");
				else
				...
				str.append(folder_data.short_name);
				break;
			}
		}
	}
...
// ***************************************************************** MAME_g.INI settings **************************************************************************
int GetRomAuditResults(std::size_t driver_index)
{
	return game_opts.rom(driver_index);
}

void SetRomAuditResults(std::size_t driver_index, int audit_results)
{
	game_opts.rom(driver_index, audit_results);
}

int GetSampleAuditResults(std::size_t driver_index)
{
	return game_opts.sample(driver_index);
}

void SetSampleAuditResults(std::size_t driver_index, int audit_results)
{
	game_opts.sample(driver_index, audit_results);
}

static void IncrementPlayVariable(std::size_t driver_index, std::string play_variable, uint32_t increment)
{
...
void IncrementPlayCount(std::size_t driver_index)
{
	IncrementPlayVariable(driver_index, "count", 1);
}

uint32_t GetPlayCount(std::size_t driver_index)
{
	return game_opts.play_count(driver_index);
}

// int needed here so we can reset all games
static void ResetPlayVariable(std::size_t driver_index, std::string play_variable)
{
	if (driver_index < 0)
		// all games
...
// int needed here so we can reset all games
void ResetPlayCount(std::size_t driver_index)
{
	ResetPlayVariable(driver_index, "count");
}

// int needed here so we can reset all games
void ResetPlayTime(std::size_t driver_index)
{
	ResetPlayVariable(driver_index, "time");
}

uint32_t GetPlayTime(std::size_t driver_index)
{
	return game_opts.play_time(driver_index);
}

void IncrementPlayTime(std::size_t driver_index, uint32_t playtime)
{
	IncrementPlayVariable(driver_index, "time", playtime);
}

std::wstring GetTextPlayTime(std::size_t driver_index)
{
...
{
	const std::size_t numFolders = GetNumFolders();
...
uint64_t GetDriverCacheLower(std::size_t driver_index)
{
	return game_opts.cache_lower(driver_index);
}

uint32_t GetDriverCacheUpper(std::size_t driver_index)
{
	return game_opts.cache_upper(driver_index);
}

void SetDriverCache(std::size_t driver_index, uint32_t val)
{
...
bool DriverIsComputer(std::size_t driver_index)
{
...
bool DriverIsConsole(std::size_t driver_index)
{
	assert(driver_index < driver_list::total());
	uint64_t system_type = GetDriverCacheLower(driver_index) & lower_cache::MASK_SYSTEMTYPE;
...
bool DriverIsModified(std::size_t driver_index)
{
	assert(driver_index < driver_list::total());
	return is_flag_set(game_opts.cache_lower(driver_index), lower_cache::UNOFFICIAL); // UNOFFICIAL
}

bool DriverIsImperfect(std::size_t driver_index)
{
	assert(driver_index < driver_list::total());
...
constexpr bool LOG_SOFTWARE = true;
