...
static void TabFlagsDecodeString(std::string_view ss, int *data, int fallback_tab_index)
{
	...
	*data = ALL_TABS_ON;
	...
	for (auto disabled_tabname : tokenizer)
	{
...
bool DriverIsComputer(int driver_index)
{
	...
	return system_type == lower_cache_flags::SYSTEMTYPE_COMPUTER;
}
...
bool DriverIsConsole(int driver_index)
{
	assert(driver_index < driver_list::total());
	uint64_t system_type = GetDriverCacheLower(driver_index) & lower_cache_flags::MASK_SYSTEMTYPE;
	return system_type == lower_cache_flags::SYSTEMTYPE_CONSOLE;
}

bool DriverIsModified(int driver_index)
{
	assert(driver_index < driver_list::total());
	return extract_bit(game_opts.cache_lower(driver_index), 8); // UNOFFICIAL
}
