...
std::wstring  GetImageTabLongName(int tab_index)
{
	return std::wstring(image_tabnames[tab_index].long_name);
}

std::wstring  GetImageTabShortName(int tab_index)
{
	return std::wstring(image_tabnames[tab_index].short_name);
}
...
static COLORREF options_get_color(std::string option_name)
{
	...
	std::string option_value = settings.getter(option_name);
...
static LPBITS GetShowFolderFlags(LPBITS bits)
{
	...
	if (!show_folder_option.empty())
	{
		...
		mui_stringtokenizer tokenizer(show_folder_option, ",");
		while (tokenizer.has_next())
		{
			std::string_view short_name = tokenizer.next_token();
...
static void CusColorDecodeString(std::string_view ss, COLORREF *value)
{
	mui_stringtokenizer tokenizer(ss, ",");
	for (size_t i = 0; i < 16; i++)
	{
		std::istringstream decode_stream;
		if (tokenizer.has_next())
		{
			decode_stream.str(std::string(tokenizer.next_token()));
...
static void ColumnDecodeStringWithCount(std::string_view ss, int *value, const size_t count)
{
	...
	{
		if(tokenizer.has_next())
			value[i] = std::stoi(std::string(tokenizer.next_token()));
	}
}
...
static void FontDecodeString(std::string_view ss, LOGFONTW* f)
{
	...
	auto next_int = [&]() -> int {
		if (tokenizer.has_next())
			return std::stoi(std::string(tokenizer.next_token()));
		return 0;
		};

	auto next_long = [&]() -> long {
		if (tokenizer.has_next())
			return std::stol(std::string(tokenizer.next_token()));
		return 0;
		};

	f->lfHeight = next_long();
	f->lfWidth = next_long();
	f->lfEscapement = next_long();
	f->lfOrientation = next_long();
	f->lfWeight = next_long();
	f->lfItalic = next_int();
	f->lfUnderline = next_int();
	f->lfStrikeOut = next_int();
	f->lfCharSet = next_int();
	f->lfOutPrecision = next_int();
	f->lfClipPrecision = next_int();
	f->lfQuality = next_int();
	f->lfPitchAndFamily = next_int();

	if (tokenizer.has_next())
	{
		std::wstring face_name = mui_utf16_from_utf8string(tokenizer.next_token());
...
static std::string TabFlagsEncodeString(int data)
{
	...
	{
		std::string imagetab_shortname = mui_utf8_from_utf16string(GetImageTabShortName(i));
		if (((data & (1 << i)) != 0) || imagetab_shortname.empty())
			continue;
...
static void TabFlagsDecodeString(std::string_view ss, int *data)
{
	...
	*data = ALL_TABS_ON;
	...
	while (tokenizer.has_next())
	{
		std::string short_name = std::string(tokenizer.next_token());
		//token.resize(token.size() - 3);
		for (size_t i=0; i < MAX_TAB_TYPES; i++)
		{
			std::string imagetab_shortname = mui_utf8_from_utf16string(GetImageTabShortName(i));
			if (imagetab_shortname.empty() || (short_name == imagetab_shortname))
			{
				
				*data &= ~(1 << i); // disable this tab
				break;
			}
		}
	}
...
void mui_save_ini(void)
{
	...
	settings.save_file(mui_inipath);
}
...
void SaveGameListOptions(void)
{
	...
	game_opts.save_file(gameinfo_inipath);
}
