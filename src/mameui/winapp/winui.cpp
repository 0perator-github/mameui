...
static void ResizeTreeAndListViews(bool bResizeHidden)
{
	...
	for (size_t i = 0; g_splitterInfo[i].nSplitterWindow; i++)
	{
		...
		if (bResizeHidden || bVisible)
		{
			...
			//std::cout << "Sizes: nLastWidth " << nLastWidth << ", nFullWidth " << nFullWidth << ", nLastWidth + width " << (nLastWidth + width) << std::endl;
			...
			//std::cout << "ResizeTreeAndListViews: Window " << i << ", Left " << nLastWidth << ", Right " << width + nLastWidth << std::endl;
...
{
	// first time through can't do this stuff
	//std::cout << "Update Screenshot : A" << std::endl;
	if (hwndList == nullptr)
		return;
	...
	//std::cout << "Update Screenshot: B" << std::endl;
	if (bShowStatusBar)
		rect.bottom -= bottomMargin;
	if (bShowToolBar)
		rect.top += topMargin;

	//std::cout << "Update Screenshot: C" << std::endl;
	bool bShowScreenShot = is_flag_set(GetWindowPanes(), window_pane::SCREENSHOT_PANE);
	...
	//std:cout << "Update Screenshot: F" << std::endl;
//	ResizePickerControls(hMain);
	ResizeTreeAndListViews(FALSE);

	//std::cout << "Update Screenshot: G" << std::endl;
	FreeScreenShot();

	std::string t_software;
	//std::cout << "Update Screenshot: H" << std::endl;
	if (have_selection)
	{
//#ifdef MESS
...
//#endif
	...
	//std::cout << "Update Screenshot: I" << std::endl;
	t_software = g_szSelectedItem;
	UpdateHistory(t_software);

	// setup the picture area

	//std::cout << "Update Screenshot: J" << std::endl;
	if (bShowScreenShot)
	{
		DWORD dwStyle;
		DWORD dwStyleEx;
...
static LRESULT CALLBACK MameUIWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MEASUREITEM:
	{
		...
		//std::cout << "Default row height calculation gives " << lpmis->itemHeight << std::endl;
		...
		if (lpmis->itemHeight < 17)
			lpmis->itemHeight = 17;
		//std::cout << "We would do " << (tm.tmHeight + tm.tmExternalLeading + 1) << std::endl;
	...
	case WM_MAMEUI_FILECHANGED:
	{
		...
		if (pfnGetAuditResults && pfnSetAuditResults)
		{
			...
			{
				for (int nParentIndex = nGameIndex; nGameIndex == -1; nParentIndex = GetParentIndex(&driver_list::driver(nParentIndex)))
				{
					if (mui_stricmp(driver_list::driver(nParentIndex).name, svFilename) == 0)
					{
...
//#endif
	...
	default:
		...
		for (size_t i = 0; g_helpInfo[i].nMenuItem > 0; i++)
		{
			...
			{
				//std::wcout << std::hex << std::uppercase << g_helpInfo[i].bIsHtmlHelp << ": " << g_helpInfo[i].lpFile << std::endl;
...
{
	//std::cout << "entering " << driver_list::driver(item_index).name << std::endl;
...
//	if (success)
//	{
		//std::wcout << "Got filename " << filename << " nFileExtension " << ofn.nFileExtension << std::endl;
...
#else
	std::cout << "Using vswprintf" << std::endl;
...
//	utf8_filename = file_path.filename().string();
	...
	for (driver_index = 0; driver_index < driver_list::total(); driver_index++) // find game and play it
	{
		...
		{
			std::shared_ptr<play_options> playopts = std::make_shared<play_options>();
			playopts->playback = utf8_filename;
			playopts_apply = 0x57;
			MamePlayGameWithOptions(driver_index, playopts);
			return;
		}
...
static void MamePlayRecordWave()
{
	...
	std::string utf8_filename = std::string(driver_list::driver(driver_index).name) + ".wav";
	std::filesystem::path file_path = emu_opts.GetGlobalOpts().state_directory();
	file_path /= utf8_filename;
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, buffer.data(), FILETYPE_WAVE_FILES))
	{
		...
		_wsplitpath(buffer.data(), wcs_drive, wcs_dir, wcs_filename, wcs_ext);
