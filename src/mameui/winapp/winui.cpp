...
static DWORD RunMAME(int nGameIndex, std::shared_ptr<play_options> playopts)
{
	...
	if (playopts_apply == 0x57)
	{
		...
		load_options(o, SOFTWARETYPE_GAME, nGameIndex, 0);
		if (!playopts->record.empty())
		...
		save_options(o, SOFTWARETYPE_GAME, nGameIndex);
	}
	playopts_apply = 0;
...
{
	HICON hIcon = 0;
	mui_stringtokenizer tokenizer(dir_get_value(DIRPATH_ICONS_PATH), ";");
	while (tokenizer.has_next() && !hIcon)
	{

		std::filesystem::path icon_path(tokenizer.next_token());
		std::filesystem::path icon_filename(std::string(iconname) + ".ico");

		icon_path /= icon_filename;
		std::string icon_path_str = icon_path.string();
		hIcon = shell::extract_icon_utf8(hInst, icon_path_str.c_str(), 0);
		if (!hIcon)
		{
			util::archive_file::ptr compressed_archive;
			bool result = false;

			icon_path.replace_filename("icons");
			icon_path.replace_extension(".zip");
			icon_path_str = icon_path.string();
			result = !util::archive_file::open_zip(icon_path_str.c_str(), compressed_archive);
			if (!result) // no zip file? Then try to open a 7z file.
			{
				//compressed_archive.reset();
				icon_path.replace_extension(".7z");
				icon_path_str = icon_path.string();
				result = !util::archive_file::open_7z(icon_path_str.c_str(), compressed_archive);
			}

			if (result)
			{
				result = compressed_archive->search(icon_path_str.c_str(), false) >= 0;
				if (result)
				{
...
//#endif
	...
	if (s_pWatcher)
	{
		DirWatcher_Watch(s_pWatcher, 0, dir_get_value(DIRPATH_MEDIAPATH), true);  // roms
		DirWatcher_Watch(s_pWatcher, 1, dir_get_value(DIRPATH_SAMPLEPATH), true);  // samples
	}
...
static LRESULT CALLBACK MameUIWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
		int (*pfnGetAuditResults)(uint32_t driver_index) = nullptr;
		void (*pfnSetAuditResults)(uint32_t driver_index, int audit_results) = nullptr;
		...
		if (pfnGetAuditResults && pfnSetAuditResults)
		{
			...
			char* s = const_cast<char*>(mui_strchr(szFileName, '.'));
			if (s)
				*s = '\0';
			s = const_cast<char*>(mui_strchr(szFileName, '\\'));
			if (s)
				*s = '\0';
...
static void ProgressBarHide()
{
	...
	int  widths[4];
	SIZE size;
	HDC hDC = gdi::get_dc(hProgWnd);

	(void)windows::show_window(hProgWnd, SW_HIDE);

	(void)gdi::get_text_extent_point_32(hDC, L"MMX", 3, &size);
	widths[3] = size.cx;
	(void)gdi::get_text_extent_point_32(hDC, L"MMMM games", 10, &size);
	widths[2] = size.cx;
	//Just specify 24 instead of 30, gives us sufficient space to display the message, and saves some space
	(void)gdi::get_text_extent_point_32(hDC, L"Screen flip support is missing", 24, &size);
	widths[1] = size.cx;

	(void)gdi::release_dc(hProgWnd, hDC);
...
//#endif
		...
		case TVN_ENDLABELEDIT :
		{
			...
			std::string utf8_szText = mui_utf8_from_utf16cstring(ptvdi->item.pszText);
				...
				if (folder->m_dwFlags & F_INIEDIT)
				{
					...
					if (data)
						if (data->m_soft_type_opt < TOTAL_SOFTWARETYPE_OPTIONS)
							InitPropertyPage(hInst, hwnd, GetSelectedFolderIcon(), data->m_soft_type_opt, folder->m_nFolderId, -1);
				}
		}
		UpdateStatusBar();
		break;
...
//#endif
			...
			{
				if (bUpdateRoms)
					DirWatcher_Watch(s_pWatcher, 0, dir_get_value(DIRPATH_MEDIAPATH), true);
				if (bUpdateSamples)
					DirWatcher_Watch(s_pWatcher, 1, dir_get_value(DIRPATH_SAMPLEPATH), true);
			}
	...
	case ID_RC_CLEAN:
		{
			...
			{
				// INI
				mui_stringtokenizer tokenizer(dir_get_value(DIRPATH_INIPATH), ";");
				while (tokenizer.has_next())
				{
					std::filesystem::path ini_filepath = tokenizer.next_token();
					std::filesystem::path ini_filename = std::string(driver_list::driver(nGame).name) + ".ini";
					ini_filepath /= ini_filename;
					std::cout << "Deleting " << ini_filepath << std::endl;
					//remove(ini_filepath.c_str());
				}
				// CFG
				tokenizer.str(dir_get_value(DIRPATH_CFG_DIRECTORY));
				while (tokenizer.has_next())
				{
					std::filesystem::path cfg_filepath = tokenizer.next_token();
					std::filesystem::path cfg_filename = std::string(driver_list::driver(nGame).name) + ".cfg";
					cfg_filepath /= cfg_filename;
					std::cout << "Deleting " << cfg_filepath << std::endl;
					//remove(cfg_filepath.c_str());
				}
				// NVRAM
				tokenizer.str(dir_get_value(DIRPATH_NVRAM_DIRECTORY));
				while (tokenizer.has_next())
				{
					std::filesystem::path nvram_filepath = tokenizer.next_token();
					std::filesystem::path nvram_filename = driver_list::driver(nGame).name;
					nvram_filepath /= nvram_filename;
					std::cout << "Deleting rd /s /q " << nvram_filepath << std::endl;
					//remove(nvram_filepath.c_str());
				}
				// Save states
				tokenizer.str(dir_get_value(DIRPATH_STATE_DIRECTORY));
				while (tokenizer.has_next())
				{
					std::filesystem::path state_filepath = tokenizer.next_token();
					std::filesystem::path state_filename = driver_list::driver(nGame).name;
					state_filepath /= state_filename;
					std::cout << "Deleting rd /s /q " << state_filepath << std::endl;
					//remove(state_filepath.c_str());
				}
			}
			(void)input::set_focus(hwndList);
		}
	...
	case ID_NOTEPAD:
		{
			...
			if (nGame >= 0)
			{
				...
				std::string history_filepath = history_path.string();
				std::ofstream outfile (history_filepath, std::ios::out | std::ios::trunc);
				std::string file_contents = GetGameHistory(nGame);
				outfile.write(file_contents.c_str(), file_contents.size());
				outfile.close();
				ShellExecuteCommon(hMain, history_filepath.c_str());
			}
			(void)input::set_focus(hwndList);
		}
		break;
	...
	case ID_OPTIONS_BG:
		{
			...
			if (bResult)
			{
				...
				std::unique_ptr<const char[]> utf8_filepath(mui_utf8_from_utf16cstring(wcs_filepath));
...
{
	const wchar_t *wcs_item_string = nullptr;
	const char* utf8_s = nullptr;
	...
	switch(nColumn)
	{
		...
		{
			// Play time
			std::string play_time = GetTextPlayTime(nItem);
			utf8_s = play_time.c_str();
			break;
		}
		case COLUMN_TYPE:
		{
		...
		case COLUMN_PLAYED:
		{   // Times played
			std::string play_time = std::to_string(GetPlayCount(nItem));
			utf8_s = play_time.c_str();
			break;
		}
	...
	if(utf8_s && *utf8_s != '\0')
	{
		std::unique_ptr<wchar_t[]> wcsItemString(mui_utf16_from_utf8cstring(utf8_s));
		if( !wcsItemString)
...
//#endif
	...
	(void)menus::enable_menu_item(hMainMenu, ID_FILE_LOADSTATE, (driver_list::driver(nItem).flags & MACHINE_SUPPORTS_SAVE) ? MFS_ENABLED : MFS_GRAYED);
}
...
static void InitListView()
{
	...
	wcs_BgDir = std::unique_ptr<wchar_t[]>(mui_utf16_from_utf8cstring(&GetBgDir()[0]));
	if( !wcs_BgDir)
		return;
...
//  char file1[MAX_PATH];
//  char file2[MAX_PATH];
	const char *strCmpParam1 = nullptr, *strCmpParam2 = nullptr;
...
//      ofn.lpstrTitle  = _T("Select a HLSL shader file");
	...
	ofn.lpstrInitialDir   = mui_utf16_from_utf8cstring(dirname.c_str());
...
static void CLIB_DECL ATTR_PRINTF(1,2) MameMessageBox(const char *fmt, ...)
{
	...
	std::unique_ptr<char[]> utf8_mameuiname(mui_utf8_from_utf16cstring(&MAMEUINAME[0]));
...
static void MamePlayBackGame()
{
	...
	if (nGame != -1)
	{
		...
		std::unique_ptr<wchar_t[]> utf8_to_wcs(mui_utf16_from_utf8cstring(driver_name));
		(void)mui_wcscpy(wcs_filepath, utf8_to_wcs.get());
	}

	if (CommonFileDialog(dialog_boxes::get_open_filename, wcs_filepath, FILETYPE_INPUT_FILES))
	{
		...
		std::unique_ptr<char[]> utf8_filename(mui_utf8_from_utf16cstring(wcs_filename));
...
static void MameLoadState()
{
	...
	if (nGame != -1)
	{
		std::unique_ptr<wchar_t[]> wcs_driver_name(mui_utf16_from_utf8cstring(driver_list::driver(nGame).name));
		(void)mui_wcscpy(wcs_filepath, wcs_driver_name.get());
		(void)mui_wcscpy(selected_filename, wcs_driver_name.get());
	}
	if (CommonFileDialog(dialog_boxes::get_open_filename, wcs_filepath, FILETYPE_SAVESTATE_FILES))
	{
		wchar_t wcs_drive[_MAX_DRIVE];
		wchar_t wcs_dir[_MAX_DIR];
		wchar_t wcs_ext[_MAX_EXT];
		...
		//int rc = state_manager::check_file(nullptr, pSaveState, selected_filename, MameMessageBox);
		//if (rc)
...
static void MamePlayRecordGame()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, wcs_filepath, FILETYPE_INPUT_FILES))
	{
		wchar_t wcs_drive[_MAX_DRIVE];
		wchar_t wcs_dir[_MAX_DIR];
...
static void MamePlayRecordWave()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, wcs_filepath, FILETYPE_WAVE_FILES))
	{
		wchar_t wcs_drive[_MAX_DRIVE];
		wchar_t wcs_dir[_MAX_DIR];
		wchar_t wcs_filename[_MAX_FNAME];
...
static void MamePlayRecordMNG()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, wcs_filepath, FILETYPE_MNG_FILES))
	{
		wchar_t wcs_drive[_MAX_DRIVE];
		wchar_t wcs_dir[_MAX_DIR];
		wchar_t wcs_filename[_MAX_FNAME];
...
static void MamePlayRecordAVI()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, wcs_filepath, FILETYPE_AVI_FILES))
	{
		wchar_t wcs_drive[_MAX_DRIVE];
		wchar_t wcs_dir[_MAX_DIR];
		wchar_t wcs_filename[_MAX_FNAME];
...
void InitBodyContextMenu(HMENU hBodyContextMenu)
{
	...
	std::unique_ptr<wchar_t[]> utf8_to_wcs(mui_utf16_from_utf8cstring(GetDriverFilename(nCurrentGame)));
