...
// number of tooltips in the application; used to size the array of tooltip strings
constexpr std::size_t NUM_TOOLTIPS = 9;
...
namespace
{
	...
	const wchar_t szTbStrings[NUM_TOOLTIPS + 1][30] =
	{
	...
	{
		{ AnchorIndex::Id,   { ID_TOOLBAR_EDIT }, true,  Anchor::Right | Anchor::Top, nullptr },
...
int GetCompatIndex(const game_driver *driver)
{
	...
	if (compat_name == nullptr || compat_name[0] == '\0')
		return not_found;

	int compat_index = GetGameNameIndex(compat_name);
	if (compat_index == not_found)
		return not_found;
...
int GetParentIndex(const game_driver *driver)
{
	...
	int parent_index = GetGameNameIndex(parent_name);
	if (parent_index == not_found)
		return not_found;
...
int GetParentRomSetIndex(const game_driver *driver)
{
	...
	int nParentIndex = GetParentIndex(driver);
	if (nParentIndex == not_found)
		return not_found;
...
int GetBiosRomSetIndex(const game_driver *driver)
{
	constexpr int not_found = -1;
	if (driver == nullptr)
		return not_found;
...
int GetGameNameIndex(const char *name)
{
	...
	int game_index = driver_list::find(name);
	if (game_index < 0 || game_index >= static_cast<int>(driver_list::total()))
		return not_found;
...
static bool MameUICommand(HWND hwnd,int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case ID_VIEW_FOLDERS:
	{
		...
		bool bShowTree = is_flag_set(val, window_pane::TREEVIEW_PANE);
		SetWindowPanes(val);
		(void)menus::check_menu_item(hMainMenu, ID_VIEW_FOLDERS, (bShowTree) ? MF_CHECKED : MF_UNCHECKED);
		(void)tool_bar::check_button(s_hToolBar, ID_VIEW_FOLDERS, (bShowTree) ? MF_CHECKED : MF_UNCHECKED);
...
static std::wstring GamePicker_GetItemString(HWND hwndPicker, int item_index, int nColumn)
{
	...
	{
		// Vector/Raster
		return DriverUsesVectorDisplay(item_index) ? L"Vector" : L"Raster";
	}
...
static int GamePicker_Compare(HWND hwndPicker, int index1, int index2, int sort_subitem, bool ascending)
{
	...
	case COLUMN_TYPE: value = DriverUsesVectorDisplay(index1) - DriverUsesVectorDisplay(index2); break;
...
void ToggleShowFolder(int folder)
{
	...
	bool show = is_flag_set(GetWindowPanes(), window_pane::SCREENSHOT_PANE);
