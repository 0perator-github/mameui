...
#define LG_ICONMAP_WIDTH    windows::get_system_metrics(SM_CXICON)
#define LG_ICONMAP_HEIGHT   windows::get_system_metrics(SM_CYICON)
#define ICONMAP_WIDTH       windows::get_system_metrics(SM_CXSMICON)
#define ICONMAP_HEIGHT      windows::get_system_metrics(SM_CYSMICON)
...
#define WM_MAMEUI_FILECHANGED (WM_USER + 0)
#define WM_MAMEUI_AUDITGAME   (WM_USER + 1)
#define WM_MAMEUI_PLAYGAME (WM_APP + 15000)

static PDIRWATCHER s_pWatcher;

// use a joystick subsystem in the gui?
static const struct osd_joystick_t* g_pJoyGUI = NULL;
...
#define NUM_GUI_SEQUENCES (std::size(GUISequenceControl))
...
static DWORD RunMAME(int nGameIndex, const play_options *playopts)
{
	...
	std::unique_ptr<mame_machine_manager> manager(mame_machine_manager::instance(global_opts, osd));
...
//#endif
	...
	t_software = std::string(g_szSelectedItem);
...
{
	const size_t buffer_size = mui_strlen(str) + 3; // pad with extra space for the added white space and comma
	static char* buf;
	...
	if (!mui_strncmp(str, "The ",4))
	{
		bool paren_found = false;
		char *buf_iter = buf;
		const char* str_iter = str + 4;
...
//#endif
	...
	if (s_pWatcher)
	{
		DirWatcher_Watch(s_pWatcher, 0, dir_get_value(DIRMAP_MEDIAPATH), TRUE);  // roms
		DirWatcher_Watch(s_pWatcher, 1, dir_get_value(DIRMAP_SAMPLEPATH), TRUE);  // samples
	}
...
//#endif
	...
	const char *description = ModifyThe(driver_list::driver(nGame).type.fullname());
	description = ConvertAmpersandString(description);
	std::unique_ptr<wchar_t[]> wcs_description(mui_wcstring_from_utf8(description));
	if( !wcs_description )
		return;
...
static const char* GetCloneParentName(int nItem)
{
	...
	if (DriverIsClone(nItem) == TRUE)
	{
		...
		if (nParentIndex >= 0)
		{
			fullname = ModifyThe(driver_list::driver(nParentIndex).type.fullname());
		}
	}
...
//#endif
		...
		case TVN_ENDLABELEDIT :
		{
			TV_DISPINFO* ptvdi = (TV_DISPINFO*)nm;
			LPTREEFOLDER folder = (LPTREEFOLDER)ptvdi->item.lParam;
//			std::unique_ptr<const char[]> utf8_szText;
			BOOL result = 0;
...
//			utf8_szText = std::unique_ptr<const char[]>(mui_utf8_from_wcstring(ptvdi->item.pszText));
			char* utf8_szText = mui_utf8_from_wcstring(ptvdi->item.pszText);
			if (!utf8_szText)
				return FALSE;
...
//#endif
			...
			{
				if (bUpdateRoms)
					DirWatcher_Watch(s_pWatcher, 0, dir_get_value(DIRMAP_MEDIAPATH), TRUE);
				if (bUpdateSamples)
					DirWatcher_Watch(s_pWatcher, 1, dir_get_value(DIRMAP_SAMPLEPATH), TRUE);
			}
	...
	case ID_OPTIONS_BG:
		{
			...
			if (bResult)
			{
				std::filesystem::path file_path(szFile);
				if (!std::filesystem::exists(szFile))
					return FALSE;

				std::unique_ptr<const char[]> utf8_filepath(mui_utf8_from_wcstring(szFile));
...
static const wchar_t *GamePicker_GetItemString(HWND hwndPicker, int nItem, int nColumn, wchar_t *pszBuffer, UINT nBufferLength)
{
	...
	switch(nColumn)
	{
		case COLUMN_GAMES:
			// Driver description
			utf8_s = ModifyThe(driver.type.fullname());
			break;
...
//  char file2[MAX_PATH];
	...
	case COLUMN_GAMES:
	{
		strCmpParam1 = ModifyThe(game1.type.fullname());
		strCmpParam2 = ModifyThe(game2.type.fullname());
...
static void MamePlayBackGame()
{
	...
	*filename = 0;
	...
	if (nGame != -1)
	{
		...
		(void)mui_wcscpy(filename, utf8_to_wcs.get());
	}

	if (CommonFileDialog(GetOpenFileNameW, filename, FILETYPE_INPUT_FILES))
	{
		play_options playopts;
		wchar_t drive[_MAX_DRIVE];
		wchar_t dir[_MAX_DIR];
		wchar_t bare_fname[_MAX_FNAME];
		...
		wsprintf(path,L"%s%s",drive,dir);
		wsprintf(fname,L"%s%s",bare_fname,ext);
		const size_t path_size = mui_wcslen(path) - 1;
		if (path[path_size] == L'\\')
			path[path_size] = L'\0'; // take off trailing back slash

		std::unique_ptr<char[]> utf8_filename(mui_utf8_from_wcstring(fname));
		...
		if (fileerr)
		{
			MameMessageBoxW(L"Could not open '%s' as a valid input file.", filename);
			return;
		}
...
static void MameLoadState()
{
	...
	*filename = 0;
	...
	if (nGame != -1)
	{
		...
		(void)mui_wcscpy(filename, wcs_driver_name.get());
		(void)mui_wcscpy(selected_filename, wcs_driver_name.get());
	}
	if (CommonFileDialog(GetOpenFileNameW, filename, FILETYPE_SAVESTATE_FILES))
	{
		wchar_t drive[_MAX_DRIVE];
		wchar_t dir[_MAX_DIR];
		wchar_t ext[_MAX_EXT];
		...
		std::unique_ptr<char[]> utf8_filename(mui_utf8_from_wcstring(filename));
...
static void MamePlayRecordGame()
{
	...
	*filename = 0;
	...
	if (CommonFileDialog(GetSaveFileNameW, filename, FILETYPE_INPUT_FILES))
	{
		wchar_t drive[_MAX_DRIVE];
		wchar_t dir[_MAX_DIR];
...
static void MamePlayRecordWave()
{
	...
	if (CommonFileDialog(GetSaveFileNameW, filename, FILETYPE_WAVE_FILES))
	{
		play_options playopts;

		playopts = {};
		std::unique_ptr<char[]> utf8_file_name(mui_utf8_from_wcstring(filename));
		playopts.wavwrite = utf8_file_name.get();
		playopts_apply = 0x57;
		MamePlayGameWithOptions(nGame, &playopts);
	}
}
...
static void MamePlayRecordMNG()
{
	...
	if (CommonFileDialog(GetSaveFileNameW, filename, FILETYPE_MNG_FILES))
	{
		wchar_t drive[_MAX_DRIVE];
		wchar_t dir[_MAX_DIR];
		wchar_t fname[_MAX_FNAME];
...
static void MamePlayRecordAVI()
{
	...
	if (CommonFileDialog(GetSaveFileNameW, filename, FILETYPE_AVI_FILES))
	{
		wchar_t drive[_MAX_DRIVE];
		wchar_t dir[_MAX_DIR];
		wchar_t fname[_MAX_FNAME];
...
static void UpdateMenu(HMENU hMenu)
{
	...
	if (have_selection)
	{
		const char* description = ConvertAmpersandString(ModifyThe(driver_list::driver(nGame).type.fullname()));
		std::unique_ptr<wchar_t[]> wcs_description(mui_wcstring_from_utf8(description));
		if( !wcs_description)
			return;
