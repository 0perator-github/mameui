...
std::string vformat_utf8(const char *fmt, va_list args);
...
void SetStatusBarTextF(int part_index, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	std::string formatted_text = vformat_utf8(fmt, args);
	va_end(args);

	SetStatusBarText(part_index, formatted_text.c_str());
}
...
static void MameMessageBox(const char *fmt, ...)
{
	...
	std::string formatted_text = vformat_utf8(fmt, args);
	va_end(args);
	dialog_boxes::message_box_utf8(GetMainWindow(), formatted_text.c_str(), utf8_mameuiname.c_str(), MB_OK | MB_ICONERROR);
}
...
static void MameMessageBoxW(const wchar_t *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	std::wstring formatted_text = vformat(fmt, args);
	va_end(args);

	dialog_boxes::message_box(GetMainWindow(), formatted_text.c_str(), &MAMEUINAME[0], MB_OK | MB_ICONERROR);
}
