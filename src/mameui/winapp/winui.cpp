...
inline static BOOL StatusBar_GetItemRect(HWND hWnd, int iPart, LPRECT lpRect)
{
	if (!lpRect) return FALSE;
	return static_cast<BOOL>(windows::send_message(hWnd, SB_GETRECT, static_cast<WPARAM>(iPart), reinterpret_cast<LPARAM>(lpRect)));
}

#if 0 // unused for now
inline static ToolBar_CheckButton(HWND hWnd, int idButton, BOOL fCheck)
{
	return static_cast<BOOL>(windows::send_message(hWnd, TB_CHECKBUTTON, (WPARAM)idButton, (LPARAM)MAKELONG(fCheck, 0)));
}
#endif
...
// private messages for communicating between windows
#if 0 // DEPRECATED: Old directory watcher auto-audit (no longer used)
constexpr UINT WM_MAMEUI_FILECHANGED = WM_USER + 808;
constexpr UINT WM_MAMEUI_AUDITGAME = WM_USER + 657;
#endif
constexpr UINT WM_MAMEUI_PLAYGAME = WM_APP + 1542;
...
inline static constexpr Anchor operator|(Anchor a, Anchor b) noexcept
{
...
// Timeout for the joystick timer, in milliseconds.
constexpr std::chrono::milliseconds JOYGUI_TIMEOUT{100};
constexpr unsigned int JOYGUI_MS = static_cast<unsigned int>(JOYGUI_TIMEOUT.count());

// Timer IDs
constexpr unsigned int JOYGUI_TIMER_ID = 0x1001;
constexpr unsigned int SCREENSHOT_TIMER_ID = 0x1002;
//constexpr unsigned int GAMEWND_TIMER_ID = 0x1003;

// Max size of a sub-menu
constexpr int DBU_MIN_WIDTH = 292;
constexpr int DBU_MIN_HEIGHT = 190;

//constexpr int NO_FOLDER = -1;
//constexpr int STATESAVE_VERSION = 1;

//I could not find a predefined value for this event and docs just say it has 1 for the parameter
constexpr int TOOLBAR_EDIT_ACCELERATOR_PRESSED = 1;

constexpr std::size_t MAX_MENUS = 3;

// table copied from windows/inputs.c
// table entry indices
constexpr std::size_t MAME_KEY = 0;
constexpr std::size_t DI_KEY = 1;
constexpr std::size_t VIRTUAL_KEY = 2;
constexpr std::size_t ASCII_KEY = 3;

constexpr int MAX_SCREENSHOT_HEIGHT = 264; // Max height of screenshot area

// maximum number of menu items under the "Show Folders" context menu item
constexpr WORD ID_CONTEXT_SHOW_FOLDER_END = ID_CONTEXT_SHOW_FOLDER_START + MAX_FOLDERS;
...
{
	AnchorIndex type;       // Either RA_ID or RA_HWND, to indicate which member of u is used; or RA_END
	...
	Anchor      action;     // What to do when control is resized
...
namespace
{
	...
	namespace gamestatus_iconindex
	{
		enum type : int
		{
			ICON_NOROMS = 0,
	...
	bool keyboard_state[4096]; // defines the number of internal key_codes
	...
	const GUISequence GUISequenceControl[] =
	{
	...
	constexpr std::size_t NUM_GUI_SEQUENCES = std::size(GUISequenceControl);
	...
	constexpr std::size_t NUM_TOOLBUTTONS = std::size(tbb);

	const wchar_t *szTbStrings[] =
	{
		L"Toggle Folder Tree",
		L"Toggle Software Area",
		L"Toggle Picture Area",
		L"Large Icons",
		L"Small Icons",
		L"List",
		L"Details",
	...
	constexpr std::size_t NUM_TOOLTIPS = std::size(szTbStrings);

	const int CommandToString[] =
	{
		ID_VIEW_FOLDERS,
	...
	ResizeItem toolbar_resize_items[] =
	{
		{ AnchorIndex::Id,   { IDC_TOOLBAR_EDIT }, true,  Anchor::Right | Anchor::Top, nullptr },
		{ AnchorIndex::End,  { 0 },               false, Anchor{},                   nullptr }
	};
	...
	{
		{ AnchorIndex::Hwnd, { 0 },            false, Anchor::Left | Anchor::Right | Anchor::Top,     &toolbar_resize },
		{ AnchorIndex::Hwnd, { 0 },            false, Anchor::Left | Anchor::Right | Anchor::Bottom,  nullptr },
		{ AnchorIndex::Id,   { IDC_DIVIDER },  false, Anchor::Left | Anchor::Right | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_TREE },     true,  Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_LIST },     true,  Anchor::All,                                      nullptr },
		{ AnchorIndex::Id,   { IDC_SPLITTER }, false, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_SPLITTER2 },FALSE, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_SSFRAME },  false, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_SSPICTURE },FALSE, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_HISTORY },  true,  Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
		{ AnchorIndex::Id,   { IDC_SSTAB },    false, Anchor::Left | Anchor::Top,                      nullptr },
		//#ifdef MESS
				{ AnchorIndex::Id,   { IDC_SWLIST },    true, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
				{ AnchorIndex::Id,   { IDC_SOFTLIST },  true, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
				//#endif
					{ AnchorIndex::Id,   { IDC_SPLITTER3 },FALSE, Anchor::Left | Anchor::Bottom | Anchor::Top,     nullptr },
					{ AnchorIndex::End,  { 0 },            false, Anchor{},                                           nullptr }
	};
	...
	const int LG_ICONMAP_WIDTH = windows::get_system_metrics(SM_CXICON);
	const int LG_ICONMAP_HEIGHT = windows::get_system_metrics(SM_CYICON);
	const int ICONMAP_WIDTH = windows::get_system_metrics(SM_CXSMICON);
	const int ICONMAP_HEIGHT = windows::get_system_metrics(SM_CYSMICON);
	...
	HWND s_hToolBar = nullptr;

	HINSTANCE hInst = nullptr;

	HFONT hFont = nullptr;     // Font for list view

	bool m_lock = false;    // prevent MAME from being launched twice by accident, and crashing the entire app.
	...
	bool g_listview_dragging = false;
	...
	bool bShowTabCtrl = true;
	bool bShowToolBar = true;
	bool bShowTree = true;

	bool g_in_treeview_edit = false;

	//PDirWatcher s_pWatcher; // DEPRECATED: Old directory watcher auto-audit (no longer used)

	UINT lastColumnClick = 0;
	WNDPROC g_lpHistoryWndProc = nullptr;
	WNDPROC g_lpPictureFrameWndProc = nullptr;
	WNDPROC g_lpPictureWndProc = nullptr;

	POPUPSTRING popstr[MAX_MENUS + 1];
	...
	HBITMAP hMissing_bitmap = nullptr;

	// Icon variables
	HIMAGELIST hLarge = nullptr;
	HIMAGELIST hSmall = nullptr;
	HIMAGELIST hHeaderImages = nullptr;
	std::unique_ptr<int[]> item_icon_indices; // for custom per-game icons

	HIMAGELIST himl_drag;
	std::size_t game_dragged; // which game started the drag
	HTREEITEM prev_drag_drop_target = nullptr; // which tree view item we're currently highlighting
...
// ============================================================================
	...
	HPALETTE         hpBackground = nullptr;
	HBITMAP          hbBackground = nullptr;
	MYBITMAPINFO     bmDesc;
}
...
// initialization
static bool MameUI_init(HINSTANCE hInstance, int nCmdShow);
...
static LRESULT CALLBACK StartupProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
static void ToggleShowFolder(std::size_t folder);
static void RemoveCurrentGameCustomFolder(void);
static void RemoveGameCustomFolder(std::size_t driver_index);
...
// initialization & data
static void GamePicker_Init(void);
static int GamePicker_GetItemCount(HWND hwndPicker);
...
// drag & drop
static void GamePicker_OnDragNDropBegin(NM_LISTVIEW* pnmv);
static void GamePicker_OnDragNDropEnd(POINTS pts);
static void GamePicker_OnDragNDropMove(POINTS pts);

// hover & tooltips
static void GamePicker_OnEnteringItem(HWND hwndPicker, int item_index);
static void GamePicker_OnLeavingItem(HWND hwndPicker, int item_index);
...
static HICON FindDriverIconWithFallback(int item_index);
static void AddDriverIcon(int item_index, gamestatus_iconindex::type default_icon_index);
static std::optional<HICON> LoadIconFromArchive(const std::filesystem::path& parent_path, const std::string& icon_name);
...
// folder tree
static void UpdateFolderTree();
static void ToggleFolderTree();
...
static void SetStatusBarTextF(int part_index, const char* fmt, ...) ATTR_PRINTF(2, 3);
...
// miscellaneous
static void MameMessageBox(const char* fmt, ...) ATTR_PRINTF(1, 2);
...
public:
	virtual void output_callback(osd_output_channel channel, const util::format_argument_pack<char>& args) override
	{
				...
				state.thread.emplace(
					[]()
					{
						...
						while (true)
						{
							...
							dialog_boxes::message_box_utf8(nullptr, message.c_str(), emulator_info::get_appname(), MB_ICONERROR | MB_OK);
		...
		else if (channel == OSD_OUTPUT_CHANNEL_WARNING)
		{
			// if we are in fullscreen mode, go to windowed mode
			if ((video_config.windowed == 0) && !osd_common_t::window_list().empty())
				winwindow_toggle_full_screen();
				...
				state.thread.emplace(
					[]()
					{
						...
						while (true)
						{
							...
							// Parent window isn't set because MAME could destroy
							// the window out from under us on a fatal error.
							dialog_boxes::message_box_utf8(nullptr, message.c_str(), emulator_info::get_appname(), MB_ICONWARNING | MB_OK);
							outfile.write(message.c_str(), message.size());
							outfile.flush();
						}
					});
		...
		else if (channel == OSD_OUTPUT_CHANNEL_INFO || channel == OSD_OUTPUT_CHANNEL_LOG)
		{
			auto& state(get_state());
			std::lock_guard<std::mutex> guard(state.mutex);
			util::stream_format(state.buffer, args);
			std::ofstream outfile("winui.log", std::ios::out | std::ios::app);
			outfile.write(state.buffer.str().c_str(), state.buffer.str().size());
			outfile.flush();
			state.buffer.str(std::string());
		}
	}
...
// MameUIMain - extension of the MameUI entry point found in mui_main.cpp
int MameUIMain(HINSTANCE hInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	bool bResult = MameUI_init(hInstance, nCmdShow);
...
{
	int tmpOrder[COLUMN_COUNT] = {};
	int nColumnMax = Picker_GetNumColumns(hwndList);
	if (nColumnMax < 0 || nColumnMax > COLUMN_COUNT)
	{
		std::cerr << "GetRealColumnOrder: Invalid column count " << nColumnMax << "\n";
		return;
	}
...
std::optional<HICON> LoadIconFromArchive( const std::filesystem::path &parent_path, const std::string &icon_name)
{
	...
	for (const auto& archive_path : candidates)
	{
		...
		// Decompress into in-memory buffer. If decompression fails, skip this archive.
		if (!compressed_archive->decompress(buffer.data(), size))
			continue;

		// First try to create an icon directly from ICO bytes (fast, no decode).
		if (size > 0)
		{
			HICON hIcon = CreateIconFromResourceEx(buffer.data(), static_cast<DWORD>(size), TRUE, 0x00030000, 0, 0, LR_DEFAULTCOLOR);
			if (hIcon)
				return hIcon;

			// Fallback to existing helper if direct creation failed
			hIcon = FormatICOInMemoryToHICON(buffer.data(), size);
			if (hIcon)
				return hIcon;
		}
	}
...
static void ResizeTreeAndListViews(bool bResizeHidden)
{
	...
	int nFullWidth, nLastWidth = SPLITTER_WIDTH;
	...
	for (size_t i = 0; g_splitterInfo[i].nSplitterWindow; i++)
	{
		...
		if (!leftwindow_handle)
			break;

		LONG_PTR leftwindow_style = windows::get_window_long_ptr(leftwindow_handle, GWL_STYLE);

		bool bVisible = (leftwindow_style & WS_VISIBLE) != 0 ? true : false;
		if (bResizeHidden || bVisible)
		{
			const int leftwindow_width = nSplitterOffset[i] - nLastWidth;
			int x_pos = nLastWidth;
			const int y_pos = rect.top + 2;
			int width = leftwindow_width;
			const int height = rect.bottom - rect.top - 4;

			// special case for the rightmost pane when the screenshot is gone
			if (!bShowScreenShot && !bShowSoftware && g_splitterInfo[i].nLeftWindow == IDC_LIST)
			{
				width = rect.right - nLastWidth - SPLITTER_WIDTH;
				if (bVisible)
					nLastWidth += width + SPLITTER_WIDTH;
			}
			else if (!bShowScreenShot && !g_splitterInfo[i + 1].nSplitterWindow)
			{
				width = rect.right - nLastWidth - SPLITTER_WIDTH;
				if (bVisible)
					nLastWidth += width + SPLITTER_WIDTH;
			}
			else if ((nLastWidth + width) > nFullWidth)
			...
			HWND splitter_handle = dialog_boxes::get_dlg_item(hMain, g_splitterInfo[i].nSplitterWindow);
			if (splitter_handle)
				(void)windows::move_window(splitter_handle, x_pos, y_pos, width, height, true); // splitter
...
static void UpdateFolderTree()
{
	...
	if (!hMainMenu || !s_hToolBar || !hTreeView)
		return;

	bool bShowFolderTree = is_flag_set(GetWindowPanes(), window_pane::TREEVIEW_PANE);
	bool bShowScreenShot = is_flag_set(GetWindowPanes(), window_pane::SCREENSHOT_PANE);

	(void)menus::check_menu_item(hMainMenu, ID_VIEW_FOLDERS, (bShowFolderTree) ? MF_CHECKED : MF_UNCHECKED);
	(void)tool_bar::check_button(s_hToolBar, ID_VIEW_FOLDERS, (bShowFolderTree) ? MF_CHECKED : MF_UNCHECKED);

	(void)windows::show_window(hTreeView, (bShowFolderTree) ? SW_SHOW : SW_HIDE);
...
//#ifdef MESS
	(void)windows::show_window(dialog_boxes::get_dlg_item(hMain, IDC_SWTAB), bShowSoftware ? SW_SHOW : SW_HIDE);
	(void)windows::show_window(dialog_boxes::get_dlg_item(hMain, IDC_SWLIST), bShowSoftware ? SW_SHOW : SW_HIDE);
	(void)windows::show_window(dialog_boxes::get_dlg_item(hMain, IDC_SWDEVVIEW), bShowSoftware ? SW_SHOW : SW_HIDE);
	(void)windows::show_window(dialog_boxes::get_dlg_item(hMain, IDC_SOFTLIST), bShowSoftware ? SW_SHOW : SW_HIDE);
	(void)windows::show_window(dialog_boxes::get_dlg_item(hMain, IDC_SWTAB), bShowSoftware ? SW_SHOW : SW_HIDE);
//#endif
	ResizePickerControls(hMain);
...
//#endif
	...
	HWND hPictureAreaFrameCtrl = dialog_boxes::get_dlg_item(hMain, IDC_SSFRAME);
	HWND hPictureAreaBmpCtrl = dialog_boxes::get_dlg_item(hMain, IDC_SSPICTURE);
	HWND hPictureAreaTabCtrl = dialog_boxes::get_dlg_item(hMain, IDC_SSTAB);

	if (bShowScreenShot)
	{
		RECT rect{};
		(void)windows::get_client_rect(hMain, &rect); // Size the List Control in the Picker
		...
		RECT fRect{};
		(void)windows::get_window_rect(hPictureAreaFrameCtrl, &fRect); // get the frame rect in screen coordinates
		...
		int history_tab = GetHistoryTab();
		bool showing_history = (have_history && (TabView_GetCurrentTab(hTabCtrl) == history_tab || history_tab == TAB_ALL) && GetShowTab(TAB_HISTORY) == false);
		CalculateBestScreenShotRect(hPictureAreaFrameCtrl, &rect, showing_history);

		DWORD dwStyle = windows::get_window_long_ptr(hPictureAreaBmpCtrl, GWL_STYLE);
		DWORD dwStyleEx = windows::get_window_long_ptr(hPictureAreaBmpCtrl, GWL_EXSTYLE);

		windows::adjust_window_rect_ex(&rect, dwStyle, false, dwStyleEx);
		(void)windows::move_window(hPictureAreaBmpCtrl, fRect.left + rect.left, fRect.top + rect.top, rect.right - rect.left, rect.bottom - rect.top, true);

		(void)windows::show_window(hPictureAreaBmpCtrl, TabView_GetCurrentTab(hTabCtrl) != TAB_HISTORY ? SW_SHOW : SW_HIDE);
		(void)windows::show_window(hPictureAreaFrameCtrl,SW_SHOW);
		(void)windows::show_window(hPictureAreaTabCtrl,bShowTabCtrl ? SW_SHOW : SW_HIDE);

		gdi::invalidate_rect(hPictureAreaBmpCtrl,nullptr,FALSE);
	}
	else
	{
		(void)windows::show_window(hPictureAreaBmpCtrl,SW_HIDE);
		(void)windows::show_window(hPictureAreaFrameCtrl,SW_HIDE);
		(void)windows::show_window(hPictureAreaTabCtrl,SW_HIDE);
	}
...
std::size_t GetCompatIndex(const game_driver *driver)
{
	if (driver == nullptr)
		return INVALID_INDEX;

	const char *compat_name = driver->compatible_with;
	if (!compat_name || compat_name[0] == '\0')
		return INVALID_INDEX;

	std::size_t compat_index = GetGameNameIndex(compat_name);
	if (compat_index == INVALID_INDEX)
		return INVALID_INDEX;

	return compat_index;
}

std::size_t GetParentIndex(const game_driver *driver)
{
	if (driver == nullptr)
		return INVALID_INDEX;
	...
	std::size_t parent_index = GetGameNameIndex(parent_name);
...
std::size_t GetParentRomSetIndex(const game_driver *driver)
{
	if (driver == nullptr)
		return INVALID_INDEX;

	std::size_t nParentIndex = GetParentIndex(driver);
...
std::size_t GetBiosRomSetIndex(const game_driver *driver)
{
	if (driver == nullptr)
		return INVALID_INDEX;
	...
	while (nParentIndex != INVALID_INDEX)
	{
		const game_driver &parent_drv = driver_list::driver(nParentIndex);
		if ((parent_drv.flags & MACHINE_IS_BIOS_ROOT) != 0)
			return nParentIndex;

		nParentIndex = GetParentIndex(&parent_drv);
	}

	return INVALID_INDEX;
}

std::size_t GetGameNameIndex(const char* name)
{
...
std::size_t GetGameNameIndex(const std::string& name)
{
	assert(!name.empty());
	return driver_list::find(name.c_str());
}
...
static INT_PTR CALLBACK StartupProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			HMODULE hInstance = system_services::get_module_handle(nullptr);
			const wchar_t *image_name = menus::make_int_resource(IDB_SPLASH);
			HBITMAP hSplashBmp = (HBITMAP)menus::load_image(hInstance, image_name, IMAGE_BITMAP, 0, 0, LR_SHARED);
			HWND hSplashCtrl = dialog_boxes::get_dlg_item(hDlg, IDC_SPLASH);
			(void)windows::send_message(hSplashCtrl, STM_SETIMAGE, IMAGE_BITMAP, reinterpret_cast<LPARAM>(hSplashBmp));

			HBRUSH hBrush = gdi::get_sys_color_brush(COLOR_3DFACE);
			windows::set_window_long_ptr(hDlg, DWLP_USER, reinterpret_cast<LONG_PTR>(hBrush));
		...
		case WM_CTLCOLORDLG:
		{
			HBRUSH hBrush = reinterpret_cast<HBRUSH>(windows::get_window_long_ptr(hDlg, DWLP_USER));
...
static bool MameUI_init(HINSTANCE hInstance, int nCmdShow)
{
	...
	item_icon_indices = make_unique_clear<int[]>(driver_list::total());
...
//#endif
	...
	if (GetJoyGUI() == true)
	{
		...
		if (g_pJoyGUI->init() != 0)
			...
			windows::set_timer(hMain, JOYGUI_TIMER_ID, JOYGUI_MS, nullptr);
	}
	else
		g_pJoyGUI.reset();
	...
	{
		windows::set_timer(hMain, SCREENSHOT_TIMER_ID, GetCycleScreenshot()*1000, nullptr); //scale to Seconds
	}
...
static LRESULT CALLBACK MameUIWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
	{
		switch (wParam)
		{
		case JOYGUI_TIMER_ID:
			PollGUIJoystick();
			break;
		case SCREENSHOT_TIMER_ID:
	...
	case WM_MOUSEMOVE:
	{
		...
		if (g_listview_dragging)
			GamePicker_OnDragNDropMove(MAKEPOINTS(lParam));
		else
			// for splitters
	...
	{
		if (g_listview_dragging)
			GamePicker_OnDragNDropEnd(MAKEPOINTS(lParam));
		else
			// for splitters
	...
	case WM_MAMEUI_FILECHANGED:
	{
		...
		struct AuditResults
		{
			int (*get)(std::size_t) = nullptr;
			void (*set)(std::size_t, int) = nullptr;
		}
		...
		{
		case 0:
			audit_results = { GetRomAuditResults, SetRomAuditResults };
			break;
		case 1:
			audit_results = { GetSampleAuditResults, SetSampleAuditResults };
			break;
		}

		if (audit_results.get && audit_results.set)
		{
			...
			for (int nGameIndex = 0; nGameIndex < driver_list::total(); nGameIndex++)
			{
				...
				while (nParentIndex != -1)
				{
					...
					if (mui_stricmp(driver.name, svFilename) == 0)
					{
						...
						if (audit_results.get(nGameIndex) != UNKNOWN)
						{
							audit_results.set(nGameIndex, UNKNOWN);
...
{
	for (size_t i = 0; r->items[i].type != AnchorIndex::End; i++)
	{
...
static void ResizeWindow(HWND hParent, Resize *r)
{
	...
	const ResizeItem *ri;
	int cmkindex = 0;
	while (r->items[cmkindex].type != AnchorIndex::End)
	{
		ri = &r->items[cmkindex];
		if (ri->type == AnchorIndex::Id)
			hControl = dialog_boxes::get_dlg_item(hParent, ri->u.id);
		...
		if (!static_cast<uint8_t>(ri->action & Anchor::Left))
			rect.left += dx;

		if (!static_cast<uint8_t>(ri->action & Anchor::Top))
			rect.top += dy;

		if (static_cast<uint8_t>(ri->action & Anchor::Right))
			rect.right += dx;

		if (static_cast<uint8_t>(ri->action & Anchor::Bottom))
			rect.bottom += dy;
...
static HWND InitToolbar(HWND hParent)
{
	...
	windows::create_window_ex_utf8( 0L, "Edit", &SEARCH_PROMPT[0], WS_CHILD | WS_BORDER | WS_VISIBLE | ES_LEFT,
					iPosX, iPosY, 200, iHeight, hToolBar, (HMENU)IDC_TOOLBAR_EDIT, hInst, nullptr );
...
//#endif
	...
	int games_shown = GamePicker_GetItemCount(hwndList);
	...
	int nItemIndex = Picker_GetSelectedItem(hwndList);
	if (nItemIndex < 0 || nItemIndex > games_shown)
	{
		DisableSelection();
		return;
	}
	...
	std::vector<wchar_t> statusbar_text(static_cast<std::size_t>(sb_textlength) + 1);
	...
	statusbar_text.resize(static_cast<std::size_t>(sb_textlength) + 1);
...
//#endif
	...
	if (bShowProgressBar && bListReady == true)
...
static int GamePicker_GetItemCount(HWND hwndPicker)
{
	...
	int count = 0;
	std::size_t nItemIndex = FindGame(lpFolder, 0);
	while (nItemIndex != BitBuffer::npos)
	{
...
static void ResetListView()
{
	...
	gdi::set_window_redraw(hwndList,FALSE);
...
//     for an explanation why, see SetView
	...
	gdi::set_window_redraw(hwndList, TRUE);
...
static void UpdateCache()
{
	...
	gdi::set_window_redraw(hwndList, false);   // stop screen updating
	...
	gdi::set_window_redraw(hwndList, true);             // refresh screen
}
...
static bool MameUICommand(HWND hwnd,int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDC_TOOLBAR_EDIT:
	{
		std::unique_ptr<char[]> buf(windows::get_window_text_utf8(hwndCtl));
		const char* edit_text = buf ? buf.get() : "";

		switch (codeNotify)
		{
		case TOOLBAR_EDIT_ACCELERATOR_PRESSED:
		{
			input::set_focus(hwndCtl);
			break;
		}
		...
		case EN_CHANGE:
		{
			...
			if (!mui_stricmp(edit_text, g_SearchText.c_str()) || !mui_stricmp(edit_text, SEARCH_PROMPT.data()))
				break;

			g_SearchText = edit_text;
			ResetListView();
			break;
		}
		...
		case EN_SETFOCUS:
		{
			if (*edit_text != '\0' && !mui_stricmp(edit_text, SEARCH_PROMPT.data()))
			{
				g_SearchText.clear();
		...
		{
			if (*edit_text == '\0')
				windows::set_window_text_utf8(hwndCtl, SEARCH_PROMPT.data());
			break;
		}
		}
...
//#endif
	...
	case ID_OPTIONS_INTERFACE:
	{
		...
		windows::kill_timer(hMain, SCREENSHOT_TIMER_ID);
		if (GetCycleScreenshot() > 0)
			windows::set_timer(hMain, SCREENSHOT_TIMER_ID, GetCycleScreenshot() * 1000, nullptr); // Scale to seconds
	...
	default:
	{
		...
		{
			const wchar_t *shellExecVerb = L"open", *shellExecParam = L"";
			...
			if (g_helpInfo[i].nMenuItem == id)
			{
				...
				else if (g_helpInfo[i].bIsHtmlHelp)
...
static std::wstring GamePicker_GetItemString(HWND hwndPicker, int item_index, int nColumn)
{
	...
	{
		// Vector/Raster
		return DriverIsVector(item_index) ? L"Vector" : L"Raster";
	}
...
static void GamePicker_Init()
{
	...
	static const PickerCallbacks s_gameListCallbacks =
	{
		...
		GamePicker_OnLeavingItem,       // pfnOnLeavingItem
		GamePicker_OnEnteringItem,        // pfnOnEnteringItem
		GamePicker_OnDragNDropBegin,    // pfnOnDragNDropBegin
	...
	if( hbBackground )
		(void)list_view::set_bk_image(hwndList, &bki);
...
static HICON FindDriverIconWithFallback(int item_index)
{
	const game_driver* current_driver = &driver_list::driver(item_index);

	HICON hIcon = LoadIconFromFile((char*)current_driver->name); // try to load the icon using the current romset's name
	if (hIcon) return hIcon;

	if (current_driver->parent && current_driver->parent[0] != '\0') // try to load the icon using the parent romset's name
	{
		int nParentIndex = GetGameNameIndex(current_driver->parent);
		if (nParentIndex >= 0 && nParentIndex < static_cast<int>(driver_list::total()))
		{
	...
	int nBiosIndex = GetBiosRomSetIndex(current_driver); // try to load the icon using the BIOS romset's name
	if (nBiosIndex >= 0 && nBiosIndex < static_cast<int>(driver_list::total()))
	{
		hIcon = LoadIconFromFile((char*)driver_list::driver(nBiosIndex).name);
		if (hIcon) return hIcon;
	}
...
static void AddDriverIcon(int item_index, gamestatus_iconindex::type default_icon_index)
{
	if (item_icon_indices[item_index] >= gamestatus_iconindex::ICON_COUNT)
		return;

	if (item_icon_indices[item_index] == gamestatus_iconindex::ICON_ROMS ||
		item_icon_indices[item_index] == gamestatus_iconindex::ICON_CLONE)
		return;

	HICON hIcon = FindDriverIconWithFallback(item_index);

	if (hIcon)
	{
		image_list::add_icon(hLarge, hIcon);
		int new_icon_index = image_list::add_icon(hSmall, hIcon);

		if (new_icon_index != -1)
			item_icon_indices[item_index] = new_icon_index;

		(void)DestroyIcon(hIcon); // Prevent Windows GDI handle leak
	}

	if (item_icon_indices[item_index] == gamestatus_iconindex::ICON_NOROMS)
		item_icon_indices[item_index] = static_cast<int>(default_icon_index);
}
...
static void DestroyIcons(void)
{
	if (hSmall)
	{
		(void)image_list::destroy(hSmall);
		hSmall = nullptr;
	}

	if (item_icon_indices)
	{
		for (size_t i=0;i<driver_list::total();i++)
			item_icon_indices[i] = 0; // these are indices into hSmall
	}

	if (hLarge)
	{
		(void)image_list::destroy(hLarge);
		hLarge = nullptr;
	}

	if (hHeaderImages)
	{
		(void)image_list::destroy(hHeaderImages);
...
static void ReloadIcons(void)
{
	...
	image_list::remove_all(hSmall);
	image_list::remove_all(hLarge);
...
{
	const game_driver &game1 = driver_list::driver(index1);
	const game_driver &game2 = driver_list::driver(index2);
	...
	case COLUMN_TYPE: value = DriverIsVector(index1) - DriverIsVector(index2); break;
	case COLUMN_YEAR: value = mui_stricmp(game1.year, game2.year); break;
	}

	// Ensure that games are sorted by name if they are otherwise equal
	if (0 == value && COLUMN_GAMES != sort_subitem)
		value = mui_stricmp(game1.type.fullname(), game2.type.fullname());
...
static void SetStatusBarTextF(int part_index, const char *fmt, ...)
{
	va_list args;
	va_list args_copy;
...
static void MamePlayGameWithOptions(int nGame, const play_options &playopts)
{
	m_lock = true;
	if (g_pJoyGUI)
		windows::kill_timer(hMain, JOYGUI_TIMER_ID);
...
static void MameUI_LoadState()
{
	...
	for (driver_index = 0; driver_index < driver_list::total(); driver_index++)
	{
		...
		if (mui_strcmp(driver_fullname, filename_stem) == 0)
		{
			...
			playopts_apply = 0x57;
			MamePlayGameWithOptions(driver_index, playopts);
			return;
		}
	}
...
static void MamePlayRecordWave()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, filename_buffer.data(), FILETYPE_WAVE_FILES))
	{
		...
		play_options playopts{};
		playopts.wavwrite = file_path.string(); // reuse file_path
		playopts_apply = 0x57;
		MamePlayGameWithOptions(driver_index, playopts);
	}
}
...
static void MamePlayRecordMNG()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, filename_buffer.data(), FILETYPE_MNG_FILES))
	{
		...
		play_options playopts{};
		playopts.mngwrite = file_path.string(); // reuse file_path
		playopts_apply = 0x57;
		MamePlayGameWithOptions(driver_index, playopts);
	}
}
...
static void MamePlayRecordAVI()
{
	...
	if (CommonFileDialog(dialog_boxes::get_save_filename, filename_buffer.data(), FILETYPE_AVI_FILES))
	{
		...
		play_options playopts{};
		playopts.aviwrite = file_path.string(); // reuse file_path
		playopts_apply = 0x57;
		MamePlayGameWithOptions(driver_index, playopts);
	}
}

void ToggleShowFolder(std::size_t folder)
{
	uint32_t current_id = GetCurrentFolderID();
	bool show = GetShowFolder(folder);
	SetShowFolder(folder, !show);
...
// Toggle Folder Tree Pane ON/OFF
static void ToggleFolderTree(void)
{
	uint32_t val = GetWindowPanes() ^ window_pane::TREEVIEW_PANE;
	bool show = is_flag_set(val, window_pane::TREEVIEW_PANE);
	SetWindowPanes(val);
	UpdateFolderTree();
...
// Toggle ScreenShot Pane ON/OFF
static void ToggleScreenShot(void)
{
	uint32_t val = GetWindowPanes() ^ window_pane::SCREENSHOT_PANE;
	bool show = is_flag_set(val, window_pane::SCREENSHOT_PANE);
	SetWindowPanes(val);
	UpdateScreenShot();
...
static void ToggleSoftware(void)
{
	uint32_t val = GetWindowPanes() ^ window_pane::SOFTWARE_PANE;
	bool show = is_flag_set(val, window_pane::SOFTWARE_PANE);
	SetWindowPanes(val);
	UpdateSoftware();
...
static int GetIconForDriver(int item_index)
{
	...
	{
		if (item_icon_indices[item_index] == 0)
			AddDriverIcon(item_index,static_cast<gamestatus_iconindex::type>(iconRoms));
		iconRoms = item_icon_indices[item_index];
	}
...
{
	TREEFOLDER **folders;
	std::size_t num_folders = 0;

	GetFolders(&folders,&num_folders);

	for (std::size_t i = 0;i < num_folders;i++)
	{
...
static HICON GetSelectedFolderIcon(void)
{
	...
	int image_index = tvi.iImage;
	if (image_index == I_IMAGECALLBACK)
	{
		LPTREEFOLDER folder = (LPTREEFOLDER)tvi.lParam;
		if (!folder || folder->m_nIconId == ICON_NONE)
			return nullptr; // shouldn't happen, but just in case
		image_index = folder->m_nIconId;
	}

	HIMAGELIST himage = tree_view::get_image_list(hTreeView, 1); // 1 for small image list
	if(!himage)
		return nullptr;

	return image_list::get_icon(himage, image_index, ILD_TRANSPARENT);
}
