...
static void ResizeTreeAndListViews(bool bResizeHidden)
{
	AREA area;
	GetWindowArea(&area);
	bool bShowPicture = extract_bit(GetWindowPanes(), 3);
	bool bShowSoftware = extract_bit(GetWindowPanes(), 2);
	int nLastWidth = 0;
	...
	// Tree control
	(void)windows::show_window(dialog_boxes::get_dlg_item(hMain, IDC_TREE), extract_bit(GetWindowPanes(), 0) ? SW_SHOW : SW_HIDE);
...
void UpdateSoftware()
{
	...
	bool bShowSoftware = extract_bit(GetWindowPanes(), 2);
	bool bShowImage = extract_bit(GetWindowPanes(), 3);
	//int  nWidth;
...
void UpdateScreenShot()
{
	...
	bool bShowImage = extract_bit(GetWindowPanes(), 3); // ss
...
//          nSplitterOffset[i] = rect.right * g_splitterInfo[i].dPosition;
	...
	{
		doSSControls = extract_bit(GetWindowPanes(), 3);
	}
	...
	int nListWidth = 0;
	if (extract_bit(GetWindowPanes(), 2)) // sw
...
//#endif
	...
	bShowTree = extract_bit(GetWindowPanes(), 0);
...
static void UpdateHistory(std::string software)
{
	...
	if (have_history && extract_bit(GetWindowPanes(), 3)
...
static bool MameUICommand(HWND hwnd,int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case ID_VIEW_FOLDERS:
	{
		...
		bool bShowTree = extract_bit(val, 0);
		SetWindowPanes(val);
		(void)menus::check_menu_item(hMainMenu, ID_VIEW_FOLDERS, (bShowTree) ? MF_CHECKED : MF_UNCHECKED);
		(void)tool_bar::check_button(s_hToolBar, ID_VIEW_FOLDERS, (bShowTree) ? MF_CHECKED : MF_UNCHECKED);
		UpdateScreenShot();
...
static void SwitchFullScreenMode(void)
{
	...
	if (GetRunFullScreen())
	{
		...
		(void)menus::check_menu_item(hMainMenu, ID_VIEW_FOLDERS, extract_bit(GetWindowPanes(), 0) ? MF_CHECKED : MF_UNCHECKED);
...
std::string longdots(std::string_view incoming, uint16_t howmany)
{
	// If howmany is less than or equal to 0, it's invalid.
	if (howmany == 0) return std::string{};

	// Convert string_view to a std::string
	std::string outgoing(incoming.begin(), incoming.end());
	...
	if (outgoing.length() > howmany)
		outgoing = outgoing.substr(0, howmany) + "...";
...
void delete_in_dirs(const stringtokenizer &tokenizer, const std::string &name, const std::string &extension)
{
	for (const auto &token : tokenizer)
	{
		...
		std::cout << "Deleting " << path.string() << std::endl;
