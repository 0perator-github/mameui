...
static DWORD RunMAME(int nGameIndex, std::shared_ptr<play_options> playopts)
{
	...
	emu_opts.SetDirectories(global_opts);

	emu_opts.SetSystemName(nGameIndex);
...
{
	HICON hIcon = 0;
	mui_stringtokenizer tokenizer(emu_opts.dir_get_value(DIRPATH_ICONS_PATH), ";");
	...
	case ID_RC_CLEAN:
		{
			...
			{
				// INI
				mui_stringtokenizer tokenizer(emu_opts.dir_get_value(DIRPATH_INIPATH), ";");
				...
				// CFG
				tokenizer.str(emu_opts.dir_get_value(DIRPATH_CFG_DIRECTORY));
				while (tokenizer.has_next())
				{
				...
				// NVRAM
				tokenizer.str(emu_opts.dir_get_value(DIRPATH_NVRAM_DIRECTORY));
				while (tokenizer.has_next())
				{
				...
				// Save states
				tokenizer.str(emu_opts.dir_get_value(DIRPATH_STATE_DIRECTORY));
				while (tokenizer.has_next())
				{
...
{
	// printf("entering %s\n",driver_list::driver(item_index).name);fflush(stdout);
...
//#endif
	...
	(void)menus::enable_menu_item(hMainMenu, ID_FILE_LOADSTATE, (driver_list::driver(item_index).flags & MACHINE_SUPPORTS_SAVE) ? MFS_ENABLED : MFS_GRAYED);
}
...
static int GamePicker_Compare(HWND hwndPicker, int index1, int index2, int sort_subitem, bool ascending)
{
	...
	case COLUMN_ROMS: value = GetRomAuditResults(index1) - GetRomAuditResults(index2); break;
	case COLUMN_SAMPLES: value = GetSampleAuditResults(index1) - GetSampleAuditResults(index2); break;
