...
// ============================================================================
// directories.cpp - Directories dialog box
// ============================================================================
...
// --------------------------------------------------------
// DirectoriesDialogProc - Dialog procedure for the Directories dialog box
// Parameters:
...
inline static bool  Is_Multi_Path(int dir_index) noexcept { return g_directory_options[dir_index].is_multi_path; }
inline static bool Path_Modified(DirInfo *info_ptr, int dir_index) noexcept { return info_ptr[dir_index].multi_path.is_modified(); }
inline static void Add_Path(DirInfo *info_ptr, int dir_index, std::filesystem::path new_path) { info_ptr[dir_index].multi_path.add_path(new_path); }
inline static std::filesystem::path &Get_Path(DirInfo* info_ptr, int dir_index, int path_index) { return info_ptr[dir_index].multi_path[path_index]; }
inline static void Insert_Path(DirInfo *info_ptr, int dir_index, int path_index, std::filesystem::path new_path) { info_ptr[dir_index].multi_path.insert_path(path_index, new_path); }
inline static std::size_t Path_Count(DirInfo *info_ptr, int dir_index) noexcept { return info_ptr[dir_index].multi_path.size(); }
inline static void Remove_Path(DirInfo *info_ptr, int dir_index, int path_index) { info_ptr[dir_index].multi_path.remove_path(path_index); }

// --------------------------------------------------------
// dirs_combo_box_handle - returns the handle to the combo box in the
// Directories dialog box
// Parameters:
//     none
// --------------------------------------------------------

static HWND dirs_combo_box_handle()
{
	return (!g_directories_dialog_handle) ? nullptr : dialog_boxes::get_dlg_item(g_directories_dialog_handle, IDC_DIR_COMBO);
}

// --------------------------------------------------------
// dirs_dialog_box_handle - returns the handle to the Directories dialog box
// Parameters:
//     none
// --------------------------------------------------------

static HWND dirs_dialog_box_handle()
{
	return g_directories_dialog_handle;
}

// --------------------------------------------------------
// dirs_list_view_handle - returns the handle to the list view in the
// Directories dialog box
// Parameters:
//     none
// --------------------------------------------------------

static HWND dirs_list_view_handle()
{
	return (!g_directories_dialog_handle) ? nullptr : dialog_boxes::get_dlg_item(g_directories_dialog_handle, IDC_DIR_LIST);
}

// --------------------------------------------------------
// dirs_delete_button_handle - returns the handle to the delete button in the
// Directories dialog box
// Parameters:
//     none
// --------------------------------------------------------

static HWND dirs_delete_button_handle()
{
	return (!g_directories_dialog_handle) ? nullptr : dialog_boxes::get_dlg_item(g_directories_dialog_handle, IDC_DIR_DELETE);
}

// --------------------------------------------------------
// dirs_insert_button_handle - returns the handle to the insert button in the
// Directories dialog box
// Parameters:
//     none
// --------------------------------------------------------

static HWND dirs_insert_button_handle()
{
	return (!g_directories_dialog_handle) ? nullptr : dialog_boxes::get_dlg_item(g_directories_dialog_handle, IDC_DIR_INSERT);
}

// --------------------------------------------------------
// FixSlash - removes trailing slash from a string if it exists
// Parameters:
...
// --------------------------------------------------------
// UpdateDirectoryList - updates the list view with the current directory list
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnSelChange - handles the selection change in the combo box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
// --------------------------------------------------------
// directories_init - initializes the directories data structure
// Parameters:
//     none
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnInitDialog - handles the WM_INITDIALOG message for the
// Directories dialog box
// Parameters:
//     hDlg - handle to the dialog box
//     hwndFocus - handle to the control that will receive the default
//     keyboard focus
//     lParam - additional message-specific information
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnDestroy - handles the WM_DESTROY message for the Directories
// dialog box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnClose - handles the WM_CLOSE message for the Directories
// dialog box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
static int RetrieveDirList(int dirinfo_index, int nFlagResult, void (*SetTheseDirs)(std::string_view s))
{
	...
	if (Path_Modified(g_directories, dirinfo_index))
	{
		...
		std::size_t path_count = Path_Count(g_directories, dirinfo_index);
...
// --------------------------------------------------------
// Directories_OnOk - handles the OK button click for the Directories
// dialog box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
static void Directories_OnOk(HWND hDlg)
{
	...
	for (std::size_t i = 0; i < g_directory_option_count; i++)
	{
		if (Is_Multi_Path(i))
...
// --------------------------------------------------------
// Directories_OnCancel - handles the Cancel button click for the Directories
// dialog box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnInsert - handles the Insert button click for the Directories
// dialog box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnItemChanged - handles the LVN_ITEMCHANGED notification for
// the list view in the Directories
// Parameters:
//     hDlg - handle to the dialog box
//     pNMHDR - pointer to the NMHDR structure that contains the notification
//     information
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnBrowse - handles the Browse button click for the Directories
// dialog box
// Parameters:
//     hDlg - handle to the dialog box
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnDelete - handles the Delete button click for
...
// --------------------------------------------------------
// Directories_OnBeginLabelEdit - handles the LVN_BEGINLABELEDIT
// notification for the list view in the Directories
// Parameters:
//     hDlg - handle to the dialog box
//     pNMHDR - pointer to the NMHDR structure that contains the
...
// --------------------------------------------------------
// SetDirectoryEntry - sets the directory entry in the data structure
// and updates the list view
// Parameters:
//     item_index - index of the item to update
//     fixed_dir_entry - the fixed directory entry
// --------------------------------------------------------
...
// --------------------------------------------------------
// Directories_OnEndLabelEdit - handles the LVN_ENDLABELEDIT
// notification for the list view in the Directories
// Parameters:
//     hDlg - handle to the dialog box
//     pNMHDR - pointer to the NMHDR structure that contains
...
// --------------------------------------------------------
// Directories_OnCommand - handles the WM_COMMAND message for the
// Directories dialog box
// Parameters:
//     hDlg - handle to the dialog box
//     id - identifier of the control that sent the message
//     hwndCtl - handle to the control that sent the message
...
// --------------------------------------------------------
// Directories_OnNotify - handles the WM_NOTIFY message for
// the Directories dialog box
// Parameters:
//     hDlg - handle to the dialog box
//     id - identifier of the control that sent the message
...
// --------------------------------------------------------
// BrowseCallbackProc - callback function for the SHBrowseForFolder
// dialog
// Parameters:
//     hwnd - handle to the dialog box
//     uMsg - message identifier
//     lParam - additional message-specific information
//     lpData - additional message-specific information
// --------------------------------------------------------
...
// --------------------------------------------------------
// BrowseForDirectory - opens a dialog box to browse for a directory
// Parameters:
//     hwnd - handle to the parent window
//     initial_dirpath - initial directory path to display in
//         the dialog box
//     selected_dirpath - output parameter to receive the selected
//         directory path
// --------------------------------------------------------
