...
enum _datamap_entry_type :uint32_t
{
	DM_NONE = 0,
	DM_BOOL,
	DM_INT,
...
using datamap_entry_type = _datamap_entry_type;

enum _datamap_callback_type :uint32_t
{
	DCT_READ_CONTROL,
...
using datamap_callback_type = _datamap_callback_type;

struct datamap_t;
using  datamap = datamap_t;
// MSH - Callback can now return TRUE, signifying that changes have been made, but should NOT be broadcast.'
using datamap_callback = bool (*)(datamap *map, HWND dialog, HWND hwnd, windows_options *opts, const char *option_name);
using get_option_name_callback = void (*)(datamap *map, HWND dialog, HWND hwnd, char *buffer, size_t buffer_size);
