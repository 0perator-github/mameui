...
void InitPropertyPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS software_type, std::size_t folder_id, std::size_t driver_index);

constexpr auto PROPERTIES_PAGE = 0;
constexpr auto AUDIT_PAGE = 1;

void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS software_type, std::size_t folder_id, std::size_t driver_index, int start_page);
...
// bool (*pfnFilterProc)(const machine_config *drv, const game_driver *gamedrv);
	bool (*pfnFilterProc)(std::size_t driver_index) = nullptr;
	DWORD dwDlgID = 0UL;
