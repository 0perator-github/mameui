...
[[nodiscard]] inline std::string longdots(std::string_view incoming, std::size_t howmany)
{
	if (howmany == 0) return {};
...
// MameUI passes control to MAME when handling a command line requests instead of starting the UI. lpCmdLine is not used in this function,
// but is kept for compatibility with the standard WinMain signature and possible future use.
int MameUIMain(HINSTANCE hInstance, [[maybe_unused]] LPWSTR lpCmdLine, int nCmdShow);
