...
{
	static emu_options m_mame_opts;
	return m_mame_opts; }

ui_options &emu_options_manager::GetUIOpts()
{
	static ui_options m_ui_opts;
	return m_ui_opts;
}

windows_options &emu_options_manager::GetGlobalOpts()
{
	static windows_options m_def_opts;
	return m_def_opts;
}
...
std::wstring emu_options_manager::get_ini_dir()
{
	...
	{
		std::filesystem::path ini_path = tokenizer.next_token();
...
std::wstring emu_options_manager::get_exe_path()
{
	...
	(void)system_services::get_module_filename(nullptr, filename, MAX_PATH);
	if (filename[0] == L'\0' || filename[MAX_PATH - 1] != L'\0')
		return L""s;
...
std::string emu_options_manager::get_exe_path_utf8()
{
	...
	(void)system_services::get_module_filename_utf8(nullptr, filename, MAX_PATH);
	if (filename[0] == '\0' || filename[MAX_PATH - 1] != '\0')
		return ""s;
...
{
	const game_driver *driver = (driver_index >= 0) ? &driver_list::driver(driver_index) : nullptr;
	std::filesystem::path ini_directory = get_ini_dir_utf8();
	std::filesystem::path load_inifile_path;

	// Load the base emulator config file first
	LoadSettingsFile(options, (ini_directory / (std::string(emulator_info::get_configname()) + ".ini")).string());
	...
	case SOFTWARETYPE_SOURCE:
		if (driver)
		{
			load_inifile_path = ini_directory / "source" /
				(std::filesystem::path(driver->type.source()).stem().concat(".ini"));
		}
		break;

	case SOFTWARETYPE_ARCADE: load_inifile_path = ini_directory / "arcade.ini"; break;
	case SOFTWARETYPE_COMPUTER: load_inifile_path = ini_directory / "computer.ini"; break;
	case SOFTWARETYPE_CONSOLE: load_inifile_path = ini_directory / "console.ini"; break;
	case SOFTWARETYPE_HORIZONTAL: load_inifile_path = ini_directory / "horizontal.ini"; break;
	case SOFTWARETYPE_RASTER: load_inifile_path = ini_directory / "raster.ini"; break;
	case SOFTWARETYPE_VECTOR: load_inifile_path = ini_directory / "vector.ini"; break;
	case SOFTWARETYPE_VERTICAL: load_inifile_path = ini_directory / "vertical.ini"; break;

	default:
		if (driver)
		{
			load_inifile_path = ini_directory / (std::string(driver->name) + ".ini");
		}
		break;
	}

	if (!load_inifile_path.empty())
	{
		LoadSettingsFile(options, load_inifile_path.string());

		if (set_system_name && driver)
		{
			options.set_value(OPTION_SYSTEMNAME, driver->name, OPTION_PRIORITY_HIGH);
		}
	}
...
void emu_options_manager::save_options(windows_options &options, SOFTWARETYPE_OPTIONS software_type, int driver_index)
{
	...
	switch (software_type)
	{
	case SOFTWARETYPE_GLOBAL: save_inifile_path = ini_directory / (std::string(emulator_info::get_configname()) + ".ini"); break;
	case SOFTWARETYPE_ARCADE: save_inifile_path = ini_directory / "arcade.ini"; break;
	case SOFTWARETYPE_COMPUTER: save_inifile_path = ini_directory / "computer.ini";	break;
	case SOFTWARETYPE_CONSOLE: save_inifile_path = ini_directory / "console.ini"; break;
	case SOFTWARETYPE_HORIZONTAL: save_inifile_path = ini_directory / "horizontal.ini"; break;
	case SOFTWARETYPE_RASTER: save_inifile_path = ini_directory / "raster.ini"; break;
	case SOFTWARETYPE_VECTOR: save_inifile_path = ini_directory / "vector.ini"; break;
	case  SOFTWARETYPE_VERTICAL: save_inifile_path = ini_directory / "vertical.ini"; break;
	case SOFTWARETYPE_SOURCE:
		if (driver)
		{
...
void emu_options_manager::emu_opts_init(bool b)
{
	...
	load_options(GetGlobalOpts(), SOFTWARETYPE_GLOBAL, -1, false);   // parse MAME.INI
...
void emu_options_manager::SetSelectedSoftware(int driver_index, std::string opt_name, std::string software)
{
	...
	else
	{
		...
		load_options(options, SOFTWARETYPE_GAME, driver_index, 1);
		options.set_value(&opt_name[0], &software[0], OPTION_PRIORITY_HIGH);
...
bool emu_options_manager::DriverHasSoftware(int driver_index)
{
	...
	{
		windows_options o;
		load_options(o, SOFTWARETYPE_GAME, driver_index, 1);
...
void emu_options_manager::global_save_ini()
{
	...
	SaveSettingsFile(GetGlobalOpts(), filename.c_str());
}
...
void emu_options_manager::ResetGameDefaults()
{
	...
	save_options(GetGlobalOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_OPTIONS);
}
