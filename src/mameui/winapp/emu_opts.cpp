...
void emu_options_manager::SaveSettingsFile(windows_options &options, std::string filename)
{
	...
	if (!filerr)
	{
		...
		std::cerr << __FILE__ << ": " << __LINE__ << ": Failed to open file: " << filename << " in " << __FUNCTION__ << std::endl;
...
void emu_options_manager::load_options(windows_options &options, SOFTWARETYPE_OPTIONS software_type, int driver_index, bool set_system_name)
{
	...
	{
		std::cerr << __FILE__ << ": " << __LINE__ << ": Unknown software type " << software_type << " in " << __FUNCTION__ << std::endl;
		return;
	}

	if (driver_index < 0 || driver_index >= driver_list::total())
	{
		std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid driver index " << driver_index << " in " << __FUNCTION__ << std::endl;
		return;
	}
	...
	[[unlikely]]
	default:
		std::cerr << __FILE__ << ": " << __LINE__ << ": Unhandled software type " << software_type << " in " << __FUNCTION__ << std::endl;
		break;
	}
}
