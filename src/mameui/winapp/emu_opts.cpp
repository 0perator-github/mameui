...
// ===========================================================================
// emu_opts.cpp - Interface to MAME's options and ini files.
// ===========================================================================
...
{
	util::core_file::ptr ini_file;
	std::error_condition filerr = util::core_file::open(filename.c_str(), OPEN_FLAG_READ, ini_file);
	if (!filerr)
	{
		if (!ini_file)
			return;

		options.parse_ini_file(*ini_file, OPTION_PRIORITY_HIGH, true, true);
	}
}
...
{
	util::core_file::ptr ini_file;
	std::error_condition filerr = util::core_file::open(filename.c_str(), OPEN_FLAG_WRITE | OPEN_FLAG_CREATE | OPEN_FLAG_CREATE_PATHS, ini_file);
	if (!filerr)
	{
		if (!ini_file)
			return;

		util::owritestream ini_owstream(*ini_file);
		ini_owstream.imbue(std::locale::classic());
		options.output_ini(ini_owstream);
		ini_owstream << std::flush;
	}
	else
...
{
	if (dir_index == DIRPATH_UNSET || dir_index >= TOTAL_DIRPATH_OPTIONS)
		return;
...
std::string emu_options_manager::GetSnapName()
{
	...
	return option_value ? option_value : ""s;
}
...
bool emu_options_manager::AreOptionsEqual(windows_options &options1, windows_options &options2)
{
	for (const auto &cur_entry : options1.entries())
	{
		...
		std::string_view opt1_value = opt1_cstr ? opt1_cstr : ""sv;
		std::string_view opt2_value = opt2_cstr ? opt2_cstr : ""sv;
