...
typedef enum machinepicker_column_id_t
{
	COLUMN_GAMES = 0,
	COLUMN_SRCDRIVERS,
	COLUMN_DIRECTORY,
	...
	COLUMN_ROMS,
	COLUMN_MAX
}machinepicker_column_id;

#define LOCAL_OPTIONS   -10

typedef struct window_area_t
{
	int x, y, width, height;
} AREA;

typedef struct screen_parameter_t
{
	char *screen;
	char *aspect;
	char *resolution;
...
typedef enum softwarepicker_column_id_t
{
	SW_COLUMN_IMAGES,
	SW_COLUMN_MAX
}softwarepicker_column_id;

typedef enum softwarelist_column_id_t
{
	SL_COLUMN_IMAGES,
	SL_COLUMN_GOODNAME,
