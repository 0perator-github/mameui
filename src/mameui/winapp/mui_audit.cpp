...
static INT_PTR AuditProcInitDlgMsg(HWND hDlg);
static INT_PTR AuditProcCmdMsg(HWND hDlg, WPARAM wParam, LPARAM lParam);
static INT_PTR GameAuditProcInitDlgMsg(HWND hDlg);
static INT_PTR GameAuditProcTimerMsg(HWND hDlg);
...
constexpr int MAX_AUDITBOX_TEXT = 0x7FFFFFFE; // INT_MAX - 1

static volatile HWND hAudit;
static std::atomic<int> rom_index(0);
static std::atomic<int> roms_correct(0);
static std::atomic<int> roms_incorrect(0);
static std::atomic<int> sample_index(0);
static std::atomic<int> samples_correct(0);
static std::atomic<int> samples_incorrect(0);
static volatile bool bPaused = false;
static volatile bool bCancel = false;
static int m_choice = 0;
static HANDLE hAuditThread = nullptr;
...
static DWORD WINAPI AuditThreadProc(LPVOID hDlg)
{
	...
	while (!bCancel)
	{
		...
		{
			checking_string = "File Audit";
			HWND hPauseButton = dialog_boxes::get_dlg_item(static_cast<HWND>(hDlg), IDC_AUDIT_PAUSE);
			if (!hPauseButton)
				return 0;

			(void)windows::enable_window(hPauseButton, false);
			return 1;
		}

		windows::set_window_text_utf8(static_cast<HWND>(hDlg), checking_string.c_str());
...
static INT_PTR AuditProcInitDlgMsg(HWND hDlg)
{
	...
	(void)dialog_boxes::send_dlg_item_message(hDlg, IDC_ROMS_PROGRESS, PBM_SETRANGE, 0, MAKELPARAM(0, driver_total));
	(void)dialog_boxes::send_dlg_item_message(hDlg, IDC_SAMPLES_PROGRESS, PBM_SETRANGE, 0, MAKELPARAM(0, driver_total));

	HWND hEdit = dialog_boxes::get_dlg_item(hDlg, IDC_AUDIT_DETAILS);  //RS 20030613 Set Bkg of RichEdit Ctrl
	if (!hEdit)
		return 0;
	...
	bPaused = false;
	rom_index = 0;
	HANDLE hThread = CreateThread(nullptr, 0, AuditThreadProc, hDlg, 0, &dwThreadID);
	if (!hThread)
		return 0;

	hAuditThread = hThread;
...
static INT_PTR AuditProcCmdMsg(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	case IDCANCEL:
	{
		...
		if (hAuditThread)
		{
			bCancel = true;
			DWORD dwExitCode = 0;
			if (GetExitCodeThread(hAuditThread, &dwExitCode) && (dwExitCode == STILL_ACTIVE))
				(void)windows::post_message(hDlg, WM_COMMAND, wParam, lParam);
			else
			{
				system_services::close_handle(hAuditThread);
	...
	{
		const wchar_t* buttonText = bPaused ? L"Pause" : L"Continue";
		(void)dialog_boxes::send_dlg_item_message(hDlg, IDC_AUDIT_PAUSE, WM_SETTEXT, 0, (LPARAM)buttonText);
		bPaused = !bPaused;
...
static INT_PTR GameAuditProcInitDlgMsg(HWND hDlg)
{
	FlushFileCaches();
	hAudit = hDlg;
	std::string game_title = GameInfoTitle(SOFTWARETYPE_GAME, rom_index);
	windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hDlg, IDC_PROP_TITLE), game_title.c_str());
...
static INT_PTR GameAuditProcTimerMsg(HWND hDlg)
{
	...
	bool timer_killed = KillTimer(hDlg, static_cast<UINT_PTR>(0)) != 0;
	if(!timer_killed)
		return 0;

	int iStatus = MameUIVerifyRomSet(rom_index, 0);
	const char *lpStatus = DriverUsesRoms(rom_index) ? StatusString(iStatus) : "None required";
