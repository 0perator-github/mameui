...
//	result = vsnprintf(NULL, 0, format, ap_copy) + 1;
//	va_end(ap_copy);
...
void AuditDialog(HWND hParent, int choice)
{
	...
	//RS use Riched32.dll
	HMODULE hModule = LoadLibraryW(L"Riched32.dll");
	if( hModule )
	{
		(void)dialog_boxes::dialog_box(system_services::get_module_handle(NULL),menus::make_int_resource(IDD_AUDIT),hParent,AuditWindowProc, 0L);
		FreeLibrary( hModule );
		hModule = NULL;
	}
	else
...
const char  *GetAuditString(int audit_result)
{
	...
	default:
		if (audit_result == -1)
			std::cout << "GetAuditString: Audit value -1, try doing a full F5 audit" << std::endl;
		else
			std::cout << "GetAuditString: Unknown audit value " << audit_result << std::endl;
	}
	...
	while (!bCancel)
	{
		if (!bPaused)
		{
			...
			else
			{
				...
				(void)EnableWindow(dialog_boxes::get_dlg_item((HWND)hDlg, IDPAUSE), false);
				ExitThread(1);
			}
		}
	}
	...
	case IDCANCEL:
	{
		...
		if (hThread)
		{
			...
			if (GetExitCodeThread(hThread, &dwExitCode) && (dwExitCode == STILL_ACTIVE))
				(void)PostMessageW(hDlg, WM_COMMAND, wParam, lParam);
			else
			{
				CloseHandle(hThread);
				EndDialog(hDlg, 0);
...
static HRESULT GameAuditProcInitDlgMsg(HWND hDlg)
{
	FlushFileCaches();
	hAudit = hDlg;
	windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hDlg, IDC_PROP_TITLE), GameInfoTitle(OPTIONS_GAME, rom_index).c_str());
...
static void CLIB_DECL DetailsPrintf(int box, const char *fmt, ...)
{
	...
	if (box == 0)
	{
		hEdit = dialog_boxes::get_dlg_item(hAudit, IDC_AUDIT_DETAILS);
		if (!hEdit)
	...
	std::unique_ptr<wchar_t[]> utf8_to_wcs(mui_wcstring_from_utf8(ConvertToWindowsNewlines(buffer)));
	if( !utf8_to_wcs)
		return;

	int textLength = edit_control::get_text_length(hEdit);
	edit_control::set_sel(hEdit, textLength, textLength);
	edit_control::replace_sel(hEdit, utf8_to_wcs.get());
}
