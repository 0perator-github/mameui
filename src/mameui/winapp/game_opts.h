...
class winui_game_options
{
	...
	bool create_index(std::ifstream& fp)
	{
		...
		uint32_t total_check = convert_to_uint(tokenizer.next_token_cstr());
		...
		uint32_t version_check = convert_to_uint(tokenizer.next_token_cstr());
		...
		while (std::getline(fp, file_line))
		{
				...
				mui_stringtokenizer tokenizer(file_line, "\t");
				if (tokenizer.has_next())
					index = driver_list::find(tokenizer.next_token_cstr());
				else
					index = -1;
				...
				if (index > -1)
				{
					...
					(void)tokenizer.next(); // get next part (old game number, ignore)
					m_list[index].game_number = index; // get new game number

					if (tokenizer.has_next()) // get ROM
						m_list[index].rom = convert_to_int(tokenizer.next_token_cstr());

					if (tokenizer.has_next()) // get Sample
						m_list[index].sample = convert_to_int(tokenizer.next_token_cstr());

					if (tokenizer.has_next()) // get cache lower bits
						m_list[index].cache_lower = convert_to_uint(tokenizer.next_token_cstr());

					if (tokenizer.has_next()) // get cache upper bits
						m_list[index].cache_upper = convert_to_uint(tokenizer.next_token_cstr());

					if (tokenizer.has_next()) // get play count
						m_list[index].play_count = convert_to_uint(tokenizer.next_token_cstr());

					tokenizer.set_delimiters("\n");
					if (tokenizer.has_next()) // get play time (no more delimiters, so read to the end)
						m_list[index].play_time = convert_to_uint(tokenizer.next_token_cstr());
				}
		}
