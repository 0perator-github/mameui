...
public:
	...
	uint64_t build_cache_flags(const game_driver &drv, const ui::machine_static_info &info)
	{
		...
		set_bit<uint64_t>(flags, (drv.flags & ORIENTATION_SWAP_XY) != 0, lower_cache_flags::SWAP_XY);
		...
		{
		case machine_flags::SYSTEMTYPE_CONSOLE:  flags |= lower_cache_flags::SYSTEMTYPE_CONSOLE; break;
		case machine_flags::SYSTEMTYPE_COMPUTER: flags |= lower_cache_flags::SYSTEMTYPE_COMPUTER; break;
		case machine_flags::SYSTEMTYPE_OTHER:    flags |= lower_cache_flags::SYSTEMTYPE_OTHER; break;
		default: break; // Default is arcade: 00, so no bits set
		}

		// --- Machine Features ---
		set_bit<uint64_t>(flags, (machine & machine_flags::NO_COCKTAIL) != 0, lower_cache_flags::NO_COCKTAIL);
		set_bit<uint64_t>(flags, (machine & machine_flags::IS_BIOS_ROOT) != 0, lower_cache_flags::IS_BIOS_ROOT);
		set_bit<uint64_t>(flags, (machine & machine_flags::REQUIRES_ARTWORK) != 0, lower_cache_flags::REQUIRES_ARTWORK);
		set_bit<uint64_t>(flags, (machine & machine_flags::UNOFFICIAL) != 0, lower_cache_flags::UNOFFICIAL);
		set_bit<uint64_t>(flags, (machine & machine_flags::NO_SOUND_HW) != 0, lower_cache_flags::NO_SOUND_HW);
		set_bit<uint64_t>(flags, (machine & machine_flags::MECHANICAL) != 0, lower_cache_flags::MECHANICAL);
		set_bit<uint64_t>(flags, (machine & machine_flags::IS_INCOMPLETE) != 0, lower_cache_flags::IS_INCOMPLETE);

		// --- Emulated Features ---
		device_t::flags_type emulation = info.emulation_flags();
		set_bit<uint64_t>(flags, (emulation & device_t::flags::NOT_WORKING) != 0, lower_cache_flags::NOT_WORKING);
		set_bit<uint64_t>(flags, (emulation & device_t::flags::SAVE_UNSUPPORTED) == 0, lower_cache_flags::SAVE_SUPPORTED);

		// --- Imperfect Features ---
		device_t::feature_type imperfect = info.imperfect_features();
		set_bit<uint64_t>(flags, (imperfect & device_t::feature::SOUND) != 0, lower_cache_flags::IMPERFECT_SOUND);
		set_bit<uint64_t>(flags, (imperfect & device_t::feature::GRAPHICS) != 0, lower_cache_flags::IMPERFECT_GRAPHICS);
		set_bit<uint64_t>(flags, (imperfect & device_t::feature::PALETTE) != 0, lower_cache_flags::IMPERFECT_COLOR);
		set_bit<uint64_t>(flags, (imperfect & device_t::feature::PROTECTION) != 0, lower_cache_flags::UNEMULATED_PROTECTION);
		set_bit<uint64_t>(flags, (imperfect & device_t::feature::CONTROLS) != 0, lower_cache_flags::IMPERFECT_CONTROLS);

		// --- Unemulated Features ---
		device_t::feature_type unemulated = info.unemulated_features();
		set_bit<uint64_t>(flags, (unemulated & device_t::feature::SOUND) != 0, lower_cache_flags::NO_SOUND);
		set_bit<uint64_t>(flags, (unemulated & device_t::feature::GRAPHICS) != 0, lower_cache_flags::NO_GRAPHICS);
		set_bit<uint64_t>(flags, (unemulated & device_t::feature::PALETTE) != 0, lower_cache_flags::WRONG_COLORS);
