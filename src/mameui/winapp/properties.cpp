...
//  OptionsCopy(dummy,GetCurrentOpts());
	...
	emu_opts.load_options(GetDefaultOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_DEFAULT_OPTIONS, false);
	...
	PROPSHEETPAGEW *pspages = CreatePropSheetPages(hInst, true, -1, &pshead.nPages, false);
	if (!pspages)
		return;
...
void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS opt_type, int folder_id, int game_num, int start_page )
{
	...
	emu_opts.load_options(GetCurrentOpts(), opt_type, game_num, true);
	emu_opts.load_options(GetOrigOpts(), opt_type, game_num, true);
	if (game_num == GLOBAL_OPTIONS)
		emu_opts.load_options(GetDefaultOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_DEFAULT_OPTIONS, false); // base opts is the backup for global
	else
		emu_opts.load_options(GetDefaultOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_OPTIONS, false); // global is the backup for games
	...
	case SOFTWARETYPE_GAME:
	{
		...
		pspages = CreatePropSheetPages(hInst, false, game_num, &pshead.nPages, true);
		description = mui_utf16_from_utf8string(driver_list::driver(g_nGame).type.fullname());
	}
	break;
	case SOFTWARETYPE_SOURCE:
	{
		pspages = CreatePropSheetPages(hInst, false, -1, &pshead.nPages, false);
	...
	// If we have no property pages, return.
	if (!pspages)
	{
		delete[] pspages;
		return;
	...
	// If we have no pages, or no description, return.
	if(pshead.nPages == 0 || description.empty())
		return;
...
static std::string GameInfoCPU(int nIndex)
{
	...
	for (device_execute_interface& exec : cpuiter)
	{
		...
		cpu_info << (clock / clock_factor) << "." << std::setprecision(6) << std::fixed << (clock % clock_factor) << frequency_unit << std::endl;
	}
...
static std::string GameInfoSound(int nIndex)
{
	...
	for (device_sound_interface &sound : sounditer)
	{
		...
		if (clock)
		{
			...
			sound_info << (clock / clock_factor) << "." << std::setprecision(6) << std::fixed << (clock % clock_factor) << frequency_unit << std::endl;
		}
	}
	...
	else
	{
		...
		else
		{
			for (screen_device &screen : screen_device_enumerator(config.root_device()))
			{
				...
				screen_info << visarea_width << " x " << visarea_height << screen_orientation << screen_hertz << " Hz" << std::endl;
			}
		}
	}
...
static std::string GameInfoCloneOf(int driver_index)
{
	...
	if (DriverIsClone(driver_index))
	{
		...
		const char *fullname = driver_list::driver(nParentIndex).type.fullname();
		const char *driver_name = driver_list::driver(nParentIndex).name;

		clone_info << ConvertAmpersandString(fullname) << " - \"" << (driver_name ? driver_name : ""s) << "\"" << std::endl;
	}
...
{
	const char* view_optionname = "view";
	if (m_currScreen >= 0)
	{
		const std::string view_option = view_optionname + std::to_string(m_currScreen);
		mui_strcpy(buffer, view_option.c_str());
	}
	else
		mui_strcpy(buffer, view_optionname);
}
...
{
	const char* resolution_optionname = "resolution";
	if (m_currScreen >= 0)
	{
		std::string resolution_option = resolution_optionname + std::to_string(m_currScreen);
		mui_strcpy(buffer, resolution_option.c_str());
	}
	else
		mui_strcpy(buffer, resolution_optionname);
}
		...
		for (const osd::directory::entry *entry = directory->read(); entry; entry = directory->read())
		{
			if (entry->type == osd::directory::entry::entry_type::DIR)
			{
				...
				if (!(name == "." || name == ".."))
				{
					...
					if (name == "English")
						english = count;
//					std::cout << "Language: " << name << ",count: " << count << std::endl;
					count++;
					delete[] wcsName;
				}
			}
...
//		std::cout << "Language: " << c << ", match: " << match << ", english: " << english << std::endl;
		if (match >= 0)
...
static bool ChangeJoystickMap(HWND hWnd)
{
	...
	if (mui_strcmp(joymap.get(), GetCurrentOpts().value(OPTION_JOYSTICK_MAP)))
	{
