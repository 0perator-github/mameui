...
windows_options m_OrigOpts;
windows_options m_DefaultOpts;
...
//  OptionsCopy(dummy,m_CurrentOpts);
	...
	load_options(m_CurrentOpts, SOFTWARETYPE_GLOBAL, GLOBAL_OPTIONS, false);
	load_options(m_OrigOpts, SOFTWARETYPE_GLOBAL, GLOBAL_OPTIONS, false);
	load_options(m_DefaultOpts, SOFTWARETYPE_GLOBAL, GLOBAL_DEFAULT_OPTIONS, false);
...
void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS opt_type, int folder_id, int game_num, int start_page )
{
	...
	if (game_num == GLOBAL_OPTIONS)
		load_options(m_DefaultOpts, SOFTWARETYPE_GLOBAL, -2, 0); // base opts is the backup for global
	else
		load_options(m_DefaultOpts, SOFTWARETYPE_GLOBAL, -1, 0); // global is the backup for games
	...
	case SOFTWARETYPE_GAME:
		wcs_description = mui_utf16_from_utf8cstring(driver_list::driver(g_nGame).type.fullname());
		break;
	case SOFTWARETYPE_SOURCE:
		wcs_description = mui_utf16_from_utf8cstring(GetDriverFilename(g_nGame));
		break;
...
std::string GameInfoTitle(SOFTWARETYPE_OPTIONS software_type, UINT nIndex)
{
	...
	case SOFTWARETYPE_VERTICAL:
		...
		break;
	case SOFTWARETYPE_GAME:
		title_info = util::string_format("%s\n\"%s\"", driver_list::driver(nIndex).type.fullname(), driver_list::driver(nIndex).name);
	default:
		break;
	}
	return title_info;
...
HWND hWnd;
	...
	case WM_INITDIALOG:
		...
		windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hDlg, IDC_PROP_SOURCE), GameInfoSource(g_nGame));
...
static void PropToOptions(HWND hWnd, windows_options &options)
{
	...
	if (hCtrl && hCtrl2 && hCtrl3)
	{
		...
		{
			emu_set_value(options, aspect_option, "auto");
		}
		else
		{
			int n = 0, d = 0;
			...
			emu_set_value(options, aspect_option, aspect_ratio_value);
		}
	}
	/* snapshot size */
	...
	if (hCtrl && hCtrl2 && hCtrl3)
	{
		if (button_control::get_check(hCtrl3))
		{
			emu_set_value(options, OPTION_SNAPSIZE, "auto");
		}
		else
		{
			...
			emu_set_value(options, OPTION_SNAPSIZE, snapshot_size_value);
		}
	}
}
...
static void UpdateMameShader(HWND hWnd, int slot, windows_options &options)
{
	...
	if (hCtrl)
	{
		...
		const char* value = options.value(option);
...
static void OptionsToProp(HWND hWnd, windows_options& options)
{
	...
	if (hCtrl)
	{
		...
		if (m_currScreen >= 0)
			aspect_option += std::to_string(m_currScreen);
		std::string aspect = emu_get_value(options, aspect_option);
		if( aspect == "auto")
		{
	...
	if (hCtrl)
	{
		c = emu_get_value(options, OPTION_EFFECT);
		if (c.empty())
		{
			c = "none";
			emu_set_value(options, OPTION_EFFECT, c);
		}
	...
	if (hCtrl)
	{
		if( emu_get_value(options, OPTION_SNAPSIZE) == "auto")
		{
			button_control::set_check(hCtrl, true);
	...
	if (hCtrl)
	{
		c = emu_get_value(options, OPTION_JOYSTICK_MAP);
	...
	if (hCtrl)
	{
		c = emu_get_value(options, OPTION_AUTOBOOT_SCRIPT);
	...
	if (hCtrl)
	{
		c = emu_get_value(options, OPTION_PLUGIN);
	...
	if (hCtrl)
	{
		c = emu_get_value(options, OSDOPTION_BGFX_SCREEN_CHAINS);
	...
	if (hCtrl && hCtrl2)
	{
		...
		{
			if (sscanf(options.value(OPTION_SNAPSIZE), "%dx%d", &width, &height) == 2 && width && height)
			{
...
static bool RotateReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	if (selected_index != original_selection)
	{
		...
		emu_set_value(options, OPTION_ROR, selected_index == 1);
		emu_set_value(options, OPTION_ROL, selected_index == 2);
		emu_set_value(options, OPTION_ROTATE, selected_index != 3);
		emu_set_value(options, OPTION_AUTOROR, selected_index == 4);
		emu_set_value(options, OPTION_AUTOROL, selected_index == 5);
		return true;
	}
...
static bool ScreenReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	emu_set_value(options, screen_option_name, op_val.get());
	return false;
}
...
static bool ScreenPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	for(i=0; gdi::enum_display_devices(nullptr, i, &dd, 0); i++)
	{
		if( !(dd.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER) )
		{
			...
			std::string screen = emu_get_value(options, screen_option);
...
static bool ViewPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	std::string view = emu_get_value(options, view_option);
...
static bool DefaultInputReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	emu_set_value(options, OPTION_CTRLR, input_option_index ? input_option_value : "");
	return false;
}

HANDLE winui_find_first_file_utf8(const char* filename, WIN32_FIND_DATA *findfiledata)
{
	std::unique_ptr<wchar_t[]> wcs_filename(mui_utf16_from_utf8cstring(filename));

	if (!wcs_filename)
		return 0;

	HANDLE result = storage::find_first_file(wcs_filename.get(), findfiledata);
	return result;
}
...
static bool DefaultInputPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	index++;
	snprintf(path, std::size(path), "%s\\*.*", dir_get_value(DIRPATH_CTRLRPATH).c_str());
	...
	if (hFind != INVALID_HANDLE_VALUE)
	{
		...
		{
			// copy the filename
			std::unique_ptr<char[]> root(mui_utf8_from_utf16cstring(FindFileData.cFileName));
			// find the extension
			...
			if (ext)
			{
				...
				if (!mui_strcmp(ext, ".cfg"))
				{
					...
					*ext = 0;
					...
					// add it as an option
					std::unique_ptr<wchar_t[]> t_root(mui_utf16_from_utf8cstring(root.get()));
...
static bool ResolutionReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	if (refresh_control && sizes_control)
	{
		...
		emu_set_value(options, option_name, option_value);
	}
	return false;
}
...
static void InitializePluginsUI(HWND hWnd)
{
	...
	if (hCtrl)
	{
		std::string t1 = dir_get_value(DIRPATH_PLUGINSPATH);
		const char* t2 = t1.c_str();
		osd::directory::ptr directory = osd::directory::open(t2);
...
static bool SelectLUAScript(HWND hWnd)
{
	...
	*filename = 0;
	...
	if (CommonFileDialog(dialog_boxes::get_open_filename, filename, FILETYPE_LUASCRIPT_FILES))
	{
		...
		utf8_optvalue = std::unique_ptr<char[]>(mui_utf8_from_utf16cstring(wcs_path_to_file.get()));

		(void)mui_strcpy(script, utf8_optvalue.get());
		PathRemoveExtensionW(const_cast<wchar_t*>(wcs_path_to_file.get()));
		utf8_optname = std::unique_ptr<char[]>(mui_utf8_from_utf16cstring(wcs_path_to_file.get()));
		...
		if (mui_strcmp(script, m_CurrentOpts.value(OPTION_AUTOBOOT_SCRIPT)))
		{
			...
			windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hWnd, IDC_LUASCRIPT), option);
			changed = true;
		}
	}
	...
	*filename = 0;
	...
	if (CommonFileDialog(dialog_boxes::get_open_filename, filename, FILETYPE_BGFX_FILES))
	{
		...
		if (mui_strcmp(utf8_option, m_CurrentOpts.value(OSDOPTION_BGFX_SCREEN_CHAINS)))
		{
			...
			windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hWnd, IDC_BGFX_CHAINS), utf8_option);
			changed = true;
		}
	}
...
static bool DirListReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	if (directories.empty())
		emu_set_value(options, OPTION_SWPATH, "");
	else
	{
		std::string option_value = mui_utf8_from_utf16string(directories);
		emu_set_value(options, OPTION_SWPATH, option_value);
	}
...
static bool DirListPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char* option_name)
{
	std::string swpath_value = emu_get_value(options, OPTION_SWPATH);
