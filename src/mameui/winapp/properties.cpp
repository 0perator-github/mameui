...
#include "mui_audit.h"
#include "mui_opts.h"
#include "directories.h" // deps on stuff found in mui_opts
#include "mui_util.h"
#include "resource.h"
...
//  OptionsCopy(dummy,m_CurrentOpts);
	...
	load_options(m_CurrentOpts, OPTIONS_GLOBAL, GLOBAL_OPTIONS, false);
	load_options(m_OrigOpts, OPTIONS_GLOBAL, GLOBAL_OPTIONS, false);
	load_options(m_DefaultOpts, OPTIONS_GLOBAL, GLOBAL_DEFAULT_OPTIONS, false);
...
void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, OPTIONS_TYPE opt_type, int folder_id, int game_num, int start_page )
{
	...
	{
	case OPTIONS_GAME:
		wcs_description = mui_wcstring_from_utf8(driver_list::driver(g_nGame).type.fullname());
		break;
	case OPTIONS_SOURCE:
