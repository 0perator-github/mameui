...
static bool SelectPlugins(HWND hWnd)
{
	...
	bool not_enabled = true;
	mui_stringtokenizer tokenizer(value, ",");
	while (tokenizer.has_next())
	{
		if (mui_strcmp(tokenizer.next_token(), new_value) == 0)
		{
			not_enabled = false;
			break;
		}
...
static bool DirListPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char* option_name)
{
	...
	{
		// append this item
		AppendList(control, tokenizer.next_token_cstr(), current_item);
		++current_item;
	}
