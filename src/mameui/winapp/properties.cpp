...
#ifdef MESS
#if 0 // Commit 2d0a09b removed the ability to configure ":ram" from the command line & not having a UI; gutting ram_size from emuopts.h, and extra_options & ram_device_enumerator from ram.h
...
static std::size_t g_nGame = 0;
static std::size_t g_nFolder = 0;
...
bool PropSheetFilter_Vector(const machine_config *drv, const game_driver *gamedrv)
{
	...
	return DriverIsVector(driver_index);
}
...
void InitPropertyPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS opt_type, std::size_t folder_id, std::size_t driver_index)
{
	if (driver_index >= driver_list::total())
	{
		std::wcout << L"InitPropertyPage: Invalid driver index: " << driver_index << std::endl;
		return;
	}
...
void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS opt_type, std::size_t folder_id, std::size_t driver_index, int start_page )
{
	if (driver_index >= driver_list::total())
	{
		std::wcout << L"InitPropertyPageToPage: Invalid driver index: "<< driver_index << std::endl;
		return;
	}
...
static std::string GameInfoScreen(UINT nIndex)
{
	...
	else
	{
		...
		else
		{
			...
			{
				const double screen_hertz = screen.frame_period().as_hz();
...
std::string GameInfoStatus(std::size_t driver_index, bool bRomStatus)
{
	if (driver_index >= driver_list::total())
		return std::string{};
...
std::string GameInfoTitle(SOFTWARETYPE_OPTIONS software_type, std::size_t driver_index)
{
	if (driver_index >= driver_list::total())
		return std::string{};
...
static std::string GameInfoCloneOf(int driver_index)
{
	...
	std::size_t parent_index = GetParentIndex(&driver_list::driver(driver_index));
	if (parent_index == INVALID_INDEX || parent_index >= driver_list::total())
		return {};
...
static bool RamPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, std::string option_name)
{
	...
	device_interface_enumerator<ram_device> iter(cfg.root_device());
	...
	if (device)
	{
		...
		uint64_t current_ram = 0;
		const char* this_ram_string = options->value(OPTION_RAMSIZE);
		if (this_ram_string)
			current_ram = parse_string(this_ram_string);
		if (current_ram == 0)
			current_ram = ramdev->size();

		std::string ramtext = messram_string(current_ram);
		std::wstring wcs_ramstring = mui_utf16_from_utf8string(ramtext);
		if (wcs_ramstring.empty())
			return false;

		combo_box::insert_string(control, i, (LPARAM)wcs_ramstring.c_str());
		...
		if (!ramdev->extra_options().empty())
		{
			...
			for (ram_device::extra_option const& option : ramdev->extra_options())
			{
				...
				wcs_ramstring = mui_utf16_from_utf8string(ramtext2);
				if (wcs_ramstring.empty())
					continue;
