...
#ifdef MESS
static bool DirListReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name);
static bool DirListPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name);
static bool RamPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name);
...
{
	static windows_options m_OrigOpts;
	return m_OrigOpts;
}
	...
	int nGame, UINT *pnMaxPropSheets, bool isGame )
{
	...
	for (; g_propSheets[i].pfnDlgProc; i++)
	{
		...
		{
			//machine_config config(*gamedrv,GetCurrentOpts());
...
//  OptionsCopy(dummy,emu_opts.GetGlobalOpts());
//  OptionsCopy(dummy,GetOrigOpts());
//  OptionsCopy(dummy,GetCurrentOpts());

	/* Get default options to populate property sheets */
	emu_opts.load_options(GetCurrentOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_OPTIONS, false);
	emu_opts.load_options(GetOrigOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_OPTIONS, false);
	emu_opts.load_options(emu_opts.GetGlobalOpts(), SOFTWARETYPE_GLOBAL, GLOBAL_DEFAULT_OPTIONS, false);
...
void InitPropertyPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS opt_type, int folder_id, int game_num)
{
	InitPropertyPageToPage(hInst, hWnd, hIcon, opt_type, folder_id, game_num, PROPERTIES_PAGE);
}

void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, SOFTWARETYPE_OPTIONS opt_type, int folder_id, int game_num, int start_page )
{
	...
	windows_options dummy;
	emu_opts.OptionsCopy(dummy,emu_opts.GetGlobalOpts());
	emu_opts.OptionsCopy(dummy,GetOrigOpts());
	emu_opts.OptionsCopy(dummy,GetCurrentOpts());

	emu_opts.load_options(GetCurrentOpts(), opt_type, game_num, 1);
	emu_opts.load_options(GetOrigOpts(), opt_type, game_num, 1);
	if (game_num == GLOBAL_OPTIONS)
		emu_opts.load_options(emu_opts.GetGlobalOpts(), SOFTWARETYPE_GLOBAL, -2, 0); // base opts is the backup for global
	else
		emu_opts.load_options(emu_opts.GetGlobalOpts(), SOFTWARETYPE_GLOBAL, -1, 0); // global is the backup for games
...
{
	machine_config config(driver_list::driver(nIndex), emu_opts.GetGlobalOpts());
...
static std::string GameInfoManufacturer(int driver_index)
{
	...
	manufacturer_info = util::string_format("%s %s", driver_list::driver(driver_index).year, driver_list::driver(driver_index).manufacturer);
...
std::string GameInfoTitle(SOFTWARETYPE_OPTIONS software_type, int driver_index)
{
	...
	case SOFTWARETYPE_GLOBAL:
		...
		break;
	case SOFTWARETYPE_SOURCE:
		title_info = util::string_format("Properties for machines in %s", GetDriverFilename_utf8(driver_index));
		break;
	case SOFTWARETYPE_COMPUTER:
	...
	case SOFTWARETYPE_VERTICAL:
		...
		break;
	case SOFTWARETYPE_GAME:
		title_info = util::string_format("%s\n\"%s\"", driver_list::driver(driver_index).type.fullname(), driver_list::driver(driver_index).name);
	default:
		break;
	}
	return title_info;
...
static std::string GameInfoCloneOf(int driver_index)
{
	...
	if (DriverIsClone(driver_index))
	{
		...
		clone_info = util::string_format("%s - \"%s\"",
			ConvertAmpersandString(driver_list::driver(nParentIndex).type.fullname()),
...
INT_PTR CALLBACK GameOptionsProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		g_bUseDefaults = emu_opts.AreOptionsEqual(GetCurrentOpts(), emu_opts.GetGlobalOpts()) ? false : true;
...
//              g_bUseDefaults = AreOptionsEqual(GetCurrentOpts(), emu_opts.GetGlobalOpts()) ? false : true;
//              g_bReset = AreOptionsEqual(GetCurrentOpts(), GetOrigOpts()) ? false : true;
				// Turn off Apply
...
//              break;
			...
			case IDC_USE_DEFAULT:
				...
				UpdateProperties(hDlg, properties_datamap, emu_opts.GetGlobalOpts());
				// The original options become the current options.
				UpdateOptions(hDlg, properties_datamap, GetCurrentOpts());
...
static bool SelectGLSLShader(HWND hWnd, int slot, bool is_scr)
{
	...
	*filename = 0;
	...
	if (CommonFileDialog(dialog_boxes::get_open_filename, filename, FILETYPE_SHADER_FILES))
	{
		...
		std::unique_ptr<char[]> option_name(mui_utf8_from_utf16cstring(path_to_file.get()));
		(void)mui_strcpy(option, option_name.get());


		if (mui_strcmp(option, GetCurrentOpts().value(shader)))
		{
			std::unique_ptr<char[]> option_value(mui_utf8_from_utf16cstring(filename));
			option_value[mui_strlen(option_value.get())-4] = '\0';    // must remove ext or glsl fails
			emu_opts.emu_set_value(GetCurrentOpts(), shader, option_value.get());
			windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hWnd, dialog), option);
			changed = true;
		}
	}
...
static void OptionsToProp(HWND hWnd, windows_options& options)
{
	...
	if (hCtrl)
	{
		...
		else
		{
			...
			char buffer[260];
			std::unique_ptr<wchar_t[]> wcs_filename(mui_utf16_from_utf8cstring(script));
			std::unique_ptr<wchar_t[]> wcs_path_to_file(shell::path_find_filename(wcs_filename.get()));
			shell::path_remove_extension(wcs_path_to_file.get());
			std::unique_ptr<char[]> utf8_optname(mui_utf8_from_utf16cstring(wcs_path_to_file.get()));
			(void)mui_strcpy(buffer, utf8_optname.get());
			windows::set_window_text_utf8(hCtrl, buffer);
		}
	}
...
//      }
	...
	/* Sound options */
	bool sound = (!mui_stricmp(GetCurrentOpts().value(OSDOPTION_SOUND), "none"));
...
static bool RotateReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
...
static bool RotatePopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
...
static bool ScreenReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
...
static bool ScreenPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	//int iMonitors;
	DISPLAY_DEVICEW dd;
	int i = 0;
...
static bool ViewPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	int selected_index = 0;
...
static bool SnapViewPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	int selected_index = 0;
...
static bool DefaultInputReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
...
static bool DefaultInputPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
...
static bool ResolutionReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
...
static bool ResolutionPopulateControl(datamap *map, HWND dialog, HWND control_, windows_options *options, const char *option_name)
{
	HWND sizes_control = dialog_boxes::get_dlg_item(dialog, IDC_SIZES);
	HWND refresh_control = dialog_boxes::get_dlg_item(dialog, IDC_REFRESH);
...
static void BuildDataMap(void)
{
	...
	datamap_add(properties_datamap, IDC_SNAPSIZEWIDTH,          DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_SNAPSIZEHEIGHT,         DM_STRING,  nullptr);
	...
	datamap_add(properties_datamap, IDC_ROTATE,                 DM_INT,     nullptr);
...
//  datamap_add(properties_datamap, IDC_WIDESTRETCH,            DM_BOOL,    OPTION_WIDESTRETCH);
	...
	datamap_add(properties_datamap, IDC_SCREEN,                 DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_SCREENSELECT,           DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_VIEW,                   DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_ASPECTRATIOD,           DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_ASPECTRATION,           DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_REFRESH,                DM_STRING,  nullptr);
	datamap_add(properties_datamap, IDC_SIZES,                  DM_STRING,  nullptr);
	...
	datamap_set_int_format(properties_datamap, IDC_AUDIO_LATENCY_DISP,  "%1.2f");
	...
	datamap_set_trackbar_range(properties_datamap, IDC_AUDIO_LATENCY, 0.00, 0.50, (float)0.01);
...
#ifdef MESS
	// MESS specific stuff
	datamap_add(properties_datamap, IDC_DIR_LIST,                    DM_STRING, nullptr);
...
static void InitializePluginsUI(HWND hWnd)
{
	...
	if (hCtrl)
	{
		std::string t1 = emu_opts.dir_get_value(DIRPATH_PLUGINSPATH);
		const char* t2 = t1.c_str();
		osd::directory::ptr directory = osd::directory::open(t2);
		...
		for (const osd::directory::entry *entry = directory->read(); entry; entry = directory->read())
		{
			if (entry->type == osd::directory::entry::entry_type::DIR)
			{
				...
				if (!(name == "." || name == ".." || name == "json"))
				{
					...
					std::unique_ptr<wchar_t[]> wcsName(mui_utf16_from_utf8cstring(label));
					label = 0;
					if( !wcsName)
						return;
...
static bool SelectEffect(HWND hWnd)
{
	...
	*filename = 0;
	if (CommonFileDialog(dialog_boxes::get_open_filename, filename, FILETYPE_EFFECT_FILES))
	{
		...
		option_value = GetCurrentOpts().value(OPTION_EFFECT);
		std::unique_ptr<wchar_t[]> wcs_optionvalue(mui_utf16_from_utf8cstring(option_value));
		if (mui_wcscmp(buff, wcs_optionvalue.get()))
		{
			HWND control = dialog_boxes::get_dlg_item(hWnd, IDC_EFFECT);
			emu_opts.emu_set_value(GetCurrentOpts(), OPTION_EFFECT, option_value);
			windows::set_window_text_utf8(control, option_value);
			// datamap_populate_control(properties_datamap, hWnd, GetCurrentOpts(), IDC_EFFECT);
			changed = true;
		}
	}
	return changed;
...
int winui_get_window_text_utf8(HWND hWnd, char *buffer, size_t buffer_size)
{
	...
	std::unique_ptr<char[]> utf8_buffer(mui_utf8_from_utf16cstring(wcs_buffer));
...
static bool ChangeFallback(HWND hWnd)
{
	...
	if (mui_strcmp(data, GetCurrentOpts().value(OPTION_FALLBACK_ARTWORK)))
	{
		emu_opts.emu_set_value(GetCurrentOpts(), OPTION_FALLBACK_ARTWORK, data);
		changed = true;
	}
...
static bool ChangeOverride(HWND hWnd)
{
	...
	if (mui_strcmp(data, GetCurrentOpts().value(OPTION_OVERRIDE_ARTWORK)))
	{
		emu_opts.emu_set_value(GetCurrentOpts(), OPTION_OVERRIDE_ARTWORK, data);
		changed = true;
	}
...
static bool ChangeJoystickMap(HWND hWnd)
{
	...
	if (mui_strcmp(joymap, GetCurrentOpts().value(OPTION_JOYSTICK_MAP)))
	{
		emu_opts.emu_set_value(GetCurrentOpts(), OPTION_JOYSTICK_MAP, joymap);
		changed = true;
	}
...
static bool ResetJoystickMap(HWND hWnd)
{
	...
	if (mui_strcmp(new_value, GetCurrentOpts().value(OPTION_JOYSTICK_MAP)))
	{
...
static bool SelectLUAScript(HWND hWnd)
{
	...
	if (CommonFileDialog(dialog_boxes::get_open_filename, filename, FILETYPE_LUASCRIPT_FILES))
	{
		...
		std::string script = filePath.stem().string();

		// Set the option if changed
		if (mui_strcmp(script.c_str(), GetCurrentOpts().value(OPTION_AUTOBOOT_SCRIPT)))
		{
			emu_opts.emu_set_value(GetCurrentOpts(), OPTION_AUTOBOOT_SCRIPT, script.c_str());
			windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hWnd, IDC_LUASCRIPT), script.c_str());
			changed = true;
		}
	}
...
static bool ResetLUAScript(HWND hWnd)
{
	...
	if (mui_strcmp(new_value, GetCurrentOpts().value(OPTION_AUTOBOOT_SCRIPT)))
	{
...
static bool SelectPlugins(HWND hWnd)
{
	...
	value = emu_opts.emu_get_value(GetCurrentOpts(), OPTION_PLUGIN);
	...
	if (!already_enabled)
	{
		std::ostringstream oss;
		oss << value << "," << new_value;
		std::string new_option = oss.str();
		emu_opts.emu_set_value(GetCurrentOpts(), OPTION_PLUGIN, new_option);
		windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hWnd, IDC_PLUGIN), new_option.c_str());
		changed = true;
	}
...
static bool SelectBGFXChains(HWND hWnd)
{
	...
	{
		std::filesystem::path filePath(filename);
		std::string chainName = filePath.stem().string();

		if (mui_strcmp(chainName.c_str(), GetCurrentOpts().value(OSDOPTION_BGFX_SCREEN_CHAINS)))
		{
			emu_opts.emu_set_value(GetCurrentOpts(), OSDOPTION_BGFX_SCREEN_CHAINS, chainName.c_str());
			windows::set_window_text_utf8(dialog_boxes::get_dlg_item(hWnd, IDC_BGFX_CHAINS), chainName.c_str());
			changed = true;
		}
	}
...
static bool ResetBGFXChains(HWND hWnd)
{
	...
	if (mui_strcmp(new_value, GetCurrentOpts().value(OSDOPTION_BGFX_SCREEN_CHAINS)))
	{
...
static bool DirListReadControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char *option_name)
{
	...
	for (size_t index = 0; index < directory_count; index++)
	{
		...
		lvi.iItem = index;
		lvi.pszText = item_text;
		lvi.cchTextMax = std::size(item_text);
...
static bool DirListPopulateControl(datamap *map, HWND dialog, HWND control, windows_options *options, const char* option_name)
{
	...
	int current_item = 0;
	mui_wstringtokenizer tokenizer(directories, L";");

	// add each of the directories
	for (auto token : tokenizer)
	{
		// append this item
		AppendList(control, token.data(), current_item);
		++current_item;
	}
...
static bool SoftwareDirectories_OnEndLabelEdit(HWND hDlg, NMHDR* pNMHDR)
{
	...
	if (pItem->pszText)
	{
		...
		if (dialog_boxes::message_box(nullptr, L"Folder does not exist, continue anyway?", &MAMEUINAME[0], MB_OKCANCEL) == IDOK)
			bResult = true;
	}
