...
typedef struct winui_ui_options_entry_t
{
...
public:
	...
	void reset_and_save(const char*);

	std::string_view getter(const char*);
	void setter(const char*, int);
	void setter(const char*, std::string_view);

	int int_value(const char* option_name)
	{
		std::string_view option_value = getter(option_name);
		if (option_value.empty())
			return 0;
		else
			return std::stoi(&option_value[0]);
	}

	bool bool_value(const char* option_name)
	{
		return int_value(option_name) ? true : false;
	}
...
private:
	...
	bool ends_with_filters(std::string_view str);
