...
static const LPCWSTR mess_column_names[] =
{
	L"Filename",
};

// columns for software list
static const LPCWSTR softlist_column_names[] =
{
	L"Name",
	L"List",
	L"Description",
...
void InitMessPicker(void)
{
	...
	menus::check_menu_item(GetMenu(GetMainWindow()), ID_VIEW_SOFTWARE_AREA, bShowSoftware ? MF_CHECKED : MF_UNCHECKED);
}
...
bool CreateMessIcons(void)
{
	...
	if (!mess_icon_index)
	{
		...
		mess_icon_index = make_unique_clear<int[]>(icon_index_size);
	}
...
static int GetMessIcon(int driver_index, std::string software_type)
{
	...
	if (icon_pos >= 0)
	{
		...
		while (driver)
		{
			...
			hIcon = LoadIconFromFile(&file_path[0]);
...
static std::string ProcessSWDir(int drvindex)
{
	...
	dirmap_swpath = dir_get_value(DIRMAP_SWPATH);
...
{
	LONG_PTR l = windows::get_window_long_ptr(hwndEdit, GWLP_USERDATA);
...
static BOOL MView_SetDriver(HWND hwndMView, const software_config *sconfig)
{
	...
	if (pMViewInfo->slots)
	{
		...
		std::string instance;
		LPWSTR *ppszDevices;
		ppszDevices = (LPWSTR*) alloca(pMViewInfo->slots * sizeof(*ppszDevices));
		i = 0;
		for (device_image_interface &dev : image_interface_enumerator(pMViewInfo->config->mconfig->root_device()))
		{
			if (!dev.user_loadable())
			...
			ppszDevices[i] = (LPWSTR)mui_wcstring_from_utf8(instance.c_str());
			i++;
		}
		std::cout << "MView_SetDriver: Number of media slots = " << i << std::endl;
		...
		{
			GetTextExtentPoint32W(hDc, ppszDevices[i], mui_wcslen(ppszDevices[i]), &sz);
		...
		struct MViewEntry *pEnt;
		pEnt = new struct MViewEntry[pMViewInfo->slots + 1] {};
		if (!pEnt)
			return false;
		...
		for (device_image_interface &dev : image_interface_enumerator(pMViewInfo->config->mconfig->root_device()))
		{
			...
			pEnt->hwndStatic = windows::create_window_ex_utf8(0, "STATIC", instance.c_str(), // display it
...
bool MyFillSoftwareList(int drvindex, bool bForce)
{
	...
	if (paths[0] == '1')
	{
		...
		paths.erase(0,1);
		std::unique_ptr<wchar_t[]> wcs_paths(mui_wcstring_from_utf8(&paths[0]));
		AddSoftwarePickerDirs(hwndSoftwarePicker, wcs_paths.get());
	}
...
static void MessSpecifyImage(int drvindex, const device_image_interface *device_image, std::string file_name)
{
	...
	if (device_image)
	{
		SetSelectedSoftware(drvindex, device_image->instance_name(), file_name);
		if (LOG_SOFTWARE)
	...
	if (!file_extension.empty())
	{
		...
		for (device_image_interface &current_image : image_interface_enumerator(s_config->mconfig->root_device()))
		{
			...
			if (uses_file_extension(current_image, file_extension))
			{
				...
				{
					// place the image
					SetSelectedSoftware(drvindex, option_name, file_name);
					if (LOG_SOFTWARE)
...
static std::vector<mess_image_type> SetupImageTypes(bool include_archives, const device_image_interface* dev)
{
	...
	if (dev != nullptr)
	{
		...
		{
			std::wstring dlgname;
			std::unique_ptr<const wchar_t[]> wcs_ext;

			dlgname = lookupdevice(dev->image_brief_type_name())->dlgname;
			wcs_ext = std::unique_ptr<const wchar_t[]>(mui_wcstring_from_utf8(token.c_str()));
			image_types.emplace_back(mess_image_type{ dev, wcs_ext.get(), dlgname});
		}
	}
...
static void MessSetupDevice(common_file_dialog_proc cfd, const device_image_interface *dev)
{
	...
	int drvindex;
	std::vector<mess_image_type> image_types;
	std::wstring_view software_path;
	std::string::size_type find_pos;
	wchar_t file_name[MAX_PATH];

	software_path = std::unique_ptr<wchar_t[]>(mui_wcstring_from_utf8(&dir_get_value(DIRMAP_SWPATH)[0])).get();
...
//  machine_config config(driver_list::driver(drvindex), MameUIGlobal());
	...
	bResult = CommonFileImageDialog(&software_path[0], cfd, file_name, image_types);
	if (bResult)
	{
...
static bool MView_GetOpenFileName(const device_image_interface *dev, std::wstring_view file_name)
{
	...
	load_options(options, OPTIONS_GAME, drvindex, 1);
	...
	image_types = SetupImageTypes(true, dev);
	std::unique_ptr<wchar_t[]> wcs_dst(mui_wcstring_from_utf8(&initial_directory[0]));
	bResult = CommonFileImageDialog(wcs_dst.get(), GetOpenFileNameW, const_cast<wchar_t*>(&file_name[0]), image_types);
	if (bResult)
	{
		std::unique_ptr<const char[]> utf8_file_name(mui_utf8_from_wcstring(&file_name[0]));
		SetSelectedSoftware(drvindex, dev->instance_name(), utf8_file_name.get());
		mvmap[option_name] = 1;
	}
...
static bool MView_GetOpenItemName(const device_image_interface *dev,std::wstring_view file_name)
{
	...
	std::unique_ptr<const wchar_t[]> wcs_dst(mui_wcstring_from_utf8(dst.c_str()));
	imagetypes = SetupImageTypes(true, NULL); // just get zip & 7z
	bResult = CommonFileImageDialog(wcs_dst.get(), dialog_boxes::get_open_filename, const_cast<wchar_t*>(&file_name[0]), imagetypes);
	...
	if (bResult)
	{
		...
		std::unique_ptr<char[]>utf8_parsed_file_name(mui_utf8_from_wcstring(&parsed_file_name[0]));
		SetSelectedSoftware(drvindex, dev->instance_name(), utf8_parsed_file_name.get());
		mvmap[opt_name] = 1;
	}
...
static bool MView_GetCreateFileName(const device_image_interface *dev, std::wstring_view file_name)
{
	...
	initial_directory = std::unique_ptr<wchar_t[]>(mui_wcstring_from_utf8(&ProcessSWDir(drvindex)[0])).get();
	initial_directory.erase(0,1);
	/* We only want the first path; throw out the rest */
	find_pos = initial_directory.find(';');
	if (find_pos != std::wstring::npos)
	...
	image_types = SetupImageTypes(true, dev);
	bResult = CommonFileImageDialog(&initial_directory[0], GetSaveFileNameW, const_cast<wchar_t*>(&file_name[0]), image_types);
	if (bResult)
	{
		std::string utf8_file_name;
		utf8_file_name = std::unique_ptr<char[]>(mui_utf8_from_wcstring(&file_name[0])).get();
		SetSelectedSoftware(drvindex, dev->instance_name(), utf8_file_name);
...
{
	std::unique_ptr<const char[]> utf8_filename(mui_utf8_from_wcstring(&file_name[0]));
	if( !utf8_filename )
		return;
	MessSpecifyImage(drvindex, dev, utf8_filename.get());
	MessRefreshPicker();
}
	...
	if (dev && dev->user_loadable())
	{
		...
		if (o.has_image_option(opt_name))
		{
			...
			if (!opt_value.empty())
			{
				...
				std::wstring utf8_to_wcs(std::unique_ptr<const wchar_t[]>(mui_wcstring_from_utf8(opt_value.c_str())).get());
...
static void SoftwarePicker_EnteringItem(HWND hwndSoftwarePicker, int nItem)
{
	...
	if (!s_bIgnoreSoftwarePickerNotifies)
	{
		...
		if (drvindex < 0)
		{
			std::fill(std::begin(g_szSelectedItem), std::end(g_szSelectedItem), '\0');
			return;
		}
		...
		std::cout << "SoftwarePicker_EnteringItem: full_name = \"" << part_name << "\"" << std::endl;
		// Do the dirty work
		MessSpecifyImage(drvindex, NULL, full_name.c_str());
		part_name.resize(part_name.find('.') - 1);
		// Set up g_szSelectedItem, for the benefit of UpdateScreenShot()
		(void)mui_strcpy(g_szSelectedItem, part_name.c_str());
...
{
	int nColumnCount = Picker_GetColumnCount(MyColumnDialogProc_hwndPicker);
	const LPCWSTR *ppszColumnNames = Picker_GetColumnNames(MyColumnDialogProc_hwndPicker);
...
static BOOL RunColumnDialog(HWND hwndPicker)
{
	...
	BOOL bResult = dialog_boxes::dialog_box(system_services::get_module_handle(NULL), menus::make_int_resource(IDD_COLUMNS), GetMainWindow(), MyColumnDialogProc, 0L);
	return bResult;
}
...
static void SoftwarePicker_OnHeaderContextMenu(POINT pt, int nColumn)
{
	HMENU hMenuLoad = LoadMenu(NULL, menus::make_int_resource(IDR_CONTEXT_HEADER));
...
BOOL MessCommand(HWND hwnd,int id, HWND hwndCtl, UINT codeNotify)
{
	switch (id)
	{
		case ID_MESS_OPEN_SOFTWARE:
			MessSetupDevice(GetOpenFileNameW, NULL);
			break;
...
static LPCSTR SoftwareTabView_GetTabLongName(int tab)
{
	return s_tabs[tab] + mui_strlen(s_tabs[tab]) + 1;
}
...
static void MView_ButtonClick(HWND hwndMView, struct MViewEntry *pEnt, HWND hwndButton)
{
	...
	if (software)
	{
		...
		dirmap_mediapath = dir_get_value(DIRMAP_MEDIAPATH);
		...
		while (std::getline(tokenStream, sl_root, ';'))
		{
			DWORD dwAttrib;
			std::string as;
			std::unique_ptr<const wchar_t[]> wcs_path;

			as = sl_root + std::string("\\") + slmap.find(opt_name)->second;
			wcs_path = std::unique_ptr<const wchar_t[]>(mui_wcstring_from_utf8(as.c_str()));
			dwAttrib = GetFileAttributesW(wcs_path.get());
			if ((dwAttrib != INVALID_FILE_ATTRIBUTES) && (dwAttrib & FILE_ATTRIBUTE_DIRECTORY))
			{
				std::cout << "MView_ButtonClick: wcs_path = " << wcs_path.get() << std::endl;
				slmap[opt_name] = as;
				passed_tests = true;
				break;
			}
	...
	if (pMViewInfo->pCallbacks->pfnGetOpenFileName)
		AppendMenuW(hMenu, MF_STRING, 1, L"Mount File...");

	if (passed_tests && pMViewInfo->pCallbacks->pfnGetOpenItemName)
		AppendMenuW(hMenu, MF_STRING, 4, L"Mount Item...");

	if (pEnt->dev->is_creatable())
	{
		if (pMViewInfo->pCallbacks->pfnGetCreateFileName)
			AppendMenuW(hMenu, MF_STRING, 2, L"Create...");
	}

	if (mvmap.find(opt_name)->second == 1)
		AppendMenuW(hMenu, MF_STRING, 3, L"Unmount");

	*file_name = '\0';
	(void)windows::get_window_rect(hwndButton, &r);

	rc = TrackPopupMenu(hMenu, TPM_TOPALIGN | TPM_NONOTIFY | TPM_RETURNCMD, r.left, r.bottom, 0, hwndMView, NULL);
	switch(rc)
	{
		case 1:
	...
	if (result)
	{
		SetWindowTextW(pEnt->hwndEdit, file_name);
	}
}
...
static void MView_Free(HWND hwndMView)
{
	...
	MView_Clear(hwndMView);
	pMViewInfo = GetMViewInfo(hwndMView);
	(void)DeleteObject(pMViewInfo->hFont);
	delete pMViewInfo;
}
...
static LRESULT CALLBACK MView_WndProc(HWND hwndMView, UINT nMessage, WPARAM wParam, LPARAM lParam)
{
	...
	if (!bHandled)
		rc = DefWindowProcW(hwndMView, nMessage, wParam, lParam);
...
void MView_RegisterClass(void)
{
	...
	wc.hInstance = system_services::get_module_handle(NULL);
