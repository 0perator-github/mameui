...
static std::string ProcessSWDir(int drvindex)
{
	...
	bool b_dir = false;
	int nCloneIndex = -1, nParentIndex = -1;
	size_t find_pos;
	...
	if (!dirmap_swpath.empty())
	{
		...
		{
			std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid SWPATH directory " << dirmap_swpath << " in " << __FUNCTION__ << std::endl;
			return result;
		}
	}
	...
	{
		std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid SWPATH option " << option_swpath << " in " << __FUNCTION__ << std::endl;
		return result;
	}
	...
	if (DriverIsClone(drvindex))
	{
		...
		if (nParentIndex >= 0)
		{
			...
			{
				std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid SOFTWARETYPE_PARENT option " << option_swpath << " in " << __FUNCTION__ << std::endl;
				return result;
			}
		}
	}

	// don't pass -1 to compat check
	if (nParentIndex < 0) nParentIndex = drvindex;
