...
static std::string ProcessSWDir(int drvindex)
{
	...
	dirmap_swpath = dir_get_value(DIRPATH_SWPATH);
	...
	windows_options o;
	load_options(o, SOFTWARETYPE_GAME, drvindex, 0);
	...
	if (DriverIsClone(drvindex))
	{
		...
		if (nParentIndex >= 0)
		{
			...
			load_options(o, SOFTWARETYPE_PARENT, nParentIndex, 0);
	...
	if (nCloneIndex >= 0)
	{
		...
		load_options(o, SOFTWARETYPE_PARENT, nCloneIndex, 0);
		option_swpath = o.value(OPTION_SWPATH);
...
static BOOL MView_SetDriver(HWND hwndMView, const software_config *sconfig)
{
	...
	if (pMViewInfo->slots)
	{
		...
		for (device_image_interface &dev : image_interface_enumerator(pMViewInfo->config->mconfig->root_device()))
		{
			...
			ppszDevices[i] = (LPWSTR)utf16_instance_name.c_str();
			i++;
		}
		...
		{
			gdi::get_text_extent_point_32(hDc, ppszDevices[i], mui_wcslen(ppszDevices[i]), &sz);
...
static void MessRefreshPicker(void)
{
	...
	windows_options o;
	load_options(o, SOFTWARETYPE_GAME, s_config->driver_index, 1);
...
static std::vector<mess_image_type> SetupImageTypes(bool include_archives, const device_image_interface* dev)
{
	...
	if (dev != nullptr)
	{
		...
		while (std::getline(tokenStream, token, ','))
		{
			...
			image_types.emplace_back(mess_image_type{ dev, mui_utf16_from_utf8string(token), dlgname});
		}
	}
...
static void MessSetupDevice(common_file_dialog_proc cfd, const device_image_interface *dev)
{
	...
	software_path = dir_get_value(DIRPATH_SWPATH);
...
static bool MView_GetOpenFileName(const device_image_interface *dev, std::wstring_view file_name)
{
	...
	load_options(options, SOFTWARETYPE_GAME, drvindex, 1);
	...
	if (bResult)
	{
		SetSelectedSoftware(drvindex, dev->instance_name(), mui_utf8_from_utf16string(file_name));
		mvmap[option_name] = 1;
	}
...
static bool MView_GetOpenItemName(const device_image_interface *dev,std::wstring_view file_name)
{
	...
	if (bResult)
	{
		...
		SetSelectedSoftware(drvindex, dev->instance_name(), mui_utf8_from_utf16string(parsed_file_name));
		mvmap[opt_name] = 1;
	}
...
static bool MView_GetCreateFileName(const device_image_interface *dev, std::wstring_view file_name)
{
	...
	if (bResult)
	{
		SetSelectedSoftware(drvindex, dev->instance_name(), mui_utf8_from_utf16string(file_name.data()));
...
static void MView_ButtonClick(HWND hwndMView, MViewEntry *pEnt, HWND hwndButton)
{
	...
	if (software)
	{
		...
		dirmap_mediapath = dir_get_value(DIRPATH_MEDIAPATH);
