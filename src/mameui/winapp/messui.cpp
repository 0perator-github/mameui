...
using MViewCallbacks = struct mview_callbacks
{
	...
	bool (*pfnGetCreateFileName)(device_image_interface *dev, std::wstring_view file_name);
...
static bool ProcessFloppyDevice(floppy_image_device *floppy_dev, const std::string &file_extension);

static bool MView_GetOpenFileName(const device_image_interface *dev, std::wstring_view file_name);
static std::wstring GetSoftwareNameFromPath(std::wstring_view full_path_view);
static bool MView_GetOpenItemName(const device_image_interface *dev, std::wstring_view file_name);
static bool MView_GetCreateFileName(device_image_interface *dev, std::wstring_view file_name);
...
static std::pair<std::error_condition, std::string> ProcessSWDir(int drvindex)
{
	...
	{
		std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid driver index (" << drvindex << ") in " << __FUNCTION__ << std::endl;
	...
	{
		std::cerr << __FILE__ << ": " << __LINE__ << ": No value for directory map index (" << DIRPATH_SWPATH << ") in " << __FUNCTION__ << std::endl;
	...
	if (std::filesystem::exists(dirmap_path))  // make sure its valid
		map_path_exist = true;
	...
	if (option_value && *option_value != '\0')
	{
		...
		if (std::filesystem::exists(software_path))  // make sure its valid
		{
			if (map_path_exist && dirmap_path != software_path)
			{

				std::cout << __FUNCTION__ << ": Using " << driver_list::driver(drvindex).type.fullname() << "'s software path = (" << software_path << ")" << std::endl;
				result.first = std::errc::is_a_directory; // show loose software
				result.second = software_path;
				return result;
			}
		}
	}
	...
	if (DriverIsClone(nParentIndex))
	{
		nParentIndex = GetParentIndex(&driver_list::driver(drvindex));
		if (nParentIndex < 0 || nParentIndex >= driver_list::total())
		{
			std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid parent index (" << nParentIndex << ") in " << __FUNCTION__ << std::endl;
			result.first = std::errc::invalid_argument;
			return result;
		}
		else
		{
			emu_opts.load_options(o, SOFTWARETYPE_PARENT, nParentIndex, false);
			...
			if (option_value && *option_value != '\0')
			{
				...
				if (std::filesystem::exists(software_path))  // make sure its valid
				{
					if (map_path_exist && dirmap_path != software_path)
					{
						std::cout << __FUNCTION__ << ": Using parent, " << driver_list::driver(nParentIndex).type.fullname() << ", with software path = (" << software_path << ")" << std::endl;
	...
	if (nCloneIndex == -1)
		nCloneIndex = nParentIndex;

	if (nCloneIndex < 0 || nCloneIndex >= driver_list::total())
	{
		std::cerr << __FILE__ << ": " << __LINE__ << ": Invalid clone index (" << nCloneIndex << ") in " << __FUNCTION__ << std::endl;
		result.first = std::errc::invalid_argument;
		return result;
	}

	if (nCloneIndex != nParentIndex)
	{
		emu_opts.load_options(o, SOFTWARETYPE_PARENT, nCloneIndex, false);
		...
		if (option_value && *option_value != '\0')
		{
			...
			if (std::filesystem::exists(software_path))  // make sure its valid
			{
				if (map_path_exist && dirmap_path != software_path)
				{
					std::cout << __FUNCTION__ << ": Using compatible system, " << driver_list::driver(nCloneIndex).type.fullname() << ", with software path = (" << software_path << ")" << std::endl;
					result.first = std::errc::is_a_directory;
					result.second = software_path;
					return result;
				}
	...
	if (map_path_exist)
	{
		std::cout << __FUNCTION__ << ": Using global software path = " << dirmap_path << std::endl;
		result.second = dirmap_path;
		return result;
	}

	// If all else fails, return the working dir
	result.second = emu_opts.get_exe_path_utf8();
	std::cout << __FUNCTION__ << ": Using working directory = " << result.second << std::endl;
...
bool MyFillSoftwareList(int drvindex, bool bForce)
{
	...
	std::cout << __FUNCTION__ << ": Calling SoftwareList_Clear" << std::endl;
	SoftwareList_Clear(hwndSoftwareList);
	std::cout << __FUNCTION__ << ": Calling SoftwarePicker_Clear" << std::endl;
	SoftwarePicker_Clear(hwndSoftwarePicker);

	// set up the device view
	std::cout << __FUNCTION__ << ": Calling MView_SetDriver" << std::endl;
	MView_SetDriver(hwndSoftwareMView, s_config);

	// set up the software picker
	std::cout << __FUNCTION__ << ": Calling SoftwarePicker_SetDriver" << std::endl;
	...
	std::cout << __FUNCTION__ << ": Processing software directories" << std::endl;
	...
	std::cout << __FUNCTION__ << ": Finished processing, directory = (" << sw_paths << ")" << std::endl;

	std::wstring utf16_sw_paths = mui_utf16_from_utf8string(sw_paths);
	std::cout << __FUNCTION__ << ": Calling AddSoftwarePickerDirs" << std::endl;
	AddSoftwarePickerDirs(hwndSoftwarePicker, utf16_sw_paths);
...
static void MessSpecifyImage(int drvindex, const device_image_interface* device_image, std::string file_name)
{
	...
	{
		if (LOG_SOFTWARE)
			std::cout << "MessSpecifyImage(): No file extension found in '" << file_name << "'" << std::endl;
		return;
	}
...
static void MessRefreshPicker(void)
{
	...
	for (device_image_interface &dev : image_interface_enumerator(config.root_device()))
	{
		...
		std::cout << "MessRefreshPicker: option_name = '" << option_name << "', option_value ='" << option_value << "'" << std::endl;
...
static std::vector<mess_image_type> SetupImageTypes(bool include_archives, const device_image_interface* dev)
{
	...
	while (std::getline(tokenStream, token, L','))
	{
		...
		const std::wstring &dlgname = devinfo->dlgname;
		if (dlgname.empty())
			continue;
...
static void MessSetupDevice(common_file_dialog_proc cfd, const device_image_interface *dev)
{
	...
	std::vector<mess_image_type> image_types = SetupImageTypes(true, dev);
...
static bool MView_GetOpenFileName(const device_image_interface* dev, std::wstring_view file_name)
{
	if (!dev || file_name.size() >= MUI_MAX_CHAR_COUNT)
		return false;
	...
	if (option_name.empty())
	{
		std::cout << __FUNCTION__ << ": used invalid device: " << std::endl;
		return false;
	}
	...
	if (!std::filesystem::exists(initial_directory) || find_pos == std::string::npos)
	{
	...
	std::cout << __FUNCTION__ << ": initial_directory = " << initial_directory << std::endl;
	std::vector<wchar_t> filename_buffer(MUI_MAX_CHAR_COUNT);
	mui_wcscpy(filename_buffer.data(), std::wstring(file_name).c_str());
	...
	if (bResult)
	{
		std::wcout << L"MView_GetOpenFileName: file path = (" << filename_buffer.data() << L")" << std::endl;

		// Notify system
		std::string utf8_software_name = mui_utf8_from_utf16string(filename_buffer.data());
		emu_opts.SetSelectedSoftware(drvindex, option_name, utf8_software_name);
		mvmap[option_name] = 1;
	}

	std::cout << __FUNCTION__ << ": bResult = " << std::boolalpha << bResult << std::endl;
...
static std::wstring GetSoftwareNameFromPath(std::wstring_view full_path_view)
{
	std::filesystem::path full_path(full_path_view);
...
static bool MView_GetOpenItemName(const device_image_interface* dev, std::wstring_view file_name)
{
	...
	if (it == slmap.end())
	{
		std::cout << __FUNCTION__ << ": device, " << option_name << ", not found " << std::endl;
		return false;
	}
	...
	if (!std::filesystem::exists(initial_directory))
		initial_directory = std::filesystem::current_path().wstring();


	std::vector<wchar_t> filename_buffer(MUI_MAX_CHAR_COUNT);
	mui_wcscpy(filename_buffer.data(), std::wstring(file_name).c_str());

	std::vector<mess_image_type> imagetypes = SetupImageTypes(true, nullptr); // get zip & 7z
	...
	if (bResult)
	{
		...
		{
			std::wcerr << L"MView_GetOpenItemName: Failed to construct a valid software name, path = (" << std::wstring(filename_buffer.data()) << L")" << std::endl;
			return false;
		}

		std::wcout << L"MView_GetOpenItemName: software_name = (" << software_name << L")" << std::endl;
...
// This is used to process a floppy image file after it has been created.
static bool ProcessFloppyDevice(floppy_image_device *floppy_dev, const std::string &file_extension)
{
	const floppy_image_format_t *matched_format = nullptr;
	for (const auto &format : floppy_dev->get_formats())
	{
		std::cout << "Checking format: " << format->name() << " (." << format->extensions() << ")" << std::endl;
		if (format->extension_matches(file_extension.c_str()))
		{
	...
	auto fs_vector = floppy_dev->get_fs();
	const floppy_image_device::fs_info* matched_fsinfo = nullptr;

	if (matched_format && fs_vector.size() > 1)
	{
		for (const auto &fs : fs_vector)
		{
			std::cout << "Checking filesystem: " << fs.m_name << " for format: " << matched_format->name() << std::endl;
			if (fs.m_type == matched_format)
			{
				matched_fsinfo = &fs;
				break;
			}
	...
	if (!matched_fsinfo)
		matched_fsinfo = &fs_vector.front();

	std::error_condition err = floppy_dev->finish_load().first;
	if (err)
	{
		std::cerr << "Error finishing load: " << err.message() << std::endl;
		return false;
	}

	fs::meta_data meta;
	floppy_dev->init_fs(matched_fsinfo, meta);

	if (matched_format)
		floppy_dev->setup_write(matched_format);

	if (floppy_dev->is_open())
		floppy_dev->unload();
...
static bool MView_GetCreateFileName(device_image_interface* dev, std::wstring_view file_name)
{
	...
	// We only want the first path; throw out the rest
	std::string::size_type find_pos = sw_path.find(';');
	if (find_pos != std::string::npos)
		sw_path = sw_path.substr(0, find_pos);

	std::wstring initial_directory = mui_utf16_from_utf8string(sw_path);
	std::wcout << L"MView_GetCreateFileName: initial_directory = " << initial_directory << std::endl;
	...
	if (bResult)
	{
		std::wcout << L"MView_GetCreateFileName: software_name = (" << filename_buffer.data() << L")" << std::endl;

		// Notify system
		std::string utf8_software_name = mui_utf8_from_utf16string(filename_buffer.data());

		auto[err, message] = dev->create(utf8_software_name);
		if (err)
		{
			std::cerr << "Error creating image: " << (message.empty() ? err.message() : message) << std::endl;
			return false;
		}

		if (auto* floppy_dev = dynamic_cast<floppy_image_device*>(dev))
		{
			if (!ProcessFloppyDevice(floppy_dev, utf8_software_name))
				return false;
		}
...
{
	if (!dev || drvindex < 0 || drvindex >= driver_list::total())
		return L"";
