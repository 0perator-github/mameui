...
// ============================================================================
// messui.cpp - MESS extensions to src/osd/winui/winui.cpp
// ============================================================================
...
constexpr int MVIEW_PADDING = 10;
constexpr int MVIEW_SPACING = 21;
...
void MView_Refresh(HWND hwndMView)
{
	...
	pMViewInfo = GetMViewInfo(hwndMView);
	int driver_index = pMViewInfo->config->driver_index();
	std::cout << "MView_Refresh: " << driver_list::driver(driver_index).name << "\n";

	if (pMViewInfo->slots)
	{
		for (size_t i = 0; i < pMViewInfo->slots; i++)
		{
			pszSelection = pMViewInfo->pCallbacks->pfnGetSelectedSoftware(driver_index,
...
static BOOL MView_SetDriver(HWND hwndMView, const software_config *sconfig)
{
	...
	device_t &root_dev = pMViewInfo->config->machine_config_ptr()->root_device();
	for (device_image_interface &dev : image_interface_enumerator(root_dev))
	{
...
bool MyFillSoftwareList(int drvindex, bool bForce)
{
	...
	if (drvindex >= 0)
	{
		s_config = new software_config(drvindex);
	...
	device_t &root_dev = s_config->machine_config_ptr()->root_device();
	for (software_list_device &swlistdev : software_list_device_enumerator(root_dev))
	{
		if (swlistdev.is_compatible() || swlistdev.is_original())
		{
			for (const software_info &swinfo : swlistdev.get_info())
			{
				...
				for (const device_image_interface &image : image_interface_enumerator(root_dev))
				{
					if (!image.user_loadable())
						continue;
					if (swlistdev.is_compatible(swpart) == SOFTWARE_IS_COMPATIBLE)
					{
						const char *image_intf = image.image_interface();
						if (image_intf && swpart.matches_interface(image_intf))
						{
...
static void MessSpecifyImage(int drvindex, device_image_interface *const device_image, const std::string &file_name)
{
	...
	for (device_image_interface& current_image : image_interface_enumerator(s_config->machine_config_ptr()->root_device()))
	{
...
static void MessRefreshPicker(void)
{
	...
	windows_options o;
	int driver_index = s_config->driver_index();
	emu_opts.load_options(o, SOFTWARETYPE_GAME, driver_index, false);
...
static void MView_ButtonClick(HWND hwndMView, MViewEntry *pEnt, HWND hwndButton)
{
	...
	device_t &root_dev = pMViewInfo->config->machine_config_ptr()->root_device();
	for (software_list_device &swlist : software_list_device_enumerator(root_dev))
	{
		for (const software_info &swinfo : swlist.get_info())
		{
			...
			{
				for (device_image_interface &image : image_interface_enumerator(root_dev))
				{
					if (!image.user_loadable())
						continue;
					...
					{
						const char *image_intf = image.image_interface();
						if (image_intf && part.matches_interface(image_intf))
						{
