...
char** mameui::winapi::shell::commandline_to_argv_utf8(const char* commandline, int* number_of_args)
{
	...
	std::wstring utf16_commandline;
	if (mui_utf16_from_utf8string(utf16_commandline, commandline) < 0)
		return nullptr;


	wchar_t** utf16_argv = commandline_to_argv(utf16_commandline.data(), number_of_args);
	if (!utf16_argv || *number_of_args <= 0)
		return nullptr;

	char** utf8_argv = new(std::nothrow) char* [*number_of_args];
	if (!utf8_argv)
	{
	...
	for (int index = 0; index < *number_of_args; index++)
	{
		...
		if (!utf8_argv[index])
		{
			for (int del_index = 0; del_index < index; ++del_index)
				delete[] utf8_argv[del_index];
