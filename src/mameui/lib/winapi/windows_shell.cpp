...
char **mameui::winapi::shell::commandline_to_argv_utf8(const char *commandline, int *number_of_args)
{
	...
	wchar_t **utf16_argv(commandline_to_argv(utf16_commandline.get(), number_of_args));
	if (!utf16_argv || *number_of_args <= 0)
		return nullptr;

	char **utf8_argv = new(std::nothrow) char*[*number_of_args + 1] {};
	if (!utf8_argv)
	{
		*number_of_args = 0;
	...
	for (int index = 0; index < *number_of_args; index++)
	{
		...
		if (!utf8_argv[index])
		{
			for (int clean_idx = 0; clean_idx < index; clean_idx++)
			{
				delete[] utf8_argv[clean_idx];
			}
			delete[] utf8_argv;
