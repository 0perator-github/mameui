...
// Some of the above headers define macros that conflict with MAMEUI's code, so they are undefined here to prevent compile errors.
#ifdef interface
#undef interface // undef interface which is defined in combaseapi.h and causes compile errors in device.h
#endif
#ifdef IN
#undef IN // undef IN which is defined in windef.h and causes compile errors in floppy.h
#endif
