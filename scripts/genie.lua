...
configuration { "mingw*" }
		if BUILD_UI_TARGET then -- MAMEUI: The project needs to be ported to c++, but for now disable ASLR.
			linkoptions {
				"-Wl,--allow-multiple-definition",
...
configuration { "vs20*" }
		...
		if BUILD_UI_TARGET then -- MAMEUI: Disable ASLR for visual studio too. !!!UNTESTED!!!
			linkoptions {
				"/DYNAMICBASE[:NO]",
				"/HIGHENTROPYVA[:NO]",
			}
...
if _OPTIONS["SOURCES"] ~= nil then
	...
	for word in string.gmatch(str, '([^,]+)') do
		...
		if (not os.isfile(fullpath)) and (not os.isdir(fullpath)) then
			word = path.join("src", PSEUDO_TARGET, word)
